 ...
 *
 * @package TMEM
 * @subpackage Classes/TMEM_DB_Availability_Meta
...
class TMEM_DB_Availability_Meta extends TMEM_DB {
	...
	public function __construct() {
		...
		$this->table_name  = $wpdb->prefix . 'tmem_availabilitymeta';
		...
		add_action( 'tmem_remove_employee_absence', array( $this, 'remove_meta_after_delete' ), 999, 2 );
	...
	public function register_table() {
		global $wpdb;
		$wpdb->tmem_availabilitymeta = $this->table_name;
	} // register_table

	/**
	 * Retrieve availability meta field for an entry.
	 *
	 * For internal use only. Use TMEM_Availabiility->get_meta() for public usage.
	 *
	...
	public function get_meta( $id = 0, $meta_key = '', $single = false ) {
		...
		return get_metadata( 'tmem_availability', $id, $meta_key, $single );
	} // get_meta

	/**
	 * Add meta data field to an entry.
	 *
	 * For internal use only. Use TMEM_DB_Availability->add_meta() for public usage.
	 *
	 * @param int   $id Entry ID.
	...
	public function add_meta( $id = 0, $meta_key = '', $meta_value, $unique = false ) {
		...
		return add_metadata( 'tmem_availability', $id, $meta_key, $meta_value, $unique );
	} // add_meta

	/**
	 * Update availability meta field based on Entry ID.
	 *
	 * For internal use only. Use TMEM_DB_Availability->update_meta() for public usage.
	 *
	...
	public function update_meta( $id = 0, $meta_key = '', $meta_value, $prev_value = '' ) {
		...
		return update_metadata( 'tmem_availability', $id, $meta_key, $meta_value, $prev_value );
	} // update_meta

	/**
	 * Remove metadata matching criteria from an entry.
	 *
	 * For internal use only. Use TMEM_DB_Availability->delete_meta() for public usage.
	 *
	...
	public function delete_meta( $id = 0, $meta_key = '', $meta_value = '' ) {
		return delete_metadata( 'tmem_availability', $id, $meta_key, $meta_value );
	} // delete_meta
	 ...
	 *
	 * @since 1.5.6
	 * @hook tmem_remove_employee_absence
 ...
 ",
		...
		foreach ( $meta_ids as $mid ) {
			delete_metadata_by_mid( 'tmem_availability', $mid );
		}
	} // delete_all_meta
	...
	public function create_table() {
		...
		$sql = "CREATE TABLE {$this->table_name} (
			...
			tmem_availability_id bigint(20) NOT NULL,
			meta_key varchar(255) DEFAULT NULL,
			meta_value longtext,
			PRIMARY KEY (meta_id),
			KEY tmem_availability_id (tmem_availability_id),
