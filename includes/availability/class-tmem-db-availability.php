...
/**
 * TMEM_DB_Availability Class
 *
 * @since 1.5.5
 */
class TMEM_DB_Availability extends TMEM_DB {
	...
	public function __construct() {
		...
		$this->table_name  = $wpdb->prefix . 'tmem_availability';
		$this->primary_key = 'id';
		$this->version     = '1.0';
	...
	public function get_required_fields() {
		return apply_filters(
			'tmem_db_availability_required_fields',
			array(
				'employee_id',
				'start',
				'end',
	...
	public function add( $data = array() ) {
		...
		if ( $return ) {
			foreach ( $meta as $key => $value ) {
				TMEM()->availability_meta->update_meta( $return, $key, $value );
			}
		}
	...
	public function get_entries( $args = array() ) {
		...
		if ( ! empty( $args['start'] ) ) {
			...
			if ( ! empty( $args['end'] ) ) {
				...
				$start = gmdate( 'Y-m-d H:i:s', $args['start'] );
				$end   = gmdate( 'Y-m-d H:i:s', $args['end'] );
				...
				$year  = gmdate( 'Y', strtotime( $args['end'] ) );
				$month = gmdate( 'm', strtotime( $args['end'] ) );
				$day   = gmdate( 'd', strtotime( $args['end'] ) );
		...
		$cache_key = md5( 'tmem_availability_' . serialize( $args ) );
