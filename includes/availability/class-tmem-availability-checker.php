...
/**
 * TMEM_Availability_Checker Class
 *
 * @since 1.3
 */
class TMEM_Availability_Checker {
	...
	public function setup_roles( $roles ) {
		$roles = ! empty( $roles ) ? $roles : tmem_get_availability_roles();
		$roles = ! empty( $roles ) ? $roles : tmem_get_roles( $roles );
	...
	public function setup_employees( $employees ) {
		$employees = ! empty( $employees ) ? $employees : tmem_get_employees( $this->roles );
	...
	public function setup_status( $status ) {
		$status = ! empty( $status ) ? $status : tmem_get_availability_statuses();
			...
			foreach ( $events as $event ) {
				...
				foreach ( $employees as $employee_id => $data ) {
					...
					$this->unavailable[ $employee_id ]['event'][ $event->ID ] = array(
						...
						'end'    => $tmem_event->get_finish_date(),
						'finish' => $tmem_event->get_finish_time(),
						'start'  => $tmem_event->get_start_time(),
	...
	public function get_absences_in_range() {
		...
		foreach ( $absences as $absence ) {
			...
			$title = tmem_do_absence_content_tags(
				tmem_get_calendar_absence_title(),
				$absence
			);

			$tip_title = tmem_do_absence_content_tags(
				tmem_get_calendar_absence_tip_title(),
				$absence
			);

			$notes = '<p>' . str_replace( PHP_EOL, '<br>', tmem_get_calendar_absence_tip_content() ) . '<p>';
			$notes = tmem_do_absence_content_tags(
				$notes,
				$absence
			);

			$actions = apply_filters( 'tmem_calendar_absence_actions', $actions );

			if ( tmem_employee_can( 'manage_employees' ) ) {
				$actions[] = sprintf(
					'<a class="tmem-delete availability-link delete-absence" data-entry="%d" href="#" >%s</a>',
					$absence->id,
			...
			$this->results[] = array(
				'allDay'          => $all_day,
				'backgroundColor' => tmem_get_calendar_color(),
				'borderColor'     => tmem_get_calendar_color( 'border' ),
				'className'       => 'tmem_calendar_absence',
				'end'             => $absence->end,
				'id'              => $absence->id,
				'notes'           => $notes,
				'start'           => $absence->start,
				'textColor'       => tmem_get_calendar_color( 'text' ),
				'tipTitle'        => $tip_title,
				'title'           => $title,
			);
	...
	public function get_events_in_range() {
		...
		$query_args = apply_filters( 'tmem_events_in_range_args', $query_args );
		$events     = tmem_get_events( $query_args );

		if ( $events ) {
			foreach ( $events as $event ) {
				$popover      = 'top';
				$tmem_event   = new TMEM_Event( $event->ID );
				$event_type   = $tmem_event->get_type();
				$event_status = $tmem_event->get_status();
				$employee     = tmem_get_employee_display_name( $tmem_event->employee_id );
				$event_id     = tmem_get_event_contract_id( $tmem_event->ID );
				$actions      = array();
				...
				$title = tmem_do_content_tags(
					tmem_get_calendar_event_title(),
					$tmem_event->ID,
					$tmem_event->client
				);

				$tip_title = tmem_do_content_tags(
					tmem_get_calendar_event_tip_title(),
					$tmem_event->ID,
					$tmem_event->client
				);

				$notes = '<p>' . str_replace( PHP_EOL, '<br>', tmem_get_calendar_event_tip_content() ) . '<p>';
				$notes = tmem_do_content_tags(
					$notes,
					$tmem_event->ID,
					$tmem_event->client
				);
				...
				$actions[] = sprintf(
					...
					$event_url,
					sprintf( esc_html__( 'View %s', 'mobile-events-manager' ), esc_html( tmem_get_label_singular( true ) ) )
				);

				$actions = apply_filters( 'tmem_calendar_event_actions', $actions );
				...
				$this->results[] = array(
					'allDay'          => false,
					'backgroundColor' => tmem_get_calendar_color( 'background', true ),
					'borderColor'     => tmem_get_calendar_color( 'border', true ),
					'className'       => 'tmem_calendar_event',
					'end'             => $tmem_event->get_finish_date() . ' ' . $tmem_event->get_finish_time(),
					'id'              => $tmem_event->ID,
					'notes'           => $notes,
					'start'           => $tmem_event->date . ' ' . $tmem_event->get_start_time(),
					'textColor'       => tmem_get_calendar_color( 'text', true ),
					'tipTitle'        => $tip_title,
					'title'           => $title,
				);
			}
