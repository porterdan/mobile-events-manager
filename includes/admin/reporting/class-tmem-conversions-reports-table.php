...
/**
 * TMEM_Conversions_Reports_Table Class
 *
 * Renders the Conversions Reports table
 *
 * @since 1.4
 */
class TMEM_Conversions_Reports_Table extends WP_List_Table {
	...
	public function __construct() {
		...
		add_action( 'tmem_reports_conversions_graph_additional_stats', array( $this, 'graph_totals' ) );
	}
	...
	public function get_columns() {
		$columns = array(
			...
			'total_events'      => __( 'Total Enquiries', 'mobile-events-manager' ),
			'total_conversions' => __( 'Successful Conversions', 'mobile-events-manager' ),
			'conversion_ratio'  => __( 'Conversion %', 'mobile-events-manager' ),
	...
	public function reports_data() {
		...
		if ( false !== $cached_reports ) {
			...
			foreach ( $categories as $category_id => $category ) {
				...
				$events     = tmem_get_events( $all_event_args );
				$statuses   = tmem_active_event_statuses();
				$statuses[] = 'tmem-completed';
				$statuses[] = 'tmem-cancelled';
				...
				if ( $events ) {
					foreach ( $events as $event ) {
						...
						if ( in_array( $tmem_event->post_status, $statuses ) ) {
							$current_value = $tmem_event->get_total_profit();
							$value        += $current_value;
						}
					}
				} else {
				...
				$reports_data[] = array(
					...
					'total_value'       => tmem_currency_filter( tmem_format_amount( $value ) ),
	...
	public function output_source_graph() {
		...
		arsort( $data );
		$data = apply_filters( 'tmem_sources_graph_data', $data );

		$options = apply_filters(
			'tmem_sources_graph_options',
			array(
	...
	public function output_earnings_graph() {
		...
		// Sort High to Low, prior to filter so people can reorder if they please
		arsort( $data );
		$data = apply_filters( 'tmem_sources_earnings_graph_data', $data );

		$options = apply_filters(
			'tmem_sources_earnings_graph_options',
			array(
	...
	public function graph_totals() {
		...
		<p class="tmem_graph_totals">
			<strong>
				<?php
					esc_html_e( 'Total enquiries for period shown: ', 'mobile-events-manager' );
		...
		<p class="tmem_graph_totals">
			<strong>
				<?php
					esc_html_e( 'Total Conversions for period shown: ', 'mobile-events-manager' );
		...
		<p class="tmem_graph_totals">
			<strong>
				<?php
					esc_html_e( 'Conversion ratio for period shown: ', 'mobile-events-manager' );
		...
		<p class="tmem_graph_totals">
			<strong>
				<?php
					esc_html_e( 'Total Value for period shown: ', 'mobile-events-manager' );
					echo tmem_currency_filter( tmem_format_amount( $this->total_value ) );
				?>
			</strong>
		</p>
		<?php
...
} // TMEM_Conversions_Reports_Table
