...
class MEM_Graph {
	...
	*/
	...
	public function get_data() {
		return apply_filters( 'mem_get_graph_data', $this->data, $this );
	}
	...
	public function load_scripts() {
		...
		wp_enqueue_script( 'jquery-flot', MEM_PLUGIN_URL . '/assets/js/jquery.flot' . $suffix . '.js' );

		do_action( 'mem_graph_load_scripts' );
	}
...
endforeach;
				...
				function mem_flot_tooltip(x, y, contents) {
					$('<div id="mem-flot-tooltip">' + contents + '</div>').css( {
						position: 'absolute',
						display: 'none',
						top: y + 5,
						left: x + 5,
				...
				$("#mem-graph-<?php echo $this->id; ?>").bind("plothover", function (event, pos, item) {
					$("#x").text(pos.x.toFixed(2));
					$("#y").text(pos.y.toFixed(2));
					if (item) {
						if (previousPoint != item.dataIndex) {
							previousPoint = item.dataIndex;
							$("#mem-flot-tooltip").remove();
							var x = item.datapoint[0].toFixed(2),
							y = item.datapoint[1].toFixed(2);
							if( item.series.id == 'earnings' || item.series.id == 'income' || item.series.id == 'expense' ) {
								if( mem_admin_vars.currency_position == 'before' ) {
									mem_flot_tooltip( item.pageX, item.pageY, item.series.label + ' ' + mem_admin_vars.currency_sign + y );
								} else {
									mem_flot_tooltip( item.pageX, item.pageY, item.series.label + ' ' + y + mem_admin_vars.currency_sign );
								}
							} else {
								mem_flot_tooltip( item.pageX, item.pageY, item.series.label + ' ' + y.replace( '.00', '' ) );
							}
						}
					} else {
						$("#mem-flot-tooltip").remove();
		...
		<div id="mem-graph-<?php echo $this->id; ?>" class="mem-graph" style="height: 300px;"></div>
		<?php
		return ob_get_clean();
	}
	...
	public function display() {
		do_action( 'mem_before_graph', $this );
		echo $this->build_graph();
		do_action( 'mem_after_graph', $this );
	}
