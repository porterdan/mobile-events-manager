...
/**
 * TMEM_Packages_Reports_Table Class
 *
 * Renders the Conversions Reports table
 *
 * @since 1.4
 */
class TMEM_Packages_Reports_Table extends WP_List_Table {
				...
				foreach ( $packages as $package ) {
					...
					if ( $events ) {
						foreach ( $events as $event ) {
							...
							$total_value += tmem_get_package_price( $package->ID, $event_date );
						}
					} else {
						continue;
					}
					...
					$reports_data[] = array(
						'ID'        => $package->ID,
						'package'   => tmem_get_package_name( $package->ID ),
						'events'    => $event_count,
	...
	public function output_source_graph() {
		...
		arsort( $data );
		$data = apply_filters( 'tmem_packages_graph_data', $data );

		$options = apply_filters(
			'tmem_packages_graph_options',
			array(
	...
	public function output_earnings_graph() {
		...
		// Sort High to Low, prior to filter so people can reorder if they please
		arsort( $data );
		$data = apply_filters( 'tmem_packages_earnings_graph_data', $data );

		$options = apply_filters(
			'tmem_packages_earnings_graph_options',
			array(
