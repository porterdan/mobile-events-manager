...
/**
 * MEM_Batch_Export_Clients Class
 *
 * @since 1.4
 */
class MEM_Batch_Export_Employees extends MEM_Batch_Export {
	...
	public function csv_cols() {
		...
		$cols = array(
			...
			'events' => sprintf( esc_html__( 'Number of %s', 'mobile-events-manager' ), esc_html( mem_get_label_plural() ) ),
	...
	public function get_data() {
		...
		if ( $employees ) {
			foreach ( $employees as $employee ) {
				...
				$events = mem_get_employee_events( $employee->ID );
				$wages  = 0;
				$paid   = 0;
				...
				if ( $events ) {
					foreach ( $events as $event ) {
						$event_wage = mem_get_employees_event_wage( $event->ID, $employee->ID );
						$wages     += $event_wage;

						if ( ! empty( $event_wage ) && 'paid' == mem_get_employees_event_payment_status( $event->ID, $employee->ID ) ) {
							$paid += $wages;
						}
					}
				}

				$data[ $i ]['wages'] = mem_format_amount( $wages );
				$data[ $i ]['paid']  = mem_format_amount( $paid );
				$data[ $i ]['owed']  = mem_format_amount( $wages - $paid );
	...
	public function get_percentage_complete() {
		...
		$total = mem_employee_count();// count( mem_get_employees() );
		error_log( $total );
		if ( $total > 0 ) {
