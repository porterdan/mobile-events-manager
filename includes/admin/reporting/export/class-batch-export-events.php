...
/**
 * MEM_Batch_Export_Events Class
 *
 * @since 1.4
 */
class MEM_Batch_Export_Events extends MEM_Batch_Export {
	...
	public function get_data() {
		...
		if ( $events ) {
			...
			foreach ( $events as $event ) {
				...
				$event_data = mem_get_event_data( $event->ID );
				$employees  = array();
				$package    = '';
				$addons     = array();

				if ( ! empty( $event_data['client'] ) ) {
					$client = '(' . $event_data['client'] . ') ' . mem_get_client_display_name( $event_data['client'] );
				}
				if ( ! empty( $event_data['employees']['primary_employee'] ) ) {
					$primary_employee = '(' . $event_data['employees']['primary_employee'] . ') ' . mem_get_employee_display_name( $event_data['employees']['primary_employee'] );
				}
				if ( ! empty( $event_data['employees']['employees'] ) ) {
					foreach ( $event_data['employees']['employees'] as $employee_id => $employee_data ) {
						$employees[] = '(' . $employee_id . ') ' . mem_get_employee_display_name( $employee_id );
					}
				}
				...
				if ( ! empty( $event_data['equipment']['addons'] ) ) {
					foreach ( $event_data['equipment']['addons'] as $addon_id ) {
						$addons[] = mem_get_addon_name( $addon_id );
					}
				}

				$data[ $i ] = array(
					'id'               => $event->ID,
					'event_id'         => mem_get_event_contract_id( $event->ID ),
					'date'             => mem_format_short_date( $event_data['date'] ),
					'status'           => $event_data['status'],
					'client'           => $client,
					'primary_employee' => '(' . $event_data['employees']['primary_employee'] . ') ' . mem_get_client_display_name( $event_data['employees']['primary_employee'] ),
					'employees'        => implode( ',', $employees ),
					'package'          => $package,
					'addons'           => implode( ', ', $addons ),
					'cost'             => mem_format_amount( $event_data['cost']['cost'] ),
					'deposit'          => mem_format_amount( $event_data['cost']['deposit'] ),
					'deposit_status'   => $event_data['cost']['deposit_status'],
					'balance'          => mem_format_amount( $event_data['cost']['balance'] ),
					'balance_status'   => $event_data['cost']['balance_status'],
					'start_time'       => mem_format_time( $event_data['start_time'] ),
					'end_time'         => mem_format_time( $event_data['end_time'] ),
					'end_date'         => mem_format_short_date( $event_data['end_date'] ),
					'setup_date'       => mem_format_short_date( $event_data['setup_date'] ),
					'setup_time'       => mem_format_time( $event_data['setup_time'] ),
	...
	public function get_percentage_complete() {
		...
		add_filter( 'mem_event_count_args', array( $this, 'filter_count_args' ) );
		$total = mem_event_count();
		remove_filter( 'mem_event_count_args', array( $this, 'filter_count_args' ) );
