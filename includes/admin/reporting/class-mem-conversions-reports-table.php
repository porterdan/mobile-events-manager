 ...
 * @since 1.4
 */
class MEM_Conversions_Reports_Table extends WP_List_Table {
	...
	public function __construct() {
		...
		add_action( 'mem_reports_conversions_graph_additional_stats', array( $this, 'graph_totals' ) );
	}
	...
	public function reports_data() {
		...
		if ( false !== $cached_reports ) {
			...
			foreach ( $categories as $category_id => $category ) {
				...
				$statuses[] = 'mem-completed';
				$statuses[] = 'mem-cancelled';
				...
				if ( $events ) {
					foreach ( $events as $event ) {
						...
						if ( in_array( $mem_event->post_status, $statuses ) ) {
							$current_value = $mem_event->get_total_profit();
							$value        += $current_value;
						}
					}
				} else {
				...
				$reports_data[] = array(
					...
					'total_value'       => mem_currency_filter( mem_format_amount( $value ) ),
	...
	public function output_source_graph() {
		...
		arsort( $data );
		$data = apply_filters( 'mem_sources_graph_data', $data );

		$options = apply_filters(
			'mem_sources_graph_options',
			array(
	...
	public function output_earnings_graph() {
		...
		// Sort High to Low, prior to filter so people can reorder if they please
		arsort( $data );
		$data = apply_filters( 'mem_sources_earnings_graph_data', $data );

		$options = apply_filters(
			'mem_sources_earnings_graph_options',
			array(
		...
		<p class="mem_graph_totals">
			<strong>
				<?php
					...
					echo mem_currency_filter( mem_format_amount( $this->total_value ) );
				?>
			</strong>
		</p>
		<?php
