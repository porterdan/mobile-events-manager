 ...
 * @since 1.4
 */
class TMEM_Pie_Graph extends TMEM_Graph {
	...
	*/
	...
	public function __construct( $_data, $options = array() ) {
		...
		$defaults = array(
			...
			'label_formatter'  => 'tmemLabelFormatter',
		...
		add_action( 'tmem_graph_load_scripts', array( $this, 'load_additional_scripts' ) );
	...
	public function load_additional_scripts() {
		...
		wp_enqueue_script( 'jquery-flot-pie', TMEM_PLUGIN_URL . '/assets/js/jquery.flot.pie' . $suffix . '.js' );
		...
		if ( count( $this->data ) ) {
			...
			<script type="text/javascript">
				...
				jQuery( document ).ready( function($) {
					var <?php echo $this->id; ?>Chart = $('#tmem-pie-graph-<?php echo $this->id; ?>');
					...
					$(<?php echo $this->id; ?>Chart).on('plothover', function (event, pos, item) {
						$('.tmem-legend-item-wrapper').css('background-color', 'inherit');
						if ( item ) {
							var label = item.series.label;
							var id = item.series.tmem_vars.id;

							var slug = label.toLowerCase().replace(/\s/g, '-');
							var legendTarget = '#' + id + slug;

							$('.tmem-legend-item-wrapper' + legendTarget).css('background-color', '#f0f0f0');
						}
					});
					<?php endif; ?>
				});
			...
			<div class="tmem-pie-graph-wrap">
				<div id="tmem-pie-graph-<?php echo $this->id; ?>" class="tmem-pie-graph" style="height: <?php echo $this->options['height']; ?>px;"></div>
				<div id="tmem-pie-legend-<?php echo $this->id; ?>" class="tmem-pie-legend"></div>
			</div>
			<?php
		}
		return apply_filters( 'tmem_pie_graph_output', ob_get_clean(), $this->id, $this->data, $this->options );
