 ...
 * @since 1.4
 */
class TMEM_Transaction_Types_Reports_Table extends WP_List_Table {
	...
	public function __construct() {
		...
		add_action( 'tmem_reports_txn_types_additional_stats', array( $this, 'graph_totals' ) );
	}
	...
	public function reports_data() {
		...
		if ( false !== $cached_reports ) {
			...
			foreach ( $categories as $category_id => $category ) {
				...
				$txns = tmem_get_txns( $txn_args );

				if ( $txns ) {
					foreach ( $txns as $txn ) {
						$txn_count++;

						$tmem_txn = new TMEM_Txn( $txn );

						if ( 'tmem-income' == $tmem_txn->post_status ) {
							$total_income += $tmem_txn->price;
						} else {
							$total_expense += $tmem_txn->price;
						}

						$total_value += $tmem_txn->price;
				...
				$reports_data[] = array(
					...
					'total_income'       => tmem_currency_filter( tmem_format_amount( $total_income ) ),
					'total_income_raw'   => $total_income,
					'total_expense'      => tmem_currency_filter( tmem_format_amount( $total_expense ) ),
					'total_expense_raw'  => $total_expense,
					'total_value'        => tmem_currency_filter( tmem_format_amount( $total_value ) ),
	...
	public function output_types_graph() {
		...
		arsort( $data );
		$data = apply_filters( 'tmem_txn_types_graph_data', $data );

		$options = apply_filters(
			'tmem_txn_types_graph_options',
			array(
	...
	public function output_values_graph() {
		...
		// Sort High to Low, prior to filter so people can reorder if they please
		arsort( $data );
		$data = apply_filters( 'tmem_txn_value_graph_data', $data );

		$options = apply_filters(
			'tmem_txn_value_graph_options',
			array(
		...
		<p class="tmem_graph_totals">
			<strong>
				<?php
					...
					echo tmem_currency_filter( tmem_format_amount( $this->total_txn_income ) );
				?>
			</strong>
		</p>
		<p class="tmem_graph_totals">
			<strong>
				<?php
					...
					echo tmem_currency_filter( tmem_format_amount( $this->total_txn_expense ) );
				?>
			</strong>
		</p>
		<p class="tmem_graph_totals">
			<strong>
				<?php
					...
					echo tmem_currency_filter( tmem_format_amount( $this->total_txn_income - $this->total_txn_expense ) );
				?>
			</strong>
		</p>
		<p class="tmem_graph_totals">
...
} // TMEM_Transaction_Types_Reports_Table
