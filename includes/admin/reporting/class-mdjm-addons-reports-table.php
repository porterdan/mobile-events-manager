...
/**
 * MEM_Addons_Reports_Table Class
 *
 * Renders the Conversions Reports table
 *
 * @since 1.4
 */
class MEM_Addons_Reports_Table extends WP_List_Table {
				...
				foreach ( $addons as $addon ) {
					...
					if ( $events ) {
						foreach ( $events as $event ) {
							...
							$total_value += mem_get_addon_price( $addon->ID, $event_date );
						}
					} else {
						continue;
					}
					...
					$reports_data[] = array(
						'ID'        => $addon->ID,
						'addon'     => mem_get_addon_name( $addon->ID ),
						'events'    => $event_count,
	...
	public function output_source_graph() {
		...
		arsort( $data );
		$data = apply_filters( 'mem_addon_graph_data', $data );

		$options = apply_filters(
			'mem_addon_graph_options',
			array(
	...
	public function output_earnings_graph() {
		...
		// Sort High to Low, prior to filter so people can reorder if they please
		arsort( $data );
		$data = apply_filters( 'mem_addon_earnings_graph_data', $data );

		$options = apply_filters(
			'mem_addon_earnings_graph_options',
			array(
