 ...
 * @return void
 */
function mem_earnings_reports_graph() {
	...
	$data = array(
		...
		esc_html( mem_get_label_plural() )   => $events_data,
	);
		...
		<div class="metabox-holder" style="padding-top: 0;">
			<div class="postbox">
				...
				<div class="inside">
					...
					<p class="mem_graph_totals">
						<strong>
							<?php
								...
								echo mem_currency_filter( mem_format_amount( $earnings_totals ) );
							?>
						</strong>
					</p>
					<p class="mem_graph_totals">
					...
					<?php do_action( 'mem_reports_earnings_graph_additional_stats' ); ?>
 ...
 * @return void
 */
function mem_transactions_reports_graph() {
	// Retrieve the queried dates
	$dates = mem_get_report_dates();
		...
		<div class="metabox-holder" style="padding-top: 0;">
			<div class="postbox">
				...
				<div class="inside">
					...
					<p class="mem_graph_totals">
						<strong>
							<?php
								...
								echo mem_currency_filter( mem_format_amount( $income_totals ) );
							?>
						</strong>
					</p>
					<p class="mem_graph_totals">
						<strong>
							<?php
								...
								echo mem_currency_filter( mem_format_amount( $expense_totals ) );
							?>
						</strong>
					</p>
					<p class="mem_graph_totals">
						<strong>
							<?php
								esc_html_e( 'Total earnings for period shown: ', 'mobile-events-manager' );
								echo mem_currency_filter( mem_format_amount( $income_totals - $expense_totals ) );
							?>
						</strong>
					</p>
					<p class="mem_graph_totals">
					...
					<?php do_action( 'mem_reports_transactions_graph_additional_stats' ); ?>
 ...
 * @return void
 */
function mem_reports_graph_controls() {
	$date_options = apply_filters(
		'mem_report_date_options',
		array(
	...
	?>
	<form id="mem-graphs-filter" method="get">
		<div class="tablenav top">
			<div class="alignleft actions">
				...
				<select id="mem-graphs-date-options" name="range">
				...
				<div id="mem-date-range-options" <?php echo $display; ?>>
					<span><?php esc_html_e( 'From', 'mobile-events-manager' ); ?>&nbsp;</span>
					<select id="mem-graphs-month-start" name="m_start">
						<?php for ( $i = 1; $i <= 12; $i++ ) : ?>
							<option value="<?php echo absint( $i ); ?>" <?php selected( $i, $dates['m_start'] ); ?>><?php echo mem_month_num_to_name( $i ); ?></option>
						<?php endfor; ?>
					</select>
					<select id="mem-graphs-day-start" name="day">
						<?php for ( $i = 1; $i <= 31; $i++ ) : ?>
							<option value="<?php echo absint( $i ); ?>" <?php selected( $i, $dates['day'] ); ?>><?php echo $i; ?></option>
						<?php endfor; ?>
					</select>
					<select id="mem-graphs-year-start" name="year">
						<?php for ( $i = 2007; $i <= gmdate( 'Y' ); $i++ ) : ?>
							<option value="<?php echo absint( $i ); ?>" <?php selected( $i, $dates['year'] ); ?>><?php echo $i; ?></option>
						<?php endfor; ?>
					</select>
					<span><?php esc_html_e( 'To', 'mobile-events-manager' ); ?>&nbsp;</span>
					<select id="mem-graphs-month-end" name="m_end">
						<?php for ( $i = 1; $i <= 12; $i++ ) : ?>
							<option value="<?php echo absint( $i ); ?>" <?php selected( $i, $dates['m_end'] ); ?>><?php echo mem_month_num_to_name( $i ); ?></option>
						<?php endfor; ?>
					</select>
					<select id="mem-graphs-day-end" name="day_end">
						<?php for ( $i = 1; $i <= 31; $i++ ) : ?>
							<option value="<?php echo absint( $i ); ?>" <?php selected( $i, $dates['day_end'] ); ?>><?php echo $i; ?></option>
						<?php endfor; ?>
					</select>
					<select id="mem-graphs-year-end" name="year_end">
						<?php for ( $i = 2007; $i <= gmdate( 'Y' ); $i++ ) : ?>
				...
				<div class="mem-graph-filter-submit graph-option-section">
					<input type="hidden" name="mem-action" value="filter_reports" />
...
function mem_get_report_dates() {
	...
	return apply_filters( 'mem_report_dates', $dates );
} // mem_get_report_dates
 ...
 * @param $data
 */
function mem_parse_report_dates( $data ) {
	$dates = mem_get_report_dates();

	$view = mem_get_reporting_view();
	$id   = isset( $_GET['event-id'] ) ? sanitize_text_field( wp_unslash( $_GET['event-id'] ) ) : null;

	wp_safe_redirect( add_query_arg( $dates, admin_url( 'edit.php?post_type=mem-event&page=mem-reports&view=' . esc_attr( $view ) . '&event-id=' . absint( $id ) ) ) );
	wp_die();
} // mem_parse_report_dates
add_action( 'mem_filter_reports', 'mem_parse_report_dates' );
