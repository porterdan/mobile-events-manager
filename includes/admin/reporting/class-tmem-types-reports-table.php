...
/**
 * TMEM_Types_Reports_Table Class
 *
 * Renders the Event Types Reports table
 *
 * @since 1.4
 */
class TMEM_Types_Reports_Table extends WP_List_Table {
			...
			foreach ( $categories as $category_id => $category ) {
				...
				$all_event_args = array(
					'post_status' => apply_filters(
						'tmem_events_by_type_statuses',
						array( 'tmem-contract', 'tmem-approved', 'tmem-completed' )
					),
					'fields'      => 'ids',
					'tax_query'   => array(
						array(
				...
				if ( $events ) {
					foreach ( $events as $event ) {
						...
						$current_average_earnings = $current_earnings = $tmem_event->get_total_profit();
				...
				$avg_earnings = round( $avg_earnings / $event_count, tmem_currency_decimal_filter() );

				$reports_data[] = array(
					'ID'                 => $category->term_id,
					'type'               => $category->name,
					'total_events'       => $event_count,
					'total_earnings'     => tmem_currency_filter( tmem_format_amount( $earnings ) ),
					'total_earnings_raw' => $earnings,
					'avg_earnings'       => tmem_currency_filter( tmem_format_amount( $avg_earnings ) ),
					'is_child'           => false,
				);
	...
	public function output_source_graph() {
		...
		arsort( $data );
		$data = apply_filters( 'tmem_types_graph_data', $data );

		$options = apply_filters(
			'tmem_types_graph_options',
			array(
	...
	public function output_earnings_graph() {
		...
		// Sort High to Low, prior to filter so people can reorder if they please
		arsort( $data );
		$data = apply_filters( 'tmem_types_earnings_graph_data', $data );

		$options = apply_filters(
			'tmem_types_earnings_graph_options',
			array(
