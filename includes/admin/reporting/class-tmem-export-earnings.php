 ...
 * @since 1.4
 */
class TMEM_Earnings_Export extends TMEM_Export {
	...
	public function headers() {
		...
		header( 'Content-Disposition: attachment; filename=' . apply_filters( 'tmem_earnings_export_filename', 'tmem-export-' . $this->export_type . '-' . gmdate( 'n' ) . '-' . gmdate( 'Y' ) ) . '.csv' );
		header( 'Expires: 0' );
	...
	public function csv_cols() {
		...
		$cols = array(
			...
			// 'events' => esc_html( tmem_get_label_plural() ),
			'earnings' => __( 'Earnings', 'mobile-events-manager' ) . ' (' . html_entity_decode( tmem_currency_filter( '' ) ) . ')',
		);
	...
	public function get_data() {
		...
		while ( $year <= $end_year ) {
			...
			while ( $m1 <= $m2 ) {
				...
				$event_status = array_keys( tmem_all_event_status() );

				$data[] = array(
					'date'     => date_i18n( 'F Y', $date1 ),
					// 'events' => $stats->get_events_by_date( null, $m1, $year ),
					'earnings' => tmem_format_amount( $stats->get_earnings( $m1, $year ) ),
				);
