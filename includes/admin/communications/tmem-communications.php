<?php
/**
 * Class Name: TMEM_Communication
 * Manages TMEM email communications
 *
 * @package TMEM
 */
...
class TMEM_Communication {
	 ...
	 *
	 * @param arr $args Adds communication data
	...
	public function insert_comm( $args ) {
		...
		if ( TMEM_DEBUG === true ) {
			TMEM()->debug->log_it( 'Starting ' . __METHOD__, true );
		}

		if ( empty( $args ) || ! is_array( $args ) ) {
			return tmem_update_notice( 'update-nag', 'The communication was not logged' );
		}
		...
		if ( empty( $post_args['post_title'] ) ) {
			return tmem_update_notice( 'update-nag', 'The communication was not logged - no title ' . $post_args['post_title'] );
		}
		if ( empty( $post_args['post_content'] ) ) {
			return tmem_update_notice( 'update-nag', 'The communication was not logged - no content ' . $post_args['post_content'] );
		}
		if ( empty( $meta_args['recipient'] ) ) {
			return tmem_update_notice( 'update-nag', 'The communication was not logged - no recipient ' . $post_args['recipient'] );
		}
		if ( empty( $meta_args['source'] ) ) {
			return tmem_update_notice( 'update-nag', 'The communication was not logged - no source ' . $post_args['source'] );
		}

		$post_args['post_type']      = 'tmem_communication';
		$post_args['ping_status']    = false;
		$post_args['comment_status'] = 'closed';

		if ( TMEM_DEBUG === true ) {
			TMEM()->debug->log_it( '	-- Inserting COMM Post' );
		}
		...
		if ( $comm_post_id ) {
			if ( TMEM_DEBUG === true ) {
				TMEM()->debug->log_it( '	-- COMM post created ' . $comm_post_id );
			}
		...
		if ( ! empty( $args['attachments'] ) && is_array( $args['attachments'] ) ) {
			if ( TMEM_DEBUG === true ) {
				TMEM()->debug->log_it( '	-- This email has attachments' );
			}
			...
			foreach ( $args['attachments'] as $file ) {
				...
				if ( ! file_exists( $file ) ) {
					if ( TMEM_DEBUG === true ) {
						TMEM()->debug->log_it( '	-- ERROR: Attachment not found, will not process' );
					}
				...
				if ( $attach_id ) {
					if ( TMEM_DEBUG === true ) {
						TMEM()->debug->log_it( '	-- Attachment post inserted successfully. ID ' . $attach_id );
					}
				} else {
					if ( TMEM_DEBUG === true ) {
						TMEM()->debug->log_it( '	-- Attachment post could not be inserted' );
					}
				}
	...
	public function insert_stat_image( $p ) {
		if ( empty( $p ) || tmem_get_option( 'track_client_emails' ) !== true ) {
			return;
		}

		$stat = sprintf( '<img alt="" src="' . home_url() . '/?tmem-api=%s&post=%s&action=%s" border="0" height="3" width="37" />', 'TMEM_EMAIL_RCPT', $p, 'open_email' );
	...
	public function track_email_open( $p ) {
		...
		if ( empty( $stat_image ) ) {
			$stat_image = TMEM_PLUGIN_DIR . '/assets/images/invpicture.png';
		}
