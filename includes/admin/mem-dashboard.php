 ...
 *
 * @since 1.0
 * A class to produce the MEM Dashboard Overview
 *
 * @version 1.0
 * @21/02/2015
 *
...
class MEM_Dashboard {
	...
	public function all_events_by_status( $status = '', $period = '', $type = false, $user_id = '' ) {
		...
		if ( ! empty( $type ) && empty( $user_id ) ) {
			if ( MEM_DEBUG == true ) {
				MEM()->debug->log_it( 'ERROR: No user ID was provided in ' . __METHOD__, true );
			}
	...
	public function period_earnings( $period = '', $user_id = '', $earned = '' ) {
		...
		if ( empty( $period ) ) {
			if ( MEM_DEBUG == true ) {
				MEM()->debug->log_it( 'ERROR: No period was provided in ' . __METHOD__, true );
			}
		} elseif ( 'week' === $period ) {
		...
		foreach ( $events as $event ) {
			$event_cost = get_post_meta( $event->ID, '_mem_event_cost', true );
			$earnings  += $event_cost;

			if ( ! empty( $earnings ) ) {
				$transactions = MEM()->txns->get_event_transactions( $event->ID );
				foreach ( $transactions as $transaction ) {
					$status = get_post_meta( $transaction->ID, '_mem_txn_status', true );
					if ( empty( $status ) || 'Completed' === $status ) {
						$txn_total = get_post_meta( $transaction->ID, '_mem_txn_total', true );
					}

					if ( ! empty( $txn_total ) ) {
						if ( 'mem-income' === $transaction->post_status ) {
							$earnings += $txn_total;
						} else {
							$earnings -= $txn_total;
						}
		...
		return mem_currency_filter( mem_sanitize_amount( $earnings ) );
	} // period_earnings

} // Class

	/* -- Insantiate the MEM_Dashboard class -- */
	$mem_dash = new MEM_Dashboard();
