 ...
 */
function tmem_contract_post_columns( $columns ) {
	...
	if ( ! tmem_employee_can( 'manage_templates' ) && isset( $columns['cb'] ) ) {
		unset( $columns['cb'] );
	}

	return $columns;
} // tmem_contract_post_columns
add_filter( 'manage_contract_posts_columns', 'tmem_contract_post_columns' );
 ...
 * @return
 */
function tmem_contract_posts_custom_column( $column_name, $post_id ) {

	switch ( $column_name ) {
		// Is Default?
		case 'event_default':
			$event_default = tmem_get_option( 'default_contract' );
		...
		case 'assigned':
			...
			echo $total . sprintf( _n( ' %1$s', ' %2$s', $total, 'mobile-events-manager' ), esc_html( tmem_get_label_singular() ), esc_html( tmem_get_label_plural() ) );

			break;
	} // switch

} // tmem_contract_posts_custom_column
add_action( 'manage_contract_posts_custom_column', 'tmem_contract_posts_custom_column', 10, 2 );
 ...
 */
function tmem_contract_post_row_actions( $actions, $post ) {
...
} // tmem_contract_post_row_actions
add_filter( 'post_row_actions', 'tmem_contract_post_row_actions', 10, 2 );
 ...
 */
function tmem_contract_title_placeholder( $title ) {
	global $post;
...
} // tmem_contract_title_placeholder
add_filter( 'enter_title_here', 'tmem_contract_title_placeholder' );
 ...
 */
function tmem_contract_rename_publish_button( $translation, $text ) {
...
} // tmem_contract_rename_publish_button
add_filter( 'gettext', 'tmem_contract_rename_publish_button', 10, 2 );
 ...
 * @return void
 */
function tmem_save_contract_post( $post_id, $post, $update ) {
	...
	if ( ! tmem_employee_can( 'manage_templates' ) ) {
		...
		if ( TMEM_DEBUG == true ) {
			TMEM()->debug->log_it( 'PERMISSION ERROR: User ' . get_current_user_id() . ' is not allowed to edit contracts' );
		}
	...
	remove_action( 'save_post_contract', 'tmem_save_contract_post', 10, 3 );

	// Fire our pre-save hook
	do_action( 'tmem_pre_contract_save', $post_id, $post, $update );
	...
	do_action( 'tmem_post_contract_save', $post_id, $post, $update );

	// Re-add the save post action to avoid loops
	add_action( 'save_post_contract', 'tmem_save_contract_post', 10, 3 );
 ...
 */
function tmem_contract_post_messages( $messages ) {
	...
	return apply_filters( 'tmem_contract_post_messages', $messages );

} // tmem_contract_post_messages
add_filter( 'post_updated_messages', 'tmem_contract_post_messages' );
