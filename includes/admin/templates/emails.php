 ...
 */
function tmem_email_template_post_columns( $columns ) {
	...
	return $columns;
} // tmem_email_template_post_columns
add_filter( 'manage_email_template_posts_columns', 'tmem_email_template_post_columns' );
 ...
 */
function tmem_email_template_post_row_actions( $actions, $post ) {
...
} // tmem_email_template_post_row_actions
add_filter( 'post_row_actions', 'tmem_email_template_post_row_actions', 10, 2 );
 ...
 */
function tmem_email_template_title_placeholder( $title ) {
	global $post;
	...
	return __( 'Enter Template name here. Used as email subject, shortcodes allowed', 'mobile-events-manager' );

} // tmem_email_template_title_placeholder
add_filter( 'enter_title_here', 'tmem_email_template_title_placeholder' );
 ...
 */
function tmem_email_template_rename_publish_button( $translation, $text ) {

	global $post;

	if ( ! isset( $post ) || 'tmem-quotes' != $post->post_type ) {
		return $translation;
	}
	...
	if ( 'Publish' === $text ) {
		return __( 'Save Template', 'mobile-events-manager' );
	} elseif ( 'Update' === $text ) {
		return __( 'Update Template', 'mobile-events-manager' );
	} else {
		return $translation;
	}

} // tmem_event_rename_publish_button
add_filter( 'gettext', 'tmem_email_template_rename_publish_button', 10, 2 );
 ...
 */
function tmem_email_template_post_messages( $messages ) {
	...
	return apply_filters( 'tmem_email_template_post_messages', $messages );

} // tmem_email_template_post_messages
add_filter( 'post_updated_messages', 'tmem_email_template_post_messages' );
