			...
			array(
				'main' => array(
					...
					'time_format'      => array(
						...
						'desc'    => sprintf( __( 'Select the format in which you want your %s times displayed. Applies to both admin and client pages', 'mobile-dj-manager' ), tmem_get_label_singular( true ) ),
						'type'    => 'select',
						'options' => array(
			...
			array(
				'main' => array(
					'event_settings'  => array(
						'id'         => 'event_settings',
						'name'       => '<h3>' . sprintf( __( '%s Settings', 'mobile-dj-manager' ), tmem_get_label_singular() ) . '</h3>',
						'desc'       => '',
						'type'       => 'header'
					),
					'event_prefix'     => array(
						'id'          => 'event_prefix',
						'name'        => sprintf( __( '%s Prefix', 'mobile-dj-manager' ), tmem_get_label_singular() ),
						'desc'        => sprintf( __( 'The prefix you enter here will be added to each unique %s, contract and invoice ID', 'mobile-dj-manager' ), tmem_get_label_singular( true ) ),
						'type'        => 'text',
						'size'        => 'small'
					),
					'show_active_only'  => array(
						'id'          => 'show_active_only',
						'name'        => sprintf( __( 'Hide Inactive %s?', 'mobile-dj-manager' ), tmem_get_label_plural() ),
						'desc'        => sprintf( __( 'Select to include only active %1$s within the <code>All</code> view on the %1$s screen.', 'mobile-dj-manager' ), tmem_get_label_plural( true ) ),
						'type'        => 'checkbox'
					),
					'employer'         => array(
						'id'          => 'employer',
					...
					'default_contract' => array(
						...
						'desc'        => sprintf( __( 'Select the default contract for your %s. Can be changed per %s', 'mobile-dj-manager' ), tmem_get_label_plural( true ), tmem_get_label_singular( true ) ),
						'type'        => 'select',
					...
					'events_order_by'  => array(
						...
						'desc'        => sprintf( __( 'Select how you want to see %s ordered within the %s admin list', 'mobile-dj-manager' ), tmem_get_label_plural( true ), tmem_get_label_singular( true ) ),
						'type'        => 'select',
						'options'     => array(
							'ID'          => __( 'Contract ID', 'mobile-dj-manager' ),
							'post_date'   => __( 'Creation Date', 'mobile-dj-manager' ),
							'event_date'  => sprintf( __( '%s Date', 'mobile-dj-manager' ), tmem_get_label_singular() ),
					...
					'set_client_inactive'  => array(
						...
						'desc'        => sprintf( __( 'Set a client to inactive when their %s is cancelled, rejected or marked as a failed enquiry and they have no other upcoming %s.', 'mobile-dj-manager' ), tmem_get_label_singular( true ), tmem_get_label_plural( true ) ),
						'type'        => 'checkbox',
						'std'         => '1'
					),
					'journaling'       => array(
						...
						'desc'        => sprintf( __( 'Log and track all client &amp; %s actions (recommended).', 'mobile-dj-manager' ), tmem_get_label_singular() ),
						'type'        => 'checkbox',
						'std'		  => '1'
					)
				),
				'playlist' => array(
					...
					'enable_playlists' => array(
						'id'          => 'enable_playlists',
						'name'        => sprintf( __( 'Enable %s Playlists by Default?', 'mobile-dj-manager' ), tmem_get_label_singular() ),
						'desc'        => sprintf( __( 'Check to enable Client Playlist features by default. Can be overridden per %s.', 'mobile-dj-manager' ), tmem_get_label_singular( true ) ),
						'type'        => 'checkbox',
						'std'         => '1'
					),
					'playlist_limit' => array(
						...
						'desc'        => sprintf( __( 'Set the global limit for the number of entries a playlist can contain or enter <code>0</code> for no limit. Can be overridden per %s', 'mobile-dj-manager' ), tmem_get_label_singular( true ) ),
						'type'        => 'number',
                        'size'	      => 'small',
                        'std'         => '0'
					),
					'close'           => array(
						...
						'desc'        => sprintf( __( 'Number of days before %s that the playlist should close to new entries.', 'mobile-dj-manager' ), tmem_get_label_singular( true ) ),
						'type'        => 'text',
						'size'        => 'small',
						'std'		  => '5'
					),
					'upload_playlists' => array(
						...
						'desc'        => sprintf( __( 'With this option checked, your playlist information will occasionally be transmitted back to the TMEM servers ' . 
										...
										'Only song, artist and the %s type information is transmitted.', 'mobile-dj-manager' ), tmem_get_label_singular( true ) ),
						'type'        => 'checkbox'
					)
				),
				// Packages & Addons
				'packages' => array(
					...
					'package_contact_btn' => array(
						...
						'hint'        => sprintf( __( 'Select to auto add a contact button to the end of package/addons post content. The link will redirect to the <code>Contact Page</code>page as defined in <a href="%s">Pages</a>', 'mobile-dj-manager' ), admin_url( 'admin.php?page=tmem-settings&tab=client_zone&section=pages' ) ),
						'desc'        => sprintf( __( 'If you use the <a href="%s" target="_blank">Dynamic Contact Forms</a> add-on, the package/addon will be auto selected on page load', 'mobile-dj-manager' ), 'http://tmem.co.uk/products/dynamic-contact-forms/' ),
						'type'        => 'checkbox'
					),
				...
				'travel' => array(
					...
					'travel_add_cost' => array(
						...
						'desc'        => sprintf( __( 'If selected, the travel cost will be added to the overall %s cost', 'mobile-dj-manager' ), tmem_get_label_singular( true ) ),
						'type'        => 'checkbox'
					),
					'travel_primary' => array(
						'id'          => 'travel_primary',
						'name'        => __( 'Primary Post/Zip Code', 'mobile-dj-manager' ), tmem_get_label_singular(),
					...
					'travel_status' => array(
						'id'          => 'travel_status',
						'name'        => sprintf( __( '%s Status', 'mobile-dj-manager' ), tmem_get_label_singular() ),
						'desc'        => sprintf( __( "CTRL (cmd on MAC) + Click to select which %s status' can have travel costs updated.", 'mobile-dj-manager' ), tmem_get_label_singular( true ) ),
					...
					'cost_per_unit' => array(
						'id'          => 'cost_per_unit',
						'name'        => sprintf( __( 'Cost per %s', 'mobile-dj-manager' ), tmem_travel_unit_label() ),
			...
			array(
				'main' => array(
					...
					'bcc_dj_to_client' => array(
						'id'          => 'bcc_dj_to_client',
						'name'        => sprintf( __( 'Copy %s in Client Emails?', 'mobile-dj-manager' ), tmem_get_option( 'artist', __( 'DJ', 'mobile-dj-manager' ) ) ),
						'desc'        => sprintf( __( 'Send a copy of client emails to the %s primary %s', 'mobile-dj-manager' ),
											tmem_get_label_plural( true ), tmem_get_option( 'artist', __( 'DJ', 'mobile-dj-manager' ) )
										),
				...
				'templates' => array(
					...
					'unavailable'      => array(
						...
						'desc'        => sprintf( __( 'This is the default template used when responding to enquiries that you are unavailable for the %s', 'mobile-dj-manager' ), tmem_get_label_singular( true ) ),
						'type'        => 'select',
                        'chosen'      => true,
						'options'     => tmem_list_templates( 'email_template' )
					),
					...
					'awaitingdeposit' => array(
                        'id'          => 'awaitingdeposit',
                        'name'        => sprintf( __( 'Awaiting %s Template', 'mobile-dj-manager' ), tmem_get_deposit_label() ),
                        'desc'        => sprintf( __( 'Select an email template to be used when sending the %s reminder to the client', 'mobile-dj-manager' ), tmem_get_deposit_label() ),
                        'type'        => 'select',
                        'chosen'      => true,
                        'options'     => tmem_list_templates( 'email_template' )
					),
					...
					'contract_to_client' => array(
						...
						'desc'        => sprintf( __( 'Do you want to auto send an email to the client when their %s changes to the <em>Awaiting Contract<em> status?', 'mobile-dj-manager' ), tmem_get_label_singular( true ) ),
						'type'        => 'checkbox',
						'std'         => '1'
					),
					'contract'         => array(
					...
					'email_dj_confirm' => array(
						'id'          => 'email_dj_confirm',
						'name'        => sprintf( __( '%s Booking Confirmation Template', 'mobile-dj-manager' ), tmem_get_option( 'artist', __( 'DJ', 'mobile-dj-manager' ) ) ),
						'desc'        => sprintf( __( 'Select an email template to be used when sending the Booking Confirmation to %s primary %s', 'mobile-dj-manager' ), tmem_get_label_plural( true ), tmem_get_option( 'artist', __( 'DJ', 'mobile-dj-manager' ) ) ),
						'type'        => 'select',
                        'chosen'      => true,
						'options'     => tmem_list_templates( 'email_template' )
					)
			...
			array(
				'main' => array(
					'client_zone_settings' => array(
						...
						'name'        => '<h3>' . sprintf( __( '%s Settings', 'mobile-dj-manager' ), tmem_get_application_name() ) . '</h3>',
						'desc'        => '',
						'type'        => 'header'
					),
					'app_name'         => array(
					...
					'pass_length'      => array(
						...
						'desc'        => sprintf( __( 'If opting to generate or reset a user password during %s creation, how many characters should the password be?', 'mobile-dj-manager' ), tmem_get_label_singular( true ) ),
						'type'        => 'select',
						'options'     => array(
							'5'  => '5',
							'6'  => '6',
				...
				'styles' => array(
					...
					'action_button_colour'      => array(
						...
						'desc'        => sprintf( __( 'Select your preferred colour for the %s action buttons', 'mobile-dj-manager' ), tmem_get_label_singular( true ) ),
						'type'        => 'select',
						'options'     => array(
				...
				'pages' => array(
					...
					'app_home_page'    => array(
						...
						'desc'        => sprintf( __( "Select the home page for the %s application. Needs to contain the shortcode %s[tmem-home]%s", 'mobile-dj-manager' ),
											tmem_get_application_name(),
											'<code>',
											'</code>' ),
						'type'        => 'select',
					...
					'quotes_page'      => array(
						...
						'desc'        => sprintf( __( "Select the page to use for online %s quotes. Needs to contain the shortcode <code>[tmem-quote]</code>", 'mobile-dj-manager' ), tmem_get_label_singular( true ) ),
						'type'        => 'select',
						'options'     => tmem_list_pages()
					),
					'contact_page'     => array(
					...
					'contracts_page'   => array(
						...
						'desc'        => sprintf( __( "Select your website's contracts page. Needs to contain the shortcode %s[tmem-contract]%s", 'mobile-dj-manager' ),
											'<code>',
											'</code>' ),
						'type'        => 'select',
						'options'     => tmem_list_pages()
					...
					'payments_page'    => array(
						...
						'desc'        => sprintf( __( "Select your website's payments page. Needs to contain the shortcode %s[tmem-payments]%s", 'mobile-dj-manager' ),
											'<code>',
											'</code>' ),
						'type'        => 'select',
						'options'     => tmem_list_pages()
					...
					'playlist_page'    => array(
						...
						'desc'        => sprintf( __( "Select your website's playlist page. Needs to contain the shortcode %s[tmem-playlist]%s", 'mobile-dj-manager' ),
											'<code>',
											'</code>' ),
						'type'        => 'select',
						'options'     => tmem_list_pages()
					...
					'profile_page'     => array(
						...
						'desc'        => sprintf( __( "Select your website's profile page. Needs to contain the shortcode %s[tmem-profile]%s", 'mobile-dj-manager' ),
											'<code>',
											'</code>' ),
						'type'        => 'select',
						'options'     => tmem_list_pages()
				...
				'availability' => array(
					...
					'availability_status' => array(
						...
						'desc'        => sprintf( __( "CTRL (cmd on MAC) + Click to select %s status' that you want availability checker to report as unavailable", 'mobile-dj-manager' ), tmem_get_label_singular( true ) ),
						'type'        => 'multiple_select',
						'options'     => tmem_all_event_status(),
			...
			array(
				'main' => array(
					...
					'deposit_settings' => array(
						'id'          => 'deposit_settings',
						'name'        => '<h3>' . sprintf( __( '%s Settings', 'mobile-dj-manager' ), tmem_get_deposit_label() ) . '</h3>',
						'desc'        => '',
						'type'        => 'header'
					),
					'deposit_type'     => array(
						...
						'desc'        => sprintf( __( 'If you require ' . tmem_get_deposit_label() . ' payments for your %s, how should they be calculated?', 'mobile-dj-manager' ),
											tmem_get_label_plural( true ) ),
						'type'        => 'select',
						'options'     => array(
							'0'          => 'Not required',
							'percentage' => '% ' . sprintf( __( 'of %s value', 'mobile-dj-manager' ), tmem_get_label_singular( true ) ),
					...
					'deposit_before_confirm'     => array(
						...
						'name'        => sprintf( __( 'Wait for %s', 'mobile-dj-manager' ), tmem_get_deposit_label() ),
						'desc'        => sprintf(
			...
			// Employee Payment Settings
				'employee_payments' => array(
					...
					'enable_employee_payments'  => array(
						...
						'desc'        => sprintf( __( 'Enable this option to be able to record employee wage payments for %s.', 'mobile-dj-manager' ), tmem_get_label_plural() ),
						'type'        => 'checkbox'
					),
					'employee_pay_status'        => array(
						'id'          => 'employee_pay_status',
						'name'        => __( 'Payment Statuses', 'mobile-dj-manager' ),
						'desc'        => sprintf( __( "CTRL (cmd on MAC) + Click to select %s status' that an event must be at before employee payments can be made.", 'mobile-dj-manager' ), tmem_get_label_singular( true ) ),
						'type'        => 'multiple_select',
						'options'     => tmem_all_event_status(),
					...
					'employee_auto_pay_complete' => array(
						...
						'name'        => sprintf( __( 'Pay when %s Completes', 'mobile-dj-manager' ), tmem_get_label_singular() ),
						'desc'        => sprintf( __( 'Enable this option to automatically pay employees once an %s completes.', 'mobile-dj-manager' ), tmem_get_label_singular() ),
						'type'        => 'checkbox'
					)
				),
			// Receipts
			...
			array(
				...
				'terms_conditions'     => array(
					...
					'agree_terms_heading' => array(
						...
						'size' => 'regular',
						'std'  => sprintf(
							__( 'Terms and Conditions for %s', 'mobile-dj-manager' ), tmem_get_label_plural()
						)
					),
			...
			array(
				'calendar'     => array(
                    ...
                    'event_background_color' => array(
						...
						'name'    => sprintf( __( '%s Background', 'mobile-dj-manager' ), tmem_get_label_singular() ),
						'desc'    => sprintf( __( 'Select the background color of %s entries.', 'mobile-dj-manager' ), tmem_get_label_singular( true ) ),
						'type'    => 'color',
                        'default' => '#2ea2cc',
                        'std'     => '#2ea2cc'
					),
                    'event_border_color' => array(
						...
						'name'    => sprintf( __( '%s Border', 'mobile-dj-manager' ), tmem_get_label_singular() ),
						'desc'    => sprintf( __( 'Select the border color of %s entries.', 'mobile-dj-manager' ), tmem_get_label_singular( true ) ),
						'type'    => 'color',
                        'default' => '#0074a2',
                        'std'     => '#0074a2'
					),
                    'event_text_color' => array(
						'id'      => 'event_text_color',
						'name'    => sprintf( __( '%s Text', 'mobile-dj-manager' ), tmem_get_label_singular() ),
						'desc'    => sprintf( __( 'Select the text color of %s entries.', 'mobile-dj-manager' ), tmem_get_label_singular( true ) ),
						'type'    => 'color',
                        'default' => '#ffffff',
                        'std'     => '#ffffff'
					),
...
function tmem_get_settings_tabs() {
	...
	$tabs['events']           = sprintf( __( '%s', 'mobile-dj-manager' ), tmem_get_label_plural() );
...
function tmem_get_registered_settings_sections() {
	...
	$sections = array(
		...
		'events'          => apply_filters( 'tmem_settings_sections_gateways', array(
			'main'               => sprintf( __( '%s Settings', 'mobile-dj-manager' ), tmem_get_label_singular() ),
		...
		'emails'          => apply_filters( 'tmem_settings_sections_emails', array(
			...
			'templates'          => sprintf( __( '%s Templates', 'mobile-dj-manager' ), tmem_get_label_singular() )
		) ),
		'client_zone'     => apply_filters( 'tmem_settings_sections_styles', array(
			'main'               => sprintf( __( '%s Settings', 'mobile-dj-manager' ), tmem_get_application_name() ),
			...
			if ( false === $license->success ) {
				...
				switch( $license->error ) {
					...
					case 'no_activations_left':
						...
						$class = 'error';
						$messages[] = sprintf( __( 'Your license key has reached its activation limit. <a href="%s">View possible upgrades</a> now.', 'mobile-dj-manager' ), 'https://tmem.co.uk/your-account/' );
					...
					default :
						...
						$messages[] = sprintf( __( 'There was an error with this license key: %s. Please <a href="%s">contact our support team</a>.', 'mobile-dj-manager' ), $error, 'https://tmem.co.uk/support' );
