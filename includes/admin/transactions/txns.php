 ...
 */
function mem_transaction_post_columns( $columns ) {
	...
	if ( ! mem_employee_can( 'edit_txns' ) && isset( $columns['cb'] ) ) {
		unset( $columns['cb'] );
	}

	return $columns;
} // mem_transaction_post_columns
add_filter( 'manage_mem-transaction_posts_columns', 'mem_transaction_post_columns' );
 ...
 */
function mem_transaction_post_sortable_columns( $sortable_columns ) {
...
} // mem_transaction_post_sortable_columns
add_filter( 'manage_edit-mem-transaction_sortable_columns', 'mem_transaction_post_sortable_columns' );
 ...
 * @return void
 */
function mem_transaction_post_order( $query ) {

	if ( ! is_admin() || 'mem-transaction' != $query->get( 'post_type' ) ) {
		return;
	}
	...
	switch ( $query->get( 'orderby' ) ) {
		...
		case 'txn_status':
			$query->set( 'orderby', '_mem_txn_status' );
			break;
		...
		case 'txn_value':
			$query->set( 'meta_key', '_mem_txn_total' );
			$query->set( 'orderby', 'meta_value_num' );
			break;

	}

} // mem_transaction_post_order
add_action( 'pre_get_posts', 'mem_transaction_post_order' );
 ...
 * @return void
 */
function mem_transaction_post_filtered( $query ) {

	global $pagenow;

	$post_type = isset( $_GET['post_type'] ) ? sanitize_key( wp_unslash( $_GET['post_type'] ) ) : '';

	if ( 'edit.php' != $pagenow || 'mem-transaction' != $post_type || ! is_admin() ) {
		return;
	}
	...
	if ( ! empty( $_GET['mem_filter_type'] ) ) {
		...
		$type = isset( $_GET['mem_filter_type'] ) ? sanitize_key( wp_unslash( $_GET['mem_filter_type'] ) ) : 0;
...
} // mem_transaction_post_filtered
add_filter( 'parse_query', 'mem_transaction_post_filtered' );
 ...
 * @return
 */
function mem_transaction_posts_custom_column( $column_name, $post_id ) {
	...
	switch ( $column_name ) {
		...
		// Direction
		case 'direction':
			if ( 'mem-income' == get_post_status( $post_id ) ) {
		...
		// Source
		case 'payee':
			echo mem_get_txn_recipient_name( $post_id );
		...
		// Value
		case 'txn_value':
			echo mem_currency_filter( mem_format_amount( get_post_meta( $post_id, '_mem_txn_total', true ) ) );
			break;

		// Status
		case 'txn_status':
			echo get_post_meta( $post_id, '_mem_txn_status', true );
			break;
	} // switch

} // mem_transaction_posts_custom_column
add_action( 'manage_mem-transaction_posts_custom_column', 'mem_transaction_posts_custom_column', 10, 2 );
 ...
 */
function mem_transaction_post_row_actions( $actions, $post ) {

	if ( 'mem-transaction' !== $post->post_type ) {
		return $actions;
	}
...
} // mem_transaction_post_row_actions
add_filter( 'post_row_actions', 'mem_transaction_post_row_actions', 10, 2 );
 ...
 * @param obj $post The WP_Post post object
 */
function mem_transaction_set_post_title( $post ) {
	...
	<script type="text/javascript">
		jQuery(document).ready(function($) {
			$("#title").val("<?php echo mem_get_event_contract_id( $post->ID ); ?>");
			$("#title").prop("readonly", true);
		});
	</script>
	<?php
} // mem_transaction_set_post_title
add_action( 'edit_form_after_title', 'mem_transaction_set_post_title' );
 ...
 */
function mem_transaction_rename_publish_button( $translation, $text ) {

	global $post;

	if ( ! isset( $post ) || 'mem-transaction' != $post->post_type ) {
		return $translation;
	}
...
} // mem_transaction_rename_publish_button
add_filter( 'gettext', 'mem_transaction_rename_publish_button', 10, 2 );
 ...
 * @return void
 */
function mem_transaction_type_filter_dropdown() {

	if ( ! isset( $_GET['post_type'] ) || 'mem-transaction' !== $_GET['post_type'] ) {
		return;
	}
	...
	<?php
		$current_value = isset( $_GET['mem_filter_type'] ) ? sanitize_key( wp_unslash( $_GET['mem_filter_type'] ) ) : '';
...
} // mem_transaction_type_filter_dropdown
add_action( 'restrict_manage_posts', 'mem_transaction_type_filter_dropdown' );
 ...
 * @return void
 */
function mem_save_txn_post( $post_id, $post, $update ) {
	global $mem_settings;
	...
	if ( ! mem_employee_can( 'edit_txns' ) ) {
		...
		if ( MEM_DEBUG == true ) {
			MEM()->debug->log_it( 'PERMISSION ERROR: User ' . get_current_user_id() . ' is not allowed to edit transactions' );
		}
	...
	do_action( 'mem_before_txn_save', $post_id, $post, $update );

	$trans_type = get_term( sanitize_key( wp_unslash( $_POST['mem_transaction_type'] ) ), 'transaction-types' );

	// Set the post data
	$trans_data['ID']          = $post_id;
	$trans_data['post_status'] = 'Out' === $_POST['transaction_direction'] ? 'mem-expenditure' : 'mem-income';
	...
	$trans_data['post_type']     = 'mem-transaction';
	$trans_data['post_category'] = array( sanitize_key( wp_unslash( $_POST['mem_transaction_type'] ) ) );

	// Set the post meta
	$trans_meta['_mem_txn_status'] = sanitize_text_field( wp_unslash( $_POST['transaction_status'] ) );
	$trans_meta['_mem_txn_source'] = sanitize_text_field( wp_unslash( $_POST['transaction_src'] ) );
	$trans_meta['_mem_txn_total']  = sanitize_text_field( wp_unslash( $_POST['transaction_amount'] ) );
	$trans_meta['_mem_txn_notes']  = sanitize_text_field( wp_unslash( $_POST['transaction_description'] ) );

	if ( 'In' === $_POST['transaction_direction'] ) {
		$trans_meta['_mem_payment_from'] = sanitize_text_field( wp_unslash( $_POST['transaction_payee'] ) );
	} elseif ( 'Out' === $_POST['transaction_direction'] ) {
		$trans_meta['_mem_payment_to'] = sanitize_text_field( wp_unslash( $_POST['transaction_payee'] ) );
	}

	$trans_meta['_mem_txn_currency'] = mem_get_currency();

	// Update the post
	if ( MEM_DEBUG == true ) {
		 MEM()->debug->log_it( 'Updating the transaction' );
	}
	...
	if ( MEM_DEBUG == true ) {
		 MEM()->debug->log_it( 'Setting the transaction type' );
	}

	wp_set_post_terms( sanitize_key( wp_unslash( $post_id, $_POST['mem_transaction_type'] ) ), 'transaction-types' );

	// Add the meta data
	if ( MEM_DEBUG == true ) {
		 MEM()->debug->log_it( 'Updating the transaction post meta' );
	}
	...
	do_action( 'mem_after_txn_save', $post_id, $post, $update );
 ...
 */
function mem_transaction_post_messages( $messages ) {
	...
	$url1 = '<a href="' . admin_url( 'edit.php?post_type=mem-transaction' ) . '">';
	$url2 = get_post_type_object( $post->post_type )->labels->singular_name;
	$url3 = '</a>';

	$messages['mem-transaction'] = array(
	...
	return apply_filters( 'mem_transaction_post_messages', $messages );

} // mem_transaction_post_messages
add_filter( 'post_updated_messages', 'mem_transaction_post_messages' );
