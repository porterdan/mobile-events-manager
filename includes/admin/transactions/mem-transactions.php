<?php
/**
 * mem-transactions.php
 * 25/02/2015
 *
 * @since 1.0
 * The class for MEM transactions
 *
 * @version 1.0
 */
class MEM_Transactions {
	...
	public function get_event_transactions( $event_id ) {
		...
		if ( empty( $event_id ) ) {
			if ( MEM_DEBUG == true ) {
				MEM()->debug->log_it( 'ERROR: No event ID provided in ' . __METHOD__, true );
			}
	...
	function list_event_transactions( $eventID, $display = 'list' ) {
		$txns = $this->get_transactions( $eventID, 'mem-income', 'Completed', 'obj' );
		...
		switch ( $display ) {
			case 'list':
				foreach ( $txns as $txn ) {
					...
					$output  = mem_currency_filter( mem_sanitize_amount( get_post_meta( $txn->ID, '_mem_txn_total', true ) ) );
					$output .= ' ';
					$output .= 'on ' . gmdate( mem_get_option( 'short_date_format' ), strtotime( $txn->post_date ) ) . ' (' . $txn_type . ')';
	...
	function get_transactions( $event_id, $direction = 'any', $status = 'Completed', $return = 'value' ) {
		...
		if ( $transactions ) {
			foreach ( $transactions as $txn ) {
				$total += get_post_meta( $txn->ID, '_mem_txn_total', true );
			}
		}
	...
	function add_transaction( $data, $meta = '', $direction = 'in', $type = '' ) {
		...
		if ( MEM_DEBUG == true ) {
			MEM()->debug->log_it( 'Beginning transaction add procedure in ' . __METHOD__, true );
		}

		// Define the post status
		$txn_status = ( strtolower( $direction ) == 'in' ? 'mem-income' : 'mem-expenditure' );

		// Define the source of payment
		if ( ! empty( $type ) ) {
			// $txn_type = get_term_by( 'name', $type, 'transaction-types' );
			if ( MEM_DEBUG == true ) {
				MEM()->debug->log_it( 'term found with ID ' . $type, true );
			}
		}

		/* -- Get the new post ID -- */
		$trans_post = get_default_post_to_edit( 'mem-transaction', true );

		$trans_data['ID']          = mem_get_option( 'event_prefix' ) . $trans_post->ID;
		$trans_data['post_title']  = mem_get_option( 'event_prefix' ) . $trans_post->ID;
		...
		$trans_meta['_mem_txn_status']      = ( ! empty( $meta['_mem_txn_status'] ) ? $meta['_mem_txn_status'] : 'Completed' );
		$trans_meta['_mem_txn_source']      = ( ! empty( $meta['_mem_txn_source'] ) ? $meta['_mem_txn_source'] : $mem_settings['payments']['default_type'] );
		$trans_meta['_mem_gw_txn_id']       = ( ! empty( $meta['_mem_gw_txn_id'] ) ? $meta['_mem_gw_txn_id'] : '' );
		$trans_meta['_mem_payer_firstname'] = ( ! empty( $meta['_mem_payer_firstname'] ) ?
			sanitize_text_field( ucfirst( stripslashes( $meta['_mem_payer_firstname'] ) ) ) : '' );

		$trans_meta['_mem_payer_lastname'] = ( ! empty( $meta['_mem_payer_lastname'] ) ?
			sanitize_text_field( ucfirst( stripslashes( $meta['_mem_payer_lastname'] ) ) ) : '' );

		$trans_meta['_mem_payer_email'] = ( ! empty( $meta['_mem_payer_email'] ) ?
			strtolower( $meta['_mem_payer_email'] ) : '' );

		$trans_meta['_mem_txn_net'] = ( ! empty( $meta['_mem_txn_net'] ) ?
												mem_format_amount( $meta['_mem_txn_net'] ) : '0.00' );

		$trans_meta['_mem_txn_currency'] = ( ! empty( $meta['_mem_txn_currency'] ) ?
			strtoupper( $meta['_mem_txn_currency'] ) : $mem_settings['payments']['currency'] );

		$trans_meta['_mem_txn_fee'] = ( ! empty( $meta['_mem_txn_fee'] ) ?
												mem_format_amount( $meta['_mem_txn_fee'] ) : '0.00' );

		$trans_meta['_mem_txn_total'] = ( ! empty( $meta['_mem_txn_total'] ) ?
												mem_format_amount( $meta['_mem_txn_total'] ) : '0.00' );

		$trans_meta['_mem_payment_to'] = ( ! empty( $meta['_mem_payment_to'] ) ?
												$meta['_mem_payment_to'] : '' );

		$trans_meta['_mem_payment_from'] = ( ! empty( $meta['_mem_payment_from'] ) ?
												$meta['_mem_payment_from'] : '' );

		// Add the transaction
		$txn_id = wp_update_post( $trans_data );
		if ( ! empty( $txn_id ) ) {
			if ( MEM_DEBUG == true ) {
				MEM()->debug->log_it( 'Added transaction with ID: ' . $txn_id );
			}
			...
			if ( ! empty( $type ) ) {
				...
				if ( MEM_DEBUG == true ) {
					MEM()->debug->log_it( 'Assigning transaction source ID ' . $type );
				}
			}
			...
			if ( ! empty( $trans_meta ) && is_array( $trans_meta ) ) {
				foreach ( $trans_meta as $key => $value ) {
					if ( add_post_meta( $txn_id, $key, $value ) ) {
						if ( MEM_DEBUG == true ) {
							MEM()->debug->log_it( 'Meta key ' . $key . ' added with value ' . $value );
						}
					} else {
						if ( MEM_DEBUG == true ) {
							MEM()->debug->log_it( 'Failed to add Meta key ' . $key . ' with value ' . $value );
						}
					}
				}
			}
			...
			if ( MEM_DEBUG == true ) {
				MEM()->debug->log_it( 'Failed to add transaction' );
			}
