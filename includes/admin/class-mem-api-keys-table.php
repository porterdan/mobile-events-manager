...
/**
 * MEM_API_Keys_Table Class
 *
 * Renders the API Keys table
 *
 * @since 1.4
 */
class MEM_API_Keys_Table extends WP_List_Table {
	...
	public function column_user( $item ) {
		...
		$actions['reissue'] = sprintf(
			'<a href="%s" class="mem-regenerate-api-key">%s</a>',
			esc_url(
				wp_nonce_url(
					add_query_arg(
						array(
							'user_id'          => $item['id'],
							'mem-action'      => 'process_api_key',
							'mem_api_process' => 'regenerate',
						)
					),
					'mem-api-nonce',
					'api_nonce'
		...
		$actions['revoke'] = sprintf(
			'<a href="%s" class="mem-revoke-api-key mem-delete">%s</a>',
			esc_url(
				wp_nonce_url(
					add_query_arg(
						array(
		...
		$actions = apply_filters( 'mem_api_row_actions', array_filter( $actions ) );
	...
	public function extra_tablenav( $which = '' ) {
		...
		?>
		<form id="api-key-generate-form" method="post" action="<?php echo esc_url_raw( admin_url( 'edit.php?post_type=mem-event&page=mem-tools&tab=api_keys' ) ); ?>">
			<?php mem_admin_action_field( 'process_api_key' ); ?>
			<input type="hidden" name="mem_api_process" value="generate" />
			<?php wp_nonce_field( 'mem-api-nonce', 'api_nonce' ); ?>
			<?php
			echo esc_attr(
				MEM()->html->users_dropdown(
					array(
						'name'             => 'user_id',
						'chosen'           => true,
	...
	public function query() {
		...
		$users = get_users(
			array(
				'meta_value' => 'mem_user_secret_key',
		...
		foreach ( $users as $user ) {
			...
			$keys[ $user->ID ]['key']    = MEM()->api->get_user_public_key( $user->ID );
			$keys[ $user->ID ]['secret'] = MEM()->api->get_user_secret_key( $user->ID );
			$keys[ $user->ID ]['token']  = MEM()->api->get_token( $user->ID );
	...
	public function total_items() {
		...
		if ( ! get_transient( 'mem-total-api-keys' ) ) {
			$total_items = $wpdb->get_var( "SELECT count(user_id) FROM $wpdb->usermeta WHERE meta_key = 'mem_user_secret_key'" );

			set_transient( 'mem-total-api-keys', $total_items, 60 * 60 );
		}

		return get_transient( 'mem-total-api-keys' );
	} // total_items
