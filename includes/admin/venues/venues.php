 ...
 */
function mem_venue_post_columns( $columns ) {
	...
	$columns = array(
		...
		'event_count' => sprintf( esc_html__( '%s', 'mobile-events-manager' ), esc_html( mem_get_label_plural() ) ),
		'info'        => __( 'Information', 'mobile-events-manager' ),
		'details'     => __( 'Details', 'mobile-events-manager' ),
	);

	if ( ! mem_employee_can( 'add_venues' ) && isset( $columns['cb'] ) ) {
		unset( $columns['cb'] );
	}

	return $columns;
} // mem_venue_post_columns
add_filter( 'manage_mem-venue_posts_columns', 'mem_venue_post_columns' );
 ...
 */
function mem_venue_post_sortable_columns( $sortable_columns ) {
...
} // mem_venue_post_sortable_columns
add_filter( 'manage_edit-mem-venue_sortable_columns', 'mem_venue_post_sortable_columns' );
 ...
 * @return void
 */
function mem_venue_post_order( $query ) {

	if ( ! is_admin() || 'mem-venue' != $query->get( 'post_type' ) ) {
		return;
	}
...
} // mem_venue_post_order
add_action( 'pre_get_posts', 'mem_venue_post_order' );
 ...
 * @return
 */
function mem_venue_posts_custom_column( $column_name, $post_id ) {
		...
		case 'event_count':
			$events_at_venue = get_posts(
				array(
					...
					'post_status' => array( 'mem-approved', 'mem-contract', 'mem-completed', 'mem-enquiry', 'mem-unattended' ),
				)
			);
...
} // mem_venue_posts_custom_column
add_action( 'manage_mem-venue_posts_custom_column', 'mem_venue_posts_custom_column', 10, 2 );
 ...
 */
function mem_venue_post_row_actions( $actions, $post ) {

	if ( 'mem-venue' !== $post->post_type ) {
		return $actions;
	}
...
} // mem_venue_post_row_actions
add_filter( 'post_row_actions', 'mem_venue_post_row_actions', 10, 2 );
 ...
 */
function mem_venue_bulk_action_list( $actions ) {

	unset( $actions['edit'] );

	return $actions;

} // mem_venue_bulk_action_list
add_filter( 'bulk_actions-edit-mem-venue', 'mem_venue_bulk_action_list' );
 ...
 * @param
 */
function mem_venue_remove_filters() {

	if ( ! isset( $_GET['post_type'] ) || 'mem-venue' !== $_GET['post_type'] ) {
		return;
	}
...
} // mem_venue_remove_filters
add_action( 'admin_head', 'mem_venue_remove_filters' );
 ...
 */
function mem_venue_title_placeholder( $title ) {
	global $post;

	if ( ! isset( $post ) || 'mem-venue' != $post->post_type ) {
		return $title;
	}

	return __( 'Enter Venue name here...', 'mobile-events-manager' );

} // mem_venue_title_placeholder
add_filter( 'enter_title_here', 'mem_venue_title_placeholder' );
 ...
 */
function mem_venue_rename_publish_button( $translation, $text ) {
...
} // mem_venue_rename_publish_button
add_filter( 'gettext', 'mem_venue_rename_publish_button', 10, 2 );
 ...
 * @return void
 */
function mem_save_venue_post( $post_id, $post, $update ) {
	...
	if ( ! mem_employee_can( 'add_venues' ) ) {
		...
		if ( MEM_DEBUG == true ) {
			MEM()->debug->log_it( 'PERMISSION ERROR: User ' . get_current_user_id() . ' is not allowed to edit venues' );
		}
	...
	do_action( 'mem_before_venue_save', $post_id, $post, $update );
	...
	do_action( 'mem_after_venue_save', $post_id, $post, $update );

	// Re-add the save post action to avoid loops
	add_action( 'save_post_mem-venue', 'mem_save_venue_post', 10, 3 );
 ...
 */
function mem_venue_post_messages( $messages ) {
	...
	$url1 = '<a href="' . admin_url( 'edit.php?post_type=mem-venue' ) . '">';
	$url2 = get_post_type_object( $post->post_type )->labels->singular_name;
	$url3 = '</a>';

	$messages['mem-venue'] = array(
	...
	return apply_filters( 'mem_venue_post_messages', $messages );

} // mem_venue_post_messages
add_filter( 'post_updated_messages', 'mem_venue_post_messages' );
