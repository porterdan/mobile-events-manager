 ...
 * @since 1.3
 */
function mem_add_wp_dashboard_widgets() {
	/* translators: %s: Company Name */
	wp_add_dashboard_widget( 'mem-widget-overview', sprintf( esc_html__( '%s Overview', 'mobile-events-manager' ), mem_get_option( 'company_name', 'MEM' ) ), 'mem_widget_events_overview' );

} // mem_add_wp_dashboard_widgets
add_action( 'wp_dashboard_setup', 'mem_add_wp_dashboard_widgets' );
 ...
 * @since 1.3
 */
function mem_widget_events_overview() {
	...
	if ( mem_employee_can( 'manage_mem' ) ) {
		...
		foreach ( $enquiry_periods as $period => $date ) {
			$current_count = mem_count_events(
				array(
					'start-date' => $date,
			...
			foreach ( $current_count as $status => $count ) {
				...
				if ( in_array( $status, array( 'mem-approved', 'mem-contract', 'mem-completed', 'mem-cancelled' ) ) ) {
		...
		?>
		<div class="mem_stat_grid">
			<?php do_action( 'mem_before_events_overview' ); ?>
			<table>
				<thead>
					<tr>
						<th>&nbsp;</th>
				...
				<tbody>
					<tr>
						<th><?php /* translators: %s: Event Type */ printf( esc_html__( '%s Received', 'mobile-events-manager' ), esc_html( get_post_status_object( 'mem-enquiry' )->plural ) ); ?></th>
					...
					<tr>
						<th><?php /* translators: %s: Event Type */ printf( esc_html__( '%s Converted', 'mobile-events-manager' ), esc_html( get_post_status_object( 'mem-enquiry' )->plural ) ); ?></th>
					...
					<tr>
						<th><?php /* translators: %s: Event Type */ printf( esc_html__( '%s Completed', 'mobile-events-manager' ), esc_html( mem_get_label_plural() ) ); ?></th>
					...
					<tr>
						...
						<td><?php echo esc_attr( mem_currency_filter( mem_format_amount( $income_month ) ) ); ?></td>
						<td><?php echo esc_attr( mem_currency_filter( mem_format_amount( $income_year ) ) ); ?></td>
						<td><?php echo esc_attr( mem_currency_filter( mem_format_amount( $income_last ) ) ); ?></td>
					</tr>
					<tr>
						<th><?php esc_html_e( 'Outgoings', 'mobile-events-manager' ); ?></th>
						<td><?php echo esc_attr( mem_currency_filter( mem_format_amount( $expense_month ) ) ); ?></td>
						<td><?php echo esc_attr( mem_currency_filter( mem_format_amount( $expense_year ) ) ); ?></td>
						<td><?php echo esc_attr( mem_currency_filter( mem_format_amount( $expense_last ) ) ); ?></td>
					</tr>
					<tr>
						<th><?php esc_html_e( 'Earnings', 'mobile-events-manager' ); ?></th>
						<td><?php echo esc_attr( mem_currency_filter( mem_format_amount( $earnings_month ) ) ); ?></td>
						<td><?php echo esc_attr( mem_currency_filter( mem_format_amount( $earnings_year ) ) ); ?></td>
						<td><?php echo esc_attr( mem_currency_filter( mem_format_amount( $earnings_last ) ) ); ?></td>
					</tr>
				</tbody>
			</table>
			...
			<p>
				...
				printf( /* translators: %1: URL %2: Event type */
					...
					esc_url( admin_url( 'post-new.php?post_type=mem-event' ) ),
				...
				printf( /* translators: %1: URL %2: Event type */
					...
					esc_url( admin_url( 'edit.php?post_type=mem-event' ) ),
				...
				printf( /* translators: %s: URL */
					...
					esc_url( admin_url( 'edit.php?post_type=mem-transaction' ) )
				);
				?>
				&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;
				<?php
				printf( /* translators: %s: URL */
					...
					esc_url( admin_url( 'admin.php?page=mem-settings' ) )
				);
				?>
			</p>
			...
			<?php do_action( 'mem_after_events_overview' ); ?>
 ...
 */
function mem_dashboard_at_a_glance_widget( $items ) {
	$num_posts = mem_count_events();
	$count     = 0;
	...
	if ( $num_posts && $count > 0 ) {
		/* translators: %s: Company Name */
		$text = wp_kses_post( _n( '%s' . mem_get_label_singular(), '%s' . mem_get_label_plural(), $count, 'mobile-events-manager' ) );

		$text = sprintf( $text, number_format_i18n( $count ) );

		if ( mem_employee_can( 'read_events' ) ) {
			$text = sprintf( '<a class="event-count" href="edit.php?post_type=mem-event">%1$s</a>', $text );
		} else {
	...
	return $items;
} // mem_dashboard_at_a_glance_widget
add_filter( 'dashboard_glance_items', 'mem_dashboard_at_a_glance_widget', 1 );
