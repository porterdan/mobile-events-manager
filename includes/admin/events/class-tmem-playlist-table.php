...
/**
 * TMEM_Playlist_Table Class
 *
 * @since 1.3
 */
class TMEM_PlayList_Table extends WP_List_Table {

	public function __construct() {

		parent::__construct(
			array(
				'singular' => 'tmem_playlist_entry', // Singular label
				'plural'   => 'tmem_playlist_entries', // Plural label, also this well be one of the table css class
	...
	private function get_entries( $per_page = -1, $page_num = 1 ) {
		...
		if ( 'category' === $orderby ) {
			...
			$playlist = tmem_get_playlist_by_category( $tmem_event->ID, $args );
			...
			$entries = tmem_get_playlist_entries( $tmem_event->ID, $args );
		...
		return apply_filters( 'tmem_list_event_playlist', $result, $tmem_event->ID );
	...
	public function count_entries( $event_id ) {
		return tmem_count_playlist_entries( $event_id );
	} // count_entries
	...
	public function column_song( $item ) {
		...
		$url               = add_query_arg(
			array(
				'tmem-action' => 'delete_song',
				'id'          => absint( $item['ID'] ),
				'tmem_nonce'  => wp_create_nonce( 'remove_playlist_entry' ),
			)
		);
		$actions['delete'] = tmem_employee_can( 'manage_events' ) ? sprintf( wp_kses_post( '<a href="%s">Delete</a>', 'mobile-events-manager' ), $url ) : '';
	...
	public function column_cb( $item ) {
		return sprintf( '<input type="checkbox" name="tmem-playlist-bulk-delete[]" value="%s" />', $item['ID'] );
	} // column_song
	...
	public function get_views() {
		...
		$categories = tmem_get_playlist_categories( sanitize_text_field( wp_unslash( $_GET['event_id'] ) ) );
		...
		if ( $categories ) {
			...
			$views['all'] = sprintf(
				...
				$class
							...
							'<span class="count">' . tmem_count_playlist_entries( sanitize_text_field( wp_unslash( $_GET['event_id'] ) ) ) . '</span>';

			foreach ( $categories as $category ) {

				$count = tmem_count_playlist_entries( sanitize_text_field( wp_unslash( $_GET['event_id'] ), $category->name ) );
	...
	public function display_header() {
		?>
		<p><?php printf( wp_kses_post( '<strong>Date</strong>: %s', 'mobile-events-manager', tmem_get_event_long_date( sanitize_text_field( wp_unslash( $_GET['event_id'] ) ) ) ) ); ?>
		<br />
		<?php printf( wp_kses_post( '<strong>Status</strong>: %s', 'mobile-events-manager', tmem_get_event_status( sanitize_text_field( wp_unslash( $_GET['event_id'] ) ) ) ) ); ?>
		<br />
		<?php printf( wp_kses_post( '<strong>Type</strong>: %s', 'mobile-events-manager', tmem_get_event_type( sanitize_text_field( wp_unslash( $_GET['event_id'] ) ) ) ) ); ?>
		<br />
		<?php printf( wp_kses_post( '<strong>Primary Employee</strong>: %s', 'mobile-events-manager', tmem_get_employee_display_name( tmem_get_event_primary_employee_id( sanitize_text_field( wp_unslash( $_GET['event_id'] ) ) ) ) ) ); ?>
		<br />
		<?php printf( wp_kses_post( '<strong>Client</strong>: %s', 'mobile-events-manager', tmem_get_employee_display_name( tmem_get_event_client_id( sanitize_text_field( wp_unslash( $_GET['event_id'] ) ) ) ) ) ); ?>
		<br />
		<?php printf( wp_kses_post( '<strong>Total Songs</strong>: %s', 'mobile-events-manager' ), count( $this->items ) ); ?>
		<br />
		<?php printf( wp_kses_post( '<strong>Current Status</strong>: %s', 'mobile-events-manager', tmem_playlist_is_open( sanitize_text_field( wp_unslash( $_GET['event_id'] ) ) ) ? __( 'Open', 'mobile-events-manager' ) : __( 'Closed', 'mobile-events-manager' ) ) ); ?>
		</p>
		...
		if ( $this->count_entries( sanitize_text_field( wp_unslash( $_GET['event_id'] ) ) > 0 ) ) :
			...
			<form method="post" target="_blank">
				<?php tmem_admin_action_field( 'print_playlist' ); ?>
				...
				<?php wp_nonce_field( 'print_playlist_entry', 'tmem_nonce', true, true ); ?>
			...
			<form method="post">
				<?php tmem_admin_action_field( 'email_playlist' ); ?>
				<?php wp_nonce_field( 'email_playlist_entry', 'tmem_nonce', true, true ); ?>
	...
	public function entry_form() {
		...
		<form id="tmem-playlist-form" name="tmem-playlist-form" action="" method="post">
			<?php wp_nonce_field( 'add_playlist_entry', 'tmem_nonce', true, true ); ?>
			<?php tmem_admin_action_field( 'add_playlist_entry' ); ?>
			<input type="hidden" id="event_id" name="event_id" value="<?php sanitize_text_field( wp_unslash( $_GET['event_id'], 'mobile-events-manager' ) ); ?>" />
			<input type="hidden" id="added_by" name="added_by" value="<?php echo esc_attr( tmem_get_event_client_id( sanitize_text_field( wp_unslash( $_GET['event_id'] ) ) ) ); ?>" />
			<table id="tmem-playlist-form-table">
				<tr>
					<td>
					...
					<td class="tmem-playlist-artist-cell">
					...
					<td class="tmem-playlist-category-cell">
						<label for="category"><?php esc_html_e( 'Category', 'mobile-events-manager' ); ?></label><br />
						<?php $playlist_categories = tmem_get_playlist_categories(); ?>
						<?php $options = array(); ?>
				...
				<tr>
					...
					<td class="tmem-playlist-djnotes-cell" colspan="3">
