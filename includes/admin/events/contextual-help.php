 ...
 * @return void
 */
function tmem_events_contextual_help() {
	$screen = get_current_screen();

	if ( 'tmem-event' !== $screen->id ) {
		return;
	}

	$singular = esc_html( tmem_get_label_singular() );
	$plural   = esc_html( tmem_get_label_plural() );
	...
	$screen->add_help_tab(
		array(
			...
			'<p>' . sprintf(
				...
				$singular,
				strtolower( $singular ),
				'https://www.mobileeventsmanager.co.uk/docs/event-statuses/'
			) . '</p>' .
	...
	$screen->add_help_tab(
		array(
			'id'      => 'tmem-settings-client-details',
	...
	do_action( 'tmem_events_contextual_help', $screen );
}
add_action( 'load-post.php', 'tmem_events_contextual_help' );
add_action( 'load-post-new.php', 'tmem_events_contextual_help' );
