 ...
 */
function mem_display_event_playlist_page() {

	if ( ! mem_employee_can( 'read_events' ) && ! mem_employee_working_event( sanitize_text_field( wp_unslash( $_GET['event_id'] ) ) ) ) {
		wp_die(
	...
	if ( ! class_exists( 'MEM_PlayList_Table' ) ) {
		require_once MEM_PLUGIN_DIR . '/includes/admin/events/class-mem-playlist-table.php';
	}

	$playlist_obj = new MEM_PlayList_Table();

	?>
	<div class="wrap">
		<h1><?php printf( wp_kses_post( 'Playlist for %1$s %2$s', 'mobile-events-manager' ), esc_html( mem_get_label_singular() ), mem_get_event_contract_id( sanitize_text_field( wp_unslash( $_GET['event_id'] ) ) ) ); ?></h1>
	...
	<?php
} // mem_display_event_playlist_page
 ...
 */
function mem_format_playlist_content( $event_id, $orderby = 'category', $order = 'ASC', $hide_empty = true, $repeat_headers = 0 ) {
	global $current_user;
	...
	if ( 'category' === $orderby ) {
		...
		$playlist = mem_get_playlist_by_category( $event_id, array( 'hide_empty' => $hide_empty ) );
	...
	// Obtain results ordered by another field.
		...
		$entries = mem_get_playlist_entries( $event_id, $args );
	...
	if ( ! empty( $results ) ) {
		...
		$output .= '<p>' .
					 __( 'Client Name', 'mobile-events-manager' ) . ': ' . mem_get_client_display_name( $mem_event->client ) . '<br />' . "\n" .
					 __( 'Event Date', 'mobile-events-manager' ) . ': ' . mem_get_event_long_date( $mem_event->ID ) . '<br />' . "\n" .
					 __( 'Event Type', 'mobile-events-manager' ) . ': ' . mem_get_event_type( $mem_event->ID ) . '<br />' . "\n" .
