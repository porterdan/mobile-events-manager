 ...
 */
function mem_quote_post_columns( $columns ) {
	...
	if ( ! mem_employee_can( 'list_own_quotes' ) && isset( $columns['cb'] ) ) {
		unset( $columns['cb'] );
	}
	...
	return $columns;
} // mem_quote_post_columns
add_filter( 'manage_mem-quotes_posts_columns', 'mem_quote_post_columns' );
 ...
 */
function mem_quote_post_sortable_columns( $sortable_columns ) {
...
} // mem_quote_post_sortable_columns
add_filter( 'manage_edit-mem-quotes_sortable_columns', 'mem_quote_post_sortable_columns' );
 ...
 * @return void
 */
function mem_quote_post_order( $query ) {

	if ( ! is_admin() || 'mem-quotes' != $query->get( 'post_type' ) ) {
		return;
	}
	...
	switch ( $query->get( 'orderby' ) ) {
		...
		case 'quote_view_date':
			$query->set( 'meta_key', '_mem_quote_viewed_date' );
...
} // mem_quote_post_order
add_action( 'pre_get_posts', 'mem_quote_post_order' );
 ...
 * @return void
 */
function mem_limit_results_to_employee_quotes( $query ) {

	if ( ! is_admin() || 'mem-quotes' != $query->get( 'post_type' ) || mem_employee_can( 'list_all_quotes' ) ) {
		return;
	}

	global $user_ID;

	$events = mem_get_employee_events( $user_ID );

	foreach ( $events as $event ) {
		$quote = mem_get_event_quote_id( $event->ID );
...
} // mem_limit_results_to_employee_quotes
add_action( 'pre_get_posts', 'mem_limit_results_to_employee_quotes' );
 ...
 * @return
 */
function mem_quote_posts_custom_column( $column_name, $post_id ) {
	...
	switch ( $column_name ) {
		...
		case 'quote_event':
			if ( ! empty( $parent ) ) {
				...
				printf(
					...
					mem_get_event_contract_id( $parent ),
					mem_get_event_date( $parent )
				);
		...
		case 'quote_client':
			...
			echo '<a href="' . admin_url( 'admin.php?page=mem-clients&action=view_client&client_id=' . $post->post_author ) . '">' . get_the_author() . '</a>';
			break;

		// Cost
		case 'quote_value':
			echo mem_currency_filter( mem_get_event_price( $parent ) );
			break;

		// Date Viewed
		case 'quote_view_date':
			if ( 'mem-quote-viewed' == get_post_status( $post_id ) ) {

				echo gmdate( 'd M Y H:i:s', strtotime( get_post_meta( $post_id, '_mem_quote_viewed_date', true ) ) );
		...
		case 'quote_view_count':
			$count = get_post_meta( $post_id, '_mem_quote_viewed_count', true );
...
} // mem_quote_posts_custom_column
add_action( 'manage_mem-quotes_posts_custom_column', 'mem_quote_posts_custom_column', 10, 2 );
 ...
 */
function mem_quote_post_row_actions( $actions, $post ) {

	if ( 'mem-quotes' !== $post->post_type ) {
		return $actions;
	}
...
} // mem_quote_post_row_actions
add_filter( 'post_row_actions', 'mem_quote_post_row_actions', 10, 2 );
 ...
 */
function mem_quote_bulk_action_list( $actions ) {

	unset( $actions['edit'] );

	return $actions;

} // mem_quote_bulk_action_list
add_filter( 'bulk_actions-edit-mem-quotes', 'mem_quote_bulk_action_list' );
 ...
 */
function mem_quote_view_filters( $views ) {

	// We only run this filter if the user has restrictive caps and the post type is mem-event
	if ( ! is_post_type_archive( 'mem-quotes' ) || mem_employee_can( 'list_all_quotes' ) ) {
		return $views;
	}
	...
	// The All filter
	$views['all'] = preg_replace( '/\(.+\)/U', '(' . mem_count_employee_events() . ')', $views['all'] );

	$event_statuses = mem_all_event_status();

	foreach ( $event_statuses as $status => $label ) {
		$events = mem_get_employee_events( '', array( 'post_status' => $status ) );
...
add_filter( 'views_edit-mem-quotes', 'mem_quote_view_filters' );
 ...
 * @param
 */
function mem_quotes_remove_add_new() {

	if ( ! isset( $_GET['post_type'] ) || 'mem-quotes' !== $_GET['post_type'] ) {
		return;
	}
...
} // mem_quotes_remove_add_new
add_action( 'admin_head', 'mem_quotes_remove_add_new' );
 ...
 */
function mem_quote_post_messages( $messages ) {
	...
	return apply_filters( 'mem_quote_post_messages', $messages );

} // mem_quote_post_messages
add_filter( 'post_updated_messages', 'mem_quote_post_messages' );
