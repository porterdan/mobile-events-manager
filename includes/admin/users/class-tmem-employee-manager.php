...
if ( ! tmem_employee_can( 'manage_employees' ) ) {
	wp_die(
 ...
 */
class TMEM_Employee_Table extends WP_List_Table {
	public function __construct() {
		parent::__construct(
			array(
				'singular' => 'tmem_list_employee', // Singular label
				'plural'   => 'tmem_list_employees', // plural label, also this well be one of the table css class
	...
	public function extra_tablenav( $which ) {
		...
		<div class="alignleft actions">
			...
			<select name="new_role" id="new_role">
				...
				<?php
				add_filter( 'tmem_user_roles', array( TMEM()->roles, 'no_admin_role' ) );
				echo TMEM()->roles->roles_dropdown();
				remove_filter( 'tmem_user_roles', array( TMEM()->roles, 'no_admin_role' ) );
				?>
			</select>
	...
	public function prepare_items() {
		// The data is prepared in the TMEM_Employee_Manager class
		$employees = TMEM_Employee_Manager::$employees;
	...
	public function column_name( $item ) {
		...
		if ( tmem_is_admin( $item->ID ) ) {
			echo '<br />' . __( '<em>TMEM Admin</em>', 'mobile-events-manager' );
		}
	...
	public function column_role( $item ) {
		...
		if ( ! empty( $item->roles ) ) {
			...
			foreach ( $item->roles as $role ) {
				...
				if ( array_key_exists( $role, TMEM_Employee_Manager::$tmem_roles ) ) {
					$roles[ $role ] = TMEM_Employee_Manager::$tmem_roles[ $role ];
				}
			}
		}
	...
	public function column_events( $item ) {
		$next_event   = tmem_get_employees_next_event( $item->ID );
		$total_events = tmem_count_employee_events( $item->ID );

		printf(
			__( 'Next: %s', 'mobile-events-manager' ),
			! empty( $next_event ) ? '<a href="' . get_edit_post_link( $next_event->ID ) . '">' .
			tmem_format_short_date( get_post_meta( $next_event->ID, '_tmem_event_date', true ) ) . '</a>' :
		...
		printf(
			...
			! empty( $total_events ) ? '<a href="' . admin_url(
				'edit.php?s&post_type=tmem-event&post_status=all' .
				'&action=-1&tmem_filter_date=0&tmem_filter_type&tmem_filter_employee=' . $item->ID .
				'&tmem_filter_client=0&filter_action=Filter&paged=1&action2=-1'
			) . '">' . $total_events . '</a>' :
				'0'
		);
	} // column_events
	...
	public function get_views() {
		$views   = array();
		$current = TMEM_Employee_Manager::$display_role;
		...
		$views['all'] = '<a href="' . $all_url . '" ' . $class . '>' . __( 'All', 'mobile-events-manager' ) .
			' <span class="count">(' . TMEM_Employee_Manager::$total_employees . ')</span></a>';

		// Loop through all roles and generate the required views for each
		foreach ( TMEM_Employee_Manager::$tmem_roles as $role_id => $role ) {
			$count = count( tmem_get_employees( $role_id ) );
	...
	public function process_bulk_actions() {
		...
		if ( 'delete' === $this->current_action() && ! empty( $_POST['employees'] ) ) {
			...
			foreach ( sanitize_key( wp_unslash( $_POST['employees'] ) ) as $user_id ) {
				TMEM()->debug->log_it( 'Deleting employee with ID ' . $user_id, true );
				wp_delete_user( $user_id );
			}

			tmem_update_notice( 'updated', __( 'Employee(s) deleted.', 'mobile-events-manager' ), true );
		...
		foreach ( TMEM_Employee_Manager::$tmem_roles as $role_id => $role ) {
			...
			if ( 'add_role_' . $role_id === $this->current_action() && ! empty( $_POST['employees'] ) ) {
				...
				foreach ( sanitize_key( wp_unslash( $_POST['employees'] ) ) as $user_id ) {
					...
					TMEM()->debug->log_it( 'Adding additional role ' . $role . ' to user ' . $user_id, true );
				...
				tmem_update_notice( 'updated', __( $role . ' added to employee(s).', 'mobile-events-manager' ), true );
...
/**
 * Class Name: TMEM_Employee_Manager
 * User management interface for employees
 */
if ( ! class_exists( 'TMEM_Employee_Manager' ) ) :
	class TMEM_Employee_Manager {
		...
		public static $tmem_roles;
		private static $total_roles;
		private static $tmem_employee_table;
		/**
		 * Init
		 */
		public static function init() {
			...
			if ( isset( $_POST['change_role'], $_POST['new_role'], $_POST['employees'] ) ) {
				foreach ( sanitize_key( wp_unslash( $_POST['employees'] ) ) as $employee ) {
					tmem_set_employee_role( $employee, sanitize_key( wp_unslash( $_POST['new_role'] ) ) );
				}

				tmem_update_notice( 'updated', __( 'Employee roles updated.', 'mobile-events-manager' ), true );
			}
			...
			// Retrieve all TMEM roles
			self::$tmem_roles  = tmem_get_roles();
			self::$total_roles = count( self::$tmem_roles );
			...
			if ( 'permissions' === self::$active_tab ) {
				...
				self::$tmem_employee_table = new TMEM_Employee_Table();
				self::$tmem_employee_table->process_bulk_actions();
				// Retrieve employee list
				self::$employees       = empty( $_POST['s'] ) ? tmem_get_employees( self::$display_role, self::$orderby, self::$order ) : self::search();
				self::$total_employees = count( tmem_get_employees() );
				self::$tmem_employee_table->prepare_items();
		...
		public static function search() {
			...
			foreach ( self::$tmem_roles as $role => $label ) {
				$roles[] = $role;
			}
		...
		public static function page_header() {
			...
			<h2 class="nav-tab-wrapper">
				<a href="admin.php?page=tmem-employees&amp;tab=user_roles" class="nav-tab
					<?php echo 'user_roles' === self::$active_tab ? 'nav-tab-active' : ''; ?>">
					<?php esc_html_e( 'Employees & Roles', 'mobile-events-manager' ); ?>
				</a>

				<a href="admin.php?page=tmem-employees&amp;tab=permissions" class="nav-tab
		...
		public static function employee_page() {
			?>
			<form name="tmem_employee_list" id="tmem_employee_list" method="post">
			<?php
			wp_nonce_field( 'tmem_user_list_table' );
			self::$tmem_employee_table->views();
			?>
			<table style="width: 100%;">
			<tr valign="top">
			<td style="width: auto;"><?php self::$tmem_employee_table->display(); ?></td>
			...
			<?php
			// Display the roles
			echo TMEM()->roles->roles_dropdown( array( 'disable_default' => true ) );
			?>
			</select>
			<br />
			...
			<tr>
			<td>
			<input type="text" name="add_tmem_role" id="add_tmem_role" />&nbsp;<a id="new_tmem_role" class="button button-primary" href="#"><?php esc_html_e( 'Add Role', 'mobile-events-manager' ); ?></a><span id="pleasewait" style="display: none;" class="page-content"><?php esc_html_e( 'Please wait...', 'mobile-events-manager' ); ?></span>
			</td>
			</tr>
			</table>
			</form>
		...
		/**
		 * Display the form for adding a new employee to TMEM
		 */
		public static function add_employee_form() {
			// Ensure user has permssion to add an employee
			if ( ! tmem_employee_can( 'manage_staff' ) ) {
				return;
			}

			?>
			<h3><?php esc_html_e( 'Employee Quick Add', 'mobile-events-manager' ); ?></h3>
			<form name="tmem_employee_add" id="tmem_employee_add" method="post">
			<?php tmem_admin_action_field( 'add_employee' ); ?>
			<?php wp_nonce_field( 'add_employee', 'tmem_nonce', true, true ); ?>
			<table class="widefat">
			<tr>
			<td style="width: 250px;"><label class="tmem-label" for="first_name"><?php esc_html_e( 'First Name', 'mobile-events-manager' ); ?>:</label><br />
			<input type="text" name="first_name" id="first_name" required /></td>
			</tr>
			<tr>
			<td style="width: 250px;"><label class="tmem-label" for="last_name"><?php esc_html_e( 'Last Name', 'mobile-events-manager' ); ?>:</label><br />
			<input type="text" name="last_name" id="last_name" required /></td>
			</tr>
			<tr>
			<td style="width: 250px;"><label class="tmem-label" for="user_email"><?php esc_html_e( 'Email Address', 'mobile-events-manager' ); ?>:</label><br />
			<input type="text" name="user_email" id="user_email" required /></td>
			</tr>
			<tr>
			<td style="width: 250px;"><label class="tmem-label" for="employee_role"><?php esc_html_e( 'Role', 'mobile-events-manager' ); ?>:</label><br />
			...
			<tr>
			<td style="text-align: right;"><?php submit_button( __( 'Add Employee', 'mobile-events-manager' ), 'primary', 'tmem-add-employee', false ); ?></td>
			</tr>
			</table>
			</form>
			<?php
		...
		public static function permissions_manager() {
			?>
			<form name="tmem_permissions" id="tmem_permissions" method="post">
			<?php wp_nonce_field( 'tmem_permissions_table' ); ?>
			<input type="hidden" name="tmem_set_permissions" id="tmem_set_permissions" />
			<table class="widefat fixed striped" style="width: 100%;">
			<thead>
			<tr>
			<td id="tmem-emp-roles"><label class="screen-reader-text"><?php esc_html_e( 'Roles', 'mobile-events-manager' ); ?></label></td>
			<th scope="col" id="tmem-full-admin" style="font-size: small;"><strong><?php esc_html_e( 'TMEM Admin', 'mobile-events-manager' ); ?></strong></th>
			...
			<?php
			$i = 0;
			foreach ( self::$tmem_roles as $role_id => $role ) {
				// Don't show the admin role as this cannot be changed within TMEM
				if ( 'administrator' == $role_id ) {
					continue;
				}
				...
				echo '<td scope="row" style="font-size: small; vertical-align: middle;">';
					echo '<input type="checkbox" name="manage_tmem_' . $role_id . '" id="manage_tmem_' . $role_id . '" value="1" style="font-size: small;"';
				if ( ! empty( $caps->capabilities['manage_tmem'] ) ) {
				...
				echo '<td scope="row" style="font-size: small;">';
					...
					echo '<option value="tmem_client_none">' . __( 'None', 'mobile-events-manager' ) . '</option>' . "\r\n";

					echo '<option value="tmem_client_edit_own"';
				if ( ! empty( $caps->capabilities['tmem_client_edit_own'] ) ) {
					echo ' selected="selected"';
				}
					echo '>' . __( 'Edit Own', 'mobile-events-manager' ) . '</option>' . "\r\n";

					echo '<option value="tmem_client_edit"';
				if ( ! empty( $caps->capabilities['tmem_client_edit'] ) ) {
					echo ' selected="selected"';
				}
					...
					echo '<option value="tmem_employee_none">' . __( 'None', 'mobile-events-manager' ) . '</option>' . "\r\n";

					echo '<option value="tmem_employee_edit"';
				if ( ! empty( $caps->capabilities['tmem_employee_edit'] ) ) {
					echo ' selected="selected"';
				}
				...
				echo '<td scope="row" style="font-size: small;">';
					...
					echo '<option value="tmem_comms_none">' . __( 'None', 'mobile-events-manager' ) . '</option>' . "\r\n";

					echo '<option value="tmem_comms_send"';
				if ( ! empty( $caps->capabilities['tmem_comms_send'] ) ) {
					echo ' selected="selected"';
				}
					...
					echo '<option value="tmem_package_none">' . __( 'None', 'mobile-events-manager' ) . '</option>' . "\r\n";

					echo '<option value="tmem_package_edit_own"';
				if ( ! empty( $caps->capabilities['tmem_package_edit_own'] ) ) {
					echo ' selected="selected"';
				}
					echo '>' . __( 'Edit Own', 'mobile-events-manager' ) . '</option>' . "\r\n";

					echo '<option value="tmem_package_edit"';
				if ( ! empty( $caps->capabilities['tmem_package_edit'] ) ) {
					echo ' selected="selected"';
				}
					echo '>' . __( 'Edit All', 'mobile-events-manager' ) . '</option>' . "\r\n";
				...
				echo '<td scope="row" style="font-size: small;">';
					...
					echo '<option value="tmem_event_none">' . __( 'None', 'mobile-events-manager' ) . '</option>' . "\r\n";

					echo '<option value="tmem_event_read_own"';
				if ( ! empty( $caps->capabilities['tmem_event_read_own'] ) ) {
					echo ' selected="selected"';
				}
					echo '>' . __( 'Read Own', 'mobile-events-manager' ) . '</option>' . "\r\n";

					echo '<option value="tmem_event_read"';
				if ( ! empty( $caps->capabilities['tmem_event_read'] ) ) {
					echo ' selected="selected"';
				}
					echo '>' . __( 'Read All', 'mobile-events-manager' ) . '</option>' . "\r\n";

					echo '<option value="tmem_event_edit_own"';
				if ( ! empty( $caps->capabilities['tmem_event_edit_own'] ) ) {
					echo ' selected="selected"';
				}
					echo '>' . __( 'Edit Own', 'mobile-events-manager' ) . '</option>' . "\r\n";

					echo '<option value="tmem_event_edit"';
				if ( ! empty( $caps->capabilities['tmem_event_edit'] ) ) {
					echo ' selected="selected"';
				}
					echo '>' . __( 'Edit All', 'mobile-events-manager' ) . '</option>' . "\r\n";
					...
					echo '<option value="tmem_template_none">' . __( 'None', 'mobile-events-manager' ) . '</option>' . "\r\n";

					echo '<option value="tmem_template_edit"';
				if ( ! empty( $caps->capabilities['tmem_template_edit'] ) ) {
					echo ' selected="selected"';
				}
					echo '>' . __( 'Edit All', 'mobile-events-manager' ) . '</option>' . "\r\n";
				...
				echo '<td scope="row" style="font-size: small;">';
					...
					echo '<option value="tmem_txn_none">' . __( 'None', 'mobile-events-manager' ) . '</option>' . "\r\n";

					echo '<option value="tmem_txn_edit"';
				if ( ! empty( $caps->capabilities['tmem_txn_edit'] ) ) {
					echo ' selected="selected"';
				}
					echo '>' . __( 'Edit All', 'mobile-events-manager' ) . '</option>' . "\r\n";
					...
					echo '<option value="tmem_reports_none">' . __( 'None', 'mobile-events-manager' ) . '</option>' . "\r\n";

					echo '<option value="tmem_reports_run"';
				if ( ! empty( $caps->capabilities['tmem_reports_run'] ) ) {
					echo ' selected="selected"';
				}
				...
				echo '<td scope="row" style="font-size: small;">';
					...
					echo '<option value="tmem_quote_none">' . __( 'None', 'mobile-events-manager' ) . '</option>' . "\r\n";

					echo '<option value="tmem_quote_view_own"';
				if ( ! empty( $caps->capabilities['tmem_quote_view_own'] ) ) {
					echo ' selected="selected"';
				}
					echo '>' . __( 'View Own', 'mobile-events-manager' ) . '</option>' . "\r\n";

					echo '<option value="tmem_quote_view"';
				if ( ! empty( $caps->capabilities['tmem_quote_view'] ) ) {
					echo ' selected="selected"';
				}
				...
				echo '<td scope="row" style="font-size: small;">';
					...
					echo '<option value="tmem_venue_none">' . __( 'None', 'mobile-events-manager' ) . '</option>' . "\r\n";

					echo '<option value="tmem_venue_read"';
				if ( ! empty( $caps->capabilities['tmem_venue_read'] ) ) {
					echo ' selected="selected"';
				}
					echo '>' . __( 'View', 'mobile-events-manager' ) . '</option>' . "\r\n";

					echo '<option value="tmem_venue_edit"';
				if ( ! empty( $caps->capabilities['tmem_venue_edit'] ) ) {
					echo ' selected="selected"';
				}
