...
if ( ! tmem_employee_can( 'view_clients_list' ) ) {
	wp_die(
		...
		'<p>' . __( 'You do not have permission to manage clients.', 'mobile-events-manager' ) . '</p>',
		403
	);
}

/**
 * Class Name: TMEM_Client_Manager
 * User management interface for employees
 */
if ( ! class_exists( 'TMEM_Client_Manager' ) ) :
	class TMEM_Client_Manager extends WP_List_Table {
		...
		public function __construct() {
			parent::__construct(
				array(
					'singular' => 'tmem_list_client', // Singular label
					'plural'   => 'tmem_list_clients', // Plural label, also this will be one of the table css class
		...
		public function get_clients() {
			...
			self::$display_role = ! empty( $_GET['display_role'] ) ? sanitize_key( wp_unslash( $_GET['display_role'] ) ) : array( 'client', 'inactive_client' );
			self::$orderby      = ! empty( $_GET['orderby'] ) ? sanitize_key( wp_unslash( $_GET['orderby'] ) ) : 'display_name';
			self::$order        = ! empty( $_GET['order'] ) ? sanitize_key( wp_unslash( $_GET['order'] ) ) : 'ASC';
			...
			if ( ! empty( $_POST['s'] ) ) {
				...
				self::$clients = tmem_get_clients(
					self::$display_role,
					sanitize_key( wp_unslash( $_POST['filter_client'] ) ),
					self::$orderby,
					self::$order
				);
				...
				self::$clients = tmem_get_clients(
					self::$display_role,
					! tmem_employee_can( 'list_all_clients' ) ? get_current_user_id() : '',
					self::$orderby,
					self::$order
				);
		...
		public function client_page() {
			...
			<h1><?php printf( esc_html__( '%s Clients', 'mobile-events-manager' ), esc_attr( tmem_get_option( 'company_name' ) ) ); ?></h1>
			<form name="tmem-client-list" id="tmem-client-list" method="post">
				<?php $this->views(); ?>
				<?php $this->search_box( __( 'Search', 'mobile-events-manager' ), 'search_id' ); ?>
				<?php $this->display(); ?>
			</form>
			<?php
		} // page_header
		...
		public function extra_tablenav( $which ) {
			...
			<div class="alignleft actions">
				<label class="screen-reader-text" for="filter_client"><?php esc_html_e( 'Only show', 'mobile-events-manager' ); ?>&hellip;</label>
				<?php
				tmem_employee_dropdown(
					array(
						'name'            => 'filter_client',
						'first_entry'     => __( 'Show clients of', 'mobile-events-manager' ) . '...',
						'first_entry_val' => '',
						'structure'       => true,
						'echo'            => true,
					)
				...
				?>
				<input type="submit" name="show_only" id="show_only" class="button" value="<?php esc_html_e( 'Go!', 'mobile-events-manager' ); ?>" />
			</div>
				<?php
			}
		} // extra_tablenav
		...
		public function get_columns() {
			$columns = array(
				...
				'events' => __( 'Total Events', 'mobile-events-manager' ),
				'next'   => __( 'Next Event', 'mobile-events-manager' ),
		...
		public function no_items() {
			esc_html_e( "No Client's found.", 'mobile-events-manager' );
		} // no_items
		...
		public function column_events( $item ) {
			$total = tmem_get_client_events( $item->ID );

			echo ( ! empty( $total ) ?
				'<a href="' . admin_url(
					'edit.php?s&post_type=tmem-event?s&post_status=all' .
					'&post_type=tmem-event&action=-1&tmem_filter_date=0&tmem_filter_type&tmem_filter_employee=0' .
					'&tmem_filter_client=' . $item->ID . '&filter_action=Filter&paged=1&action2=-1'
				) . '">' . count( $total ) . '</a>' : '0'
			);
		} // column_events
		...
		public function column_next( $item ) {
			$next = tmem_get_clients_next_event( $item->ID );

			if ( ! empty( $next ) ) {
				echo '<a href="' . get_edit_post_link( $next[0]->ID ) . '">' . tmem_get_event_date( $next[0]->ID ) . '</a>';
			} else {
		...
		public function column_login( $item ) {
			if ( '' != get_user_meta( $item->ID, 'last_login', true ) ) {
				echo gmdate( 'H:i d M Y', strtotime( get_user_meta( $item->ID, 'last_login', true ) ) );
		...
		public function get_views() {
			...
			foreach ( $roles as $key => $role ) {
				$count = count( tmem_get_clients( $role ) );
			...
			if ( ! empty( $_POST['s'] ) ) {
				$class           = ( $current == $role ? ' class="current"' : '' );
				$views['search'] = '<a href="#" ' . $class . '>' . __( 'Search Results', 'mobile-events-manager' ) .
		...
		public function get_bulk_actions() {
			$actions = array(
				'delete' => __( 'Delete Client', 'mobile-events-manager' ),
			);

			if ( ! isset( $_GET['display_role'] ) || 'inactive_client' === $_GET['display_role'] ) {
				$actions['active'] = 'Mark Active';
			}

			if ( ! isset( $_GET['display_role'] ) || 'client' === $_GET['display_role'] ) {
		...
		public function process_bulk_actions() {
			...
			if ( 'delete' === $action && ! empty( $_POST['client'] ) ) {
				foreach ( sanitize_key( wp_unslash( $_POST['client'] ) ) as $user_id ) {
					if ( TMEM_DEBUG == true ) {
						TMEM()->debug->log_it( 'Deleting client with ID ' . $user_id, true );
					}
				...
				tmem_update_notice( 'updated', __( $i . ' Client(s) deleted.', 'mobile-events-manager' ), true );
			}
			if ( 'inactive' === $action || 'active' === $action ) {
				...
				foreach ( sanitize_key( wp_unslash( $_POST['client'] ) ) as $user_id ) {
					if ( TMEM_DEBUG == true ) {
						TMEM()->debug->log_it( 'Setting client with ID ' . $user_id . ' to ' . ucfirst( $this->current_action() ), true );
					}

					$user = new WP_User( $user_id );

					$user->set_role( 'inactive' === $action ? 'inactive_client' : 'client' );
					$i++;
				}

				tmem_update_notice( 'updated', __( $i . ' Client(s) updated.', 'mobile-events-manager' ), true );
			}
		} // process_bulk_actions
	} // class TMEM_Client_Manager
endif;
	new TMEM_Client_Manager();
