...
/**
 * Class Name: TMEM_Roles
 * Manage User roles within TMEM
 */
if ( ! class_exists( 'TMEM_Roles' ) ) :
	class TMEM_Roles {
		/**
		 * Class constructor
		 */
		public function __construct() {
			...
			add_action( 'update_option_tmem_settings', array( &$this, 'rename_dj_role' ), 10, 2 );

			// Ajax event for adding new roles
			add_action( 'wp_ajax_tmem_add_role', array( &$this, 'add_role' ) );
		...
		public function messages() {
			if ( ! isset( $_GET['page'] ) || 'tmem-employees' !== $_GET['page'] || empty( $_GET['role_action'] ) || empty( $_GET['message'] ) ) {
				return;
			}
			...
			tmem_update_notice( sanitize_key( wp_unslash( $messages[ $_GET['message'] ][0] ) ), sanitize_key( wp_unslash( $messages[ $_GET['message'] ][1] ) ), true );
		} // messages

		/**
		 * Add the default roles to TMEM
		 *
		 * @since 1.3
		 * @param
		 * @return void
		...
		/**
		 * Retrieve all TMEM user roles by filtering through the registered WP roles
		 *
		 * @params
		 *
		 * @return arr $tmem_roles An array of all TMEM roles
		 */
		public function get_roles() {
			...
			// Filter the roles
			$tmem_roles = apply_filters( 'tmem_user_roles', $tmem_roles );

			return $tmem_roles;
		} // get_roles

		/**
		 * Retrieve all TMEM user roles and display return them as <options>
		 *
		...
		public function roles_dropdown( $args = '' ) {
			...
			if ( empty( $tmem_roles ) ) {
				...
				foreach ( $tmem_roles as $role_id => $role ) {
		...
		public function add_role() {
			if ( empty( $_POST['role_name'] ) ) {
				...
				$result = add_role(
					'tmem-' . sanitize_title_with_dashes( strtolower( str_replace( '_', '-', sanitize_key( wp_unslash( $_POST['role_name'] ) ) ) ) ),
		...
		public function delete_roles( $roles = '' ) {
			...
			$employees = tmem_get_employees( $roles );
		 ...
		 *
		 * Called by: update_option_tmem_event_settings hook
		 *
		...
		public function rename_dj_role( $old_value, $new_value ) {
			...
			if ( empty( $user_roles ) ) {
				if ( TMEM_DEBUG == true ) {
					TMEM()->debug->log_it( 'ERROR: Could not retrieve user roles from DB', true );
				}
			...
			if ( update_option( $wpdb->prefix . 'user_roles', $user_roles ) ) {
				if ( TMEM_DEBUG == true ) {
					TMEM()->debug->log_it( 'Updated DJ role name to ' . $new_value['artist'], true );
				}

				return;
			} else {
				if ( TMEM_DEBUG == true ) {
					TMEM()->debug->log_it( 'ERROR: Could not update DJ role name to ' . $new_value['artist'], true );
				}
			}
		} // rename_dj_role

		/**
		 * Runs against the tmem_user_roles filter to modify the results removing the Administrator option.
		 *
		 * @params arr $tmem_roles The list of TMEM roles
		 *
		 * @return arr $tmem_roles Filtered list of TMEM roles
		 */
		public static function no_admin_role( $tmem_roles ) {

			if ( isset( $tmem_roles['administrator'] ) ) {
				unset( $tmem_roles['administrator'] );
			}
