...
/* -- Build the MEM_ClientFields class -- */
if ( ! class_exists( 'MEM_ClientFields' ) ) {
	class MEM_ClientFields {
		/*
		 * The Constructor
		 */
		public function __construct() {

			$this->fields = get_option( 'mem_client_fields' );
		...
		function delete_field() {
			...
			if ( update_option( 'mem_client_fields', $this->fields ) ) {
				mem_update_notice( 'updated', __( 'The field was deleted successfully.', 'mobile-events-manager' ) );
			} else {
				mem_update_notice( 'error', __( 'Field could not be deleted', 'mobile-events-manager' ) );
			}
		} // delete_field
		...
		function add_field() {
			...
			if ( update_option( 'mem_client_fields', $this->fields ) ) {
				mem_update_notice( 'updated', stripslashes( sanitize_text_field( wp_unslash( $_POST['field_label'] ) ) . __( ' created successfully.', 'mobile-events-manager' ) ) );
			} else {
				mem_update_notice( 'error', __( 'Field could not be created', 'mobile-events-manager' ) );
			}
		...
		function update_field() {
			...
			if ( update_option( 'mem_client_fields', $this->fields ) ) {
				mem_update_notice( 'updated', sanitize_text_field( wp_unslash( $_POST['field_label'] ) ) . ' updated successfully.' );
			} else {
				mem_update_notice( 'error', __( 'Field could not be updated', 'mobile-events-manager' ) );
			}
		...
		function display_fields() {
			...
			// Start the container
			echo '<div class="mem-client-field-container">' . "\r\n";

			// Left Column
			echo '<div class="mem-client-field-column-left">' . "\r\n";

			// Start the display table
			echo '<h3>Existing Client Fields</h3>' . "\r\n";
			echo '<table class="widefat mem-client-list-item" style="width:90%">' . "\r\n";
			echo '<thead>' . "\r\n";
			echo '<tr>' . "\r\n";
			...
			foreach ( $this->fields as $field ) {
				if ( 0 === $i && true === $field['display'] ) {
					$class = 'alternate mem_sortable_row';
				} elseif ( empty( $field['display'] ) ) {
					$class = 'form-invalid mem_sortable_row';
				} else {
					$class = 'mem_sortable_row';
				}

				echo '<tr id="fields=' . $field['id'] . '"' .
					' class="' . $class . '" data-key="' . esc_attr( $field['id'] ) . '">' . "\r\n";

				echo '<td><span class="mem_draghandle"></span></td>' . "\r\n";
				...
				echo '<td>';
				echo '<a href="' . mem_get_admin_page( 'client_fields' ) . '&action=edit_field&id=' . $field['id'] .
					'" class="button button-primary button-small">' . __( 'Edit', 'mobile-events-manager' ) . '</a>';
				if ( empty( $field['default'] ) ) {
					echo '&nbsp;&nbsp;&nbsp;<a href="' . mem_get_admin_page( 'client_fields' ) . '&action=delete_field&id=' . $field['id'] .
		...
		function manage_field_form() {
			...
			// Right Column
			echo '<div class="mem-client-field-column-right">' . "\r\n";
			echo '<form name="mem-client-fields" id="mem-client-fields" method="post" action="' . mem_get_admin_page( 'client_fields' ) . '">' . "\r\n";
			...
			echo '<h3>' . ( empty( $editing ) ? __( 'Add New Client Field', 'mobile-events-manager' ) :
				wp_kses_post( 'Edit the ', 'mobile-events-manager' . '<span class="mem-color">' . stripslashes( $this->fields[ sanitize_option( wp_unslash( $_GET['id'] ) ) ]['label'] ) . '</span>' . esc_html( __( ' Field', 'mobile-events-manager' ) ) . '</h3>' . "\r\n" ) );

			// Field Label
			echo '<p>';
			echo '<label class="mem-label" for="field_label">' . __( 'Field Label', 'mobile-events-manager' ) . ':</label><br />' . "\r\n";
			...
			echo '<p>' . "\r\n";
			echo '<label class="mem-label" for="field_type">' . __( 'Field Type', 'mobile-events-manager' ) . ':</label><br />' . "\r\n";
			...
			<?php
			echo '<p>' . "\r\n";
			echo '<label class="mem-label" for="field_options">' . __( 'Selectable Options', 'mobile-events-manager' ) . ':</label> <br />' . "\r\n";
			...
			<?php
			echo '<p>' . "\r\n";
			echo '<label class="mem-label" for="field_value">' . __( 'Checked Value', 'mobile-events-manager' ) . ':</label><br />' . "\r\n";
			...
			echo '<p>' . "\r\n";
			echo '<label class="mem-label" for="field_checked">' . __( 'Checked by Default', 'mobile-events-manager' ) . '?</label><br />' . "\r\n";
			...
			// Description
			echo '<p>' . "\r\n";
			echo '<label class="mem-label" for="field_desc">' . __( 'Description', 'mobile-events-manager' ) . ':</label><br />' . "\r\n";
			...
			echo '<input type="checkbox" name="field_enabled" id="field_enabled" value="1"' .
				...
				'<label class="mem-label" for="field_enabled">' . __( ' Field Enabled?', 'mobile-events-manager' ) . '</label>';
			...
			echo '<input type="checkbox" name="field_required" id="field_required" value="1"' .
				...
				( ! empty( $editing ) && in_array( $this->fields[ $_GET['id'] ]['id'], $must ) ? ' disabled="disabled"' : '' ) . ' />' .
				'<label class="mem-label" for="field_required">' . __( ' Required Field?', 'mobile-events-manager' ) . '</label>';
			if ( ! empty( $editing ) && in_array( $this->fields[ $_GET['id'] ]['id'], $must ) ) {
			...
			if ( ! empty( $editing ) ) {
				echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
				echo '<a href="' . mem_get_admin_page( 'client_fields' ) . '" class="button-secondary">' .
