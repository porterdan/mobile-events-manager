 ...
 */
function mem_get_tasks() {
	$tasks = get_option( 'mem_schedules' );
	if ( $tasks ) {
		ksort( $tasks );
	}

	return apply_filters( 'mem_tasks', $tasks );
} // mem_get_tasks
...
function mem_update_task( $data ) {
	...
	return update_option( 'mem_schedules', $tasks );
 ...
 * @return string
 */
function mem_get_task_name( $task ) {
	if ( ! is_array( $task ) ) {
		$task = mem_get_task( $task );
	}
 ...
 * @return string
 */
function mem_get_task_description( $task ) {
	if ( ! is_array( $task ) ) {
		$task = mem_get_task( $task );
	}

	return $task['desc'];
} // mem_get_task_description
 ...
 */
function mem_is_task_active( $task ) {
	if ( ! is_array( $task ) ) {
		$task = mem_get_task( $task );
	}
 ...
 * @return arr Array of tasks
 */
function mem_can_delete_task( $task ) {
	if ( ! is_array( $task ) ) {
		$task = mem_get_task( $task );
	}
 ...
 */
function mem_get_task_schedule_options() {
	$schedules = array(
	...
	$schedules = apply_filters( 'mem_task_schedule_options', $schedules );

	return $schedules;
} // mem_get_task_schedule_options
 ...
 * @return arr Array of options
 */
function mem_get_task_run_times( $id = false ) {
	...
	$run_times = apply_filters( 'mem_task_run_times', $run_times, $id );

	return $run_times;
} // mem_get_task_run_times
 ...
 */
function mem_filter_task_run_times( $run_times, $id ) {
	...
	return $run_times;
} // mem_filter_task_run_times
add_filter( 'mem_task_run_times', 'mem_filter_task_run_times', 10, 2 );
 ...
 */
function mem_task_set_active_status( $id, $activate = null ) {
	$tasks = mem_get_tasks();
...
function mem_task_run_now( $id ) {
	...
	require_once MEM_PLUGIN_DIR . '/includes/class-mem-task-runner.php';

	return new MEM_Task_Runner( $id );
} // mem_task_run_now
 ...
 */
function mem_get_tasks_for_event( $event_id ) {
	$event           = new MEM_Event( $event_id );
	$tasks           = array();
	$completed_tasks = $event->get_tasks();
	$playlist        = mem_get_playlist_entries( $event_id, array( 'posts_per_page' => 1 ) );
	$guest_args      = array(
		'posts_per_page' => 1,
		'tax_query'      => array(
			array(
	...
	if ( in_array( $event->post_status, array( 'mem-awaitingdeposit', 'mem-approved', 'mem-contract' ) ) ) {
		...
		if ( 'Due' === $event->get_balance_status() ) {
			$tasks['balance-reminder'] = mem_get_task_name( 'balance-reminder' );
		}
	}

	/*
	If ( $playlist ) {
		$tasks['playlist-employee-notify'] = mem_get_task_name( 'playlist-employee-notify' );
	}
	*/

	/*
	If ( $guest_playlist ) {
		$tasks['playlist-notification'] = mem_get_task_name( 'playlist-notification' );
	}
	*/

	if ( 'mem-unattended' === $event->post_status ) {
		$tasks['reject-enquiry'] = __( 'Reject Enquiry', 'mobile-events-manager' );
	}

	if ( 'mem-enquiry' === $event->post_status ) {
		$tasks['fail-enquiry'] = mem_get_task_name( 'fail-enquiry' );
	}

	$tasks = apply_filters( 'mem_tasks_for_event', $tasks, $event_id );
 ...
 */
function mem_run_single_event_task( $event_id, $task_id ) {
	$task = mem_get_task( $task_id );

	switch ( $task_id ) {
		case 'fail-enquiry':
			return mem_fail_enquiry_single_task( $event_id );
			break;

		case 'request-deposit':
			return mem_request_deposit_single_task( $event_id );
			break;

		case 'balance-reminder':
			return mem_balance_reminder_single_task( $event_id );
			break;

		case 'playlist-employee-notify':
			return mem_employee_playlist_notify_single_task( $event_id );
			break;

		default:
			break;
	}
} // mem_run_single_event_task
 ...
 */
function mem_fail_enquiry_single_task( $event_id ) {
	$event = new MEM_Event( $event_id );

	if ( mem_update_event_status( $event->ID, 'mem-failed', $event->post_status ) ) {
		mem_add_journal(
			array(
				'user_id'         => 1,
				'event_id'        => $event->ID,
 ...
 * @return bool True if task ran successfully
 */
function mem_request_deposit_single_task( $event_id ) {
	$event = new MEM_Event( $event_id );
	$task  = mem_get_task( 'request-deposit' );
	...
	if ( ! empty( $task['options']['email_template'] ) && ! empty( $event->client ) ) {
		...
		$email_args = array(
			...
			'message'   => mem_get_email_template_content( $task['options']['email_template'] ),
			'track'     => true,
	...
	return false;
} // mem_request_deposit_single_task
 ...
 * @return bool True if task ran successfully
 */
function mem_balance_reminder_single_task( $event_id ) {
	$event = new MEM_Event( $event_id );
	$task  = mem_get_task( 'balance-reminder' );
	...
	return false;
} // mem_balance_reminder_single_task
 ...
 * @return bool True if task ran successfully
 */
function mem_employee_playlist_notify_single_task( $event_id ) {
	$event = new MEM_Event( $event_id );

	$content = mem_format_playlist_content( $event_id, '', 'ASC', '', true );
	$content = apply_filters( 'mem_print_playlist', $content, $event );
	...
	$args = array(
		'to_email'   => mem_get_employee_email( $event->employee_id ),
		'from_name'  => mem_get_option( 'company_name' ),
		'from_email' => mem_get_option( 'system_email' ),
		'event_id'   => $event_id,
		'client_id'  => $event->client,
		'subject'    => sprintf( esc_html__( 'Playlist for %1$s ID %2$s', 'mobile-events-manager' ), esc_html( mem_get_label_singular() ), '{contract_id}' ),
		'message'    => $html_content_start . $content . $html_content_end,
		'copy_to'    => 'disable',
	);

	$event->complete_task( 'employee-playlist-notify' );
	return mem_send_email_content( $args );
} // mem_employee_playlist_notify_single_task
