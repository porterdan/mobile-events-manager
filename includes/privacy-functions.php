 ...
 */
function tmem_get_privacy_page() {
	$privacy_page = get_option( 'wp_page_for_privacy_policy' );
	$privacy_page = apply_filters( 'tmem_privacy_page', $privacy_page );

	return $privacy_page;
} // tmem_get_privacy_page

/**
 * Register the TMEM template for a privacy policy.
 *
 * Note, this is just a suggestion and should be customized to meet your businesses needs.
 *
 * @since 1.5.3
 * @return string The TMEM suggested privacy policy
 */
function tmem_register_privacy_policy_template() {
	...
	$content .= '<ul>';
	$content .= '<li>' . sprintf( esc_html__( 'Plan, manage and deliver your %s efficiently', 'mobile-events-manager' ), esc_html( tmem_get_label_singular( true ) ) ) . '</li>';
	$content .= '<li>' . sprintf( esc_html__( 'Send you important information regarding your %s', 'mobile-events-manager' ), esc_html( tmem_get_label_singular( true ) ) ) . '</li>';
	...
	$additional_collection = apply_filters( 'tmem_privacy_policy_additional_collection', $additional_collection );
	...
	$content = apply_filters( 'tmem_privacy_policy_content', $content );
	$content = wp_kses_post( $content );

	wp_add_privacy_policy_content( 'TMEM Event Management', wpautop( $content ) );
} // tmem_register_privacy_policy_template
add_action( 'admin_init', 'tmem_register_privacy_policy_template' );
 ...
 */
function tmem_mask_string( $string = '' ) {
 ...
 */
function tmem_mask_domain( $domain = '' ) {
 ...
 */
function tmem_pseudo_mask_email( $email_address ) {
	if ( ! is_email( $email_address ) ) {
		return $email_address;
	}

	$email_parts = explode( '@', $email_address );
	$name        = tmem_mask_string( $email_parts[0] );
	$domain      = tmem_mask_domain( $email_parts[1] );

	$email_address = $name . '@' . $domain;

	return $email_address;
} // tmem_pseudo_mask_email
 ...
 * @return void
 */
function tmem_log_terms_and_privacy_times( $event_id, $form_data ) {
...
} // tmem_log_terms_and_privacy_times
add_action( 'tmem_dcf_after_create_event', 'tmem_log_terms_and_privacy_times', 10, 2 );
 ...
 * @return void
 */
function tmem_payment_terms_privacy_log_action( $data, $payment_data ) {
	$form_data = array();
	$event_id  = absint( $payment_data['event_id'] );
	$timestamp = current_time( 'timestamp' );

	if ( isset( $data['tmem_agree_privacy_policy'] ) ) {
		$form_data['form_data']['data']['privacy_accepted'] = $timestamp;
	}

	if ( isset( $data['tmem_agree_terms'] ) ) {
		$form_data['form_data']['data']['terms_agreed'] = $timestamp;
	}

	tmem_log_terms_and_privacy_times( $event_id, $form_data );
} // tmem_payment_terms_privacy_log_action
add_action( 'tmem_payment_before_gateway', 'tmem_payment_terms_privacy_log_action', 10, 2 );
 ...
 */
function tmem_anonymize_email( $email_address ) {
 ...
 * @return array
 */
function _tmem_anonymize_client( $client_id = 0 ) {
	...
	$anonymized_email = tmem_anonymize_email( $client->user_email );
	 ...
	 */
	do_action( 'tmem_anonymize_client', $client, $anonymized_email );
 ...
 * @return KBS_Ticket
 */
function _tmem_privacy_get_client_id_for_email( $email_address ) {
	$client_id = get_option( 'tmem_priv_' . md5( $email_address ), true );
	$client    = get_userdata( $client_id );

	return $client;
} // _tmem_privacy_get_client_id_for_email
 ...
 * @return array
 */
function tmem_register_privacy_exporters( $exporters ) {

	$exporters[] = array(
		'exporter_friendly_name' => __( 'TMEM Client Record', 'mobile-events-manager' ),
		'callback'               => 'tmem_privacy_client_record_exporter',
	);

	return $exporters;

} // tmem_register_privacy_exporters
add_filter( 'wp_privacy_personal_data_exporters', 'tmem_register_privacy_exporters' );
 ...
 * @return array
 */
function tmem_privacy_client_record_exporter( $email_address = '', $page = 1 ) {
	...
	$address       = tmem_get_client_address( $client->ID );
	...
	$export_data = array(
		'group_id'    => 'tmem-client-record',
		'group_label' => __( 'TMEM Client Record', 'mobile-events-manager' ),
		'item_id'     => "tmem-client-record-{$client->ID}",
		'data'        => array(),
	);
	...
	$export_data = apply_filters( 'tmem_privacy_client_record', $export_data, $client );

	return array(
		'data' => array( $export_data ),
		'done' => true,
	);
} // tmem_privacy_client_record_exporter
 ...
 * @return string
 */
function tmem_render_agree_to_privacy_policy_field() {
	$agree_to_policy = tmem_get_option( 'show_agree_to_privacy_policy', false );
	$privacy_page    = tmem_get_privacy_page();
	$label           = tmem_get_option( 'agree_privacy_label', false );
	$description     = tmem_get_option( 'agree_privacy_descripton', false );
	...
	$args = apply_filters(
		'tmem_agree_to_privacy_policy_args',
		array(
			'label_class' => '',
			'input_class' => '',
		)
	...
	if ( 'thickbox' === tmem_get_option( 'show_agree_policy_type' ) ) {
		$privacy_url = sprintf(
			'<a href="#TB_inline?width=600&height=550&inlineId=tmem-privacy-policy" class="thickbox"%s title="%s">%s</a>',
			$label_class,
	...
	<p><input type="checkbox" name="tmem_agree_privacy_policy" id="tmem-agree-privacy-policy"<?php echo $input_class; ?> value="1" /> <?php echo $privacy_url; ?></p>

	<?php if ( ! empty( $description ) ) : ?>
		<span class="tmem-description"><?php echo esc_html( $description ); ?></span>
	<?php endif; ?>

	<?php if ( 'thickbox' === tmem_get_option( 'show_agree_policy_type' ) ) : ?>
		<div id="tmem-privacy-policy" class="tmem-hidden">
			<?php do_action( 'tmem_before_privacy_policy' ); ?>
			<?php echo wpautop( do_shortcode( stripslashes( $privacy_text ) ) ); ?>
			<?php do_action( 'tmem_after_privacy_policy' ); ?>
		</div>
	<?php endif; ?>
...
} // tmem_render_agree_to_privacy_policy_field
add_action( 'tmem_payment_form_after_cc_form', 'tmem_render_agree_to_privacy_policy_field', 950 );
 ...
 * @return string
 */
function tmem_render_agree_to_terms_field() {
	$agree_to_terms = tmem_get_option( 'show_agree_to_terms', false );
	$agree_text     = tmem_get_option( 'agree_terms_text', false );
	$label          = tmem_get_option( 'agree_terms_label', false );
	$terms_heading  = tmem_get_option(
		'agree_terms_heading',
	...
	<p><input type="checkbox" name="tmem_agree_terms" id="tmem-agree-terms"<?php echo $input_class; ?> value="1" /> <a href="#TB_inline?width=600&height=550&inlineId=tmem-terms-conditions" title="<?php esc_html_e( $terms_heading, 'mobile-events-manager' ); ?>" class="thickbox"<?php echo $label_class; ?>><?php esc_html_e( $label, 'mobile-events-manager' ); ?></a></p>

	<div id="tmem-terms-conditions" class="tmem-hidden">
		<?php do_action( 'tmem_before_terms' ); ?>
		<?php echo wpautop( stripslashes( $agree_text ) ); ?>
		<?php do_action( 'tmem_after_terms' ); ?>
	</div>

	<?php
	echo ob_get_clean();

} // tmem_render_agree_to_terms_field
add_action( 'tmem_payment_form_after_cc_form', 'tmem_render_agree_to_terms_field', 999 );
