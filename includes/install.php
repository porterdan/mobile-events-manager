 ...
 * pages. After successful install, the user is redirected to the MEM Welcome
 * screen.
 *
 * @since 1.3
 * @global $wpdb
 ...
 * @return void
 */
function mem_install( $network_wide = false ) {
...
register_activation_hook( MEM_PLUGIN_FILE, 'mem_install' );
 ...
 * @return void
 */
function mem_run_install() {

	global $mem_options, $wpdb;

	$current_version = get_option( 'mem_version' );
	if ( $current_version ) {
		return;
	}
	...
	// Pull options from WP, not MEM's global.
	$current_options = get_option( 'mem_settings', array() );
	...
	if ( ! array_key_exists( 'app_home_page', $current_options ) ) {
		...
		$contract = wp_insert_post(
			array(
				'post_title'     => sprintf( esc_html__( '%s Contract', 'mobile-events-manager' ), esc_html( mem_get_label_singular() ) ),
				'post_content'   => '[mem-contract]',
				'post_status'    => 'publish',
				'post_author'    => 1,
				'post_type'      => 'page',
				'post_parent'    => $client_zone,
		...
		$quotes = wp_insert_post(
			array(
				'post_title'     => sprintf( esc_html__( '%s Quotes', 'mobile-events-manager' ), esc_html( mem_get_label_singular() ) ),
				'post_content'   => '[mem-quote]',
				'post_status'    => 'publish',
				'post_author'    => 1,
				'post_type'      => 'page',
				'post_parent'    => $client_zone,
		...
		// Store the page IDs in MEM options.
	...
	if ( ! array_key_exists( 'enquiry', $current_options ) ) {
		...
		$enquiry = wp_insert_post(
			array(
				...
				'post_content'   => '<h1>' . __( 'Your DJ Enquiry from {company_name}', 'mobile-events-manager' ) . '</h1>' .
									...
									'<br /><br />' .
									sprintf( esc_html__( 'Thank you for contacting {company_name} regarding your up and coming %s on {event_date}.', 'mobile-events-manager' ), esc_html( mem_get_label_singular( true ) ) ) .
									'<br /><br />' .
		...
		$online_enquiry = wp_insert_post(
			array(
				...
				'post_content'   => sprintf( '[caption id="" align="alignleft" width="128"]%2$s[/caption]', '{website_url}', '{company_name}' ) .
									'<h3>' . sprintf( esc_html__( '%1$s Quotation for %2$s', 'mobile-events-manager' ), esc_html( mem_get_label_singular() ), '{client_fullname}' ) . '</h3>' .
									...
									'<br />' .
									sprintf( esc_html__( 'It is with pleasure that I am providing you with the following costs for your %1$s on %2$s.', 'mobile-events-manager' ), esc_html( mem_get_label_singular( true ) ), '{event_date}' ) .
									'<br /><br />' .
									...
									'<tbody>' .
									'<tr>' .
									'<td>' . sprintf( esc_html__( '%s Date:', 'mobile-events-manager' ), esc_html( mem_get_label_singular() ) ) . '</td>' .
									'<td>{event_date}</td>' .
									'<td>' . sprintf( esc_html__( '%s Type:', 'mobile-events-manager' ), esc_html( mem_get_label_singular() ) ) . '</td>' .
									'<td>{event_type}</td>' .
									'</tr>' .
									'<tr>' .
									...
									'<td colspan="2">' . sprintf( esc_html__( '%s Cost:', 'mobile-events-manager' ), esc_html( mem_get_label_singular() ) ) . '</td>' .
		...
		$contract = wp_insert_post(
			array(
				...
				'post_content'   => '<h2>' . sprintf( esc_html__( 'Your Booking with %s', 'mobile-events-manager' ), '{company_name}' ) . '</h2>' .
									sprintf( esc_html__( 'Dear %s,', 'mobile-events-manager' ), '{client_firstname}' ) .
									'<br /><br />' .
									sprintf( esc_html__( 'Thank you for indicating that you wish to proceed with booking %1$s for your up and coming %2$s on %3$s', 'mobile-events-manager' ), '{company_name}', esc_html( mem_get_label_singular( true ) ), '{event_date}' ) .
									'<br /><br />' .
		...
		$booking_conf_client = wp_insert_post(
			array(
				...
				'post_content'   => '<h1>' . __( 'Booking Confirmation', 'mobile-events-manager' ) . '</h1>' .
									sprintf( esc_html__( 'Dear %s,', 'mobile-events-manager' ), '{client_firstname}' ) .
									'<br /><br />' .
									sprintf( esc_html__( 'Thank you for booking your up and coming %1$s with %2$s. Your booking is now confirmed.', 'mobile-events-manager' ), esc_html( mem_get_label_singular() ), '{company_name}' ) .
									'<br /><br />' .
									sprintf( esc_html__( 'My name is %1$s and I will be your DJ on %2$s. Should you wish to contact me at any stage to discuss your %3$s, my details are at the end of this email.', 'mobile-events-manager' ), '{employee_fullname}', '{event_date}', esc_html( mem_get_label_singular( true ) ) ) .
									'<br />' .
									'<h2>' . __( 'What Now?', 'mobile-events-manager' ) . '</h2>' .
									'<br />' .
									'<strong>' . __( 'Music Selection & Playlists', 'mobile-events-manager' ) . '</strong>' .
									'<br /><br />' .
									sprintf( esc_html__( 'We have an online portal where you can add songs that you would like to ensure we play during your disco. To access this feature, head over to the %1$s <a href="%2$s">%2$s</a>. The playlist feature will close %3$s days before your %4$s.', 'mobile-events-manager' ), '{company_name}', '{application_home}', '{playlist_close}', esc_html( mem_get_label_singular( true ) ) ) .
									'<br /><br />' .
									...
									'<br /><br />' .
									sprintf( esc_html__( 'I generally contact you again approximately 2 weeks before your %s to finalise details with you. However, if you have any questions, concerns, or just want a general chat about your disco, feel free to contact me at any time.', 'mobile-events-manager' ), esc_html( mem_get_label_singular( true ) ) ) .
									'<br /><br />' .
									sprintf( esc_html__( 'Thanks again for choosing %1$s to provide the DJ & Disco for your %2$s. I look forward to partying with you on %3$s.', 'mobile-events-manager' ), '{company_name}', esc_html( mem_get_label_singular( true ) ), '{event_date}' ) .
									'<br /><br />' .
									__( 'Best Regards', 'mobile-events-manager' ) .
									'<br /><br />' .
									'{employee_fullname}' .
		...
		$email_dj_confirm = wp_insert_post(
			array(
				...
				'post_content'   => '<h1>' . __( 'Booking Confirmation', 'mobile-events-manager' ) . '</h1>' .
									...
									'<br /><br />' .
									sprintf( esc_html__( 'Your client %1$s has just confirmed their booking for you to DJ at their %2$s on %3$s.', 'mobile-events-manager' ), '{client_fullname}', esc_html( mem_get_label_singular( true ) ), '{event_date}' ) .
									'<br /><br />' .
									...
									'<br /><br />' .
									sprintf( esc_html__( 'Remember it is your responsibility to remain in regular contact with your client regarding their %1$s as well as answer any queries or concerns they may have. Customer service is one of our key selling points and after the event, your client will be invited to provide feedback regarding the booking process, communication in the lead up to the %1$s, as well as the %1$s itself.', 'mobile-events-manager' ), esc_html( mem_get_label_singular( true ) ) ) .
									'<br />' .
									'<h2>' . sprintf( esc_html__( '%s Details', 'mobile-events-manager' ), esc_html( mem_get_label_singular() ) ) . '</h2>' .
									'<br />' .
		...
		$unavailable = wp_insert_post(
			array(
				...
				'post_content'   => '<h1>' . sprintf( esc_html__( 'Your DJ Enquiry with %s', 'mobile-events-manager' ), '{company_name}' ) . '</h1>' .
									sprintf( esc_html__( 'Dear %s', 'mobile-events-manager' ), '{client_firstname}' ) .
									'<br /><br />' .
									sprintf( esc_html__( 'Thank you for contacting %1$s regarding your up and coming %2$s on %3$s.', 'mobile-events-manager' ), '{company_name}', esc_html( mem_get_label_singular( true ) ), '{event_date}' ) .
									'<br /><br />' .
									sprintf( esc_html__( 'Unfortunately however, we are not available on the date you have selected for your %s.', 'mobile-events-manager' ), esc_html( mem_get_label_singular( true ) ) ) .
									__( "If you have alternative dates you are looking at, we'd love to hear from you again.", 'mobile-events-manager' ) .
									'<br /><br />' .
									sprintf( esc_html__( 'Otherwise, we hope you have a great %s and hope to hear from you again next time.', 'mobile-events-manager' ), esc_html( mem_get_label_singular( true ) ) ) .
									'<br /><br />' .
									__( 'Best Regards', 'mobile-events-manager' ) .
									'<br /><br />' .
									'{employee_fullname}' .
		...
		$payment_cfm = wp_insert_post(
			array(
				'post_title'     => sprintf( esc_html__( '%1$s %2$s Payment Confirmation', 'mobile-events-manager' ), esc_html( mem_get_label_singular() ), '{payment_for}' ),
				'post_status'    => 'publish',
				'post_type'      => 'email_template',
				'post_author'    => 1,
				'ping_status'    => 'closed',
				...
				'post_content'   => '<h4><span style="color: #ff9900;">' . sprintf( esc_html__( 'Thank you for your %s payment', 'mobile-events-manager' ), '{payment_for}' ) . '</span></h4>' .
									sprintf( esc_html__( 'Dear %s,', 'mobile-events-manager' ), '{client_firstname}' ) .
									'<br /><br />' .
									sprintf( esc_html__( 'Thank you for your recent payment of <strong>%1$s</strong> towards the <strong>%2$s</strong> for you event on <strong>%3$s</strong>. Your payment has been received and your %4$s details have been updated.', 'mobile-events-manager' ), '{payment_amount}', '{payment_for}', '{event_date}', esc_html( mem_get_label_singular( true ) ) ) .
									'<br /><br />' .
	...
	foreach ( mem_get_registered_settings() as $tab => $sections ) {
		foreach ( $sections as $section => $settings ) {
			...
			$tab_sections = mem_get_settings_tab_sections( $tab );
	...
	$options['employee_pay_status'] = array( 'mem-completed' );

	$merged_options = array_merge( $mem_options, $options );
	$mem_options   = $merged_options;

	update_option( 'mem_settings', $merged_options );
	update_option( 'mem_version', MEM_VERSION_NUM );
	update_option( 'mem_client_fields', $client_fields );

	// Setup scheduled tasks.
	MEM()->cron->create_tasks();
	...
	// Create the custom MEM User Roles.
	$roles = new MEM_Roles();
	$roles->add_roles();

	// Make all admins MEM employees and admins by default by assigning caps to the user directly.
	$administrators = get_users( array( 'role' => 'administrator' ) );

	$permissions = new MEM_Permissions();

	foreach ( $administrators as $user ) {
		update_user_meta( $user->ID, '_mem_event_staff', true );
		update_user_meta( $user->ID, '_mem_event_admin', true );
		$user->add_role( 'dj' );
		$user->add_cap( 'mem_employee' );
		$permissions->make_admin( $user->ID );
	}

	// Assign the MEM employee cap to the DJ role.
	$role = get_role( 'dj' );
	...
	if ( ! $current_version ) {
		...
		foreach ( $upgrade_routines as $upgrade ) {
			mem_set_upgrade_complete( $upgrade );
		}
	}
 ...
 * @return void
 */
function mem_deactivate() {
	MEM()->cron->unschedule_events();
} // mem_deactivate.
register_deactivation_hook( MEM_PLUGIN_FILE, 'mem_deactivate' );
