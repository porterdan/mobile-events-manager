 ...
 */
function tmem_packages_enabled() {
	return tmem_get_option( 'enable_packages', false );
} // tmem_packages_enabled
 ...
 */
function tmem_get_packages( $args = array() ) {
	...
	return apply_filters( 'tmem_get_packages', get_posts( $package_args ) );
 ...
 */
function tmem_get_package( $package_id ) {
	return tmem_get_package_by( 'id', $package_id );
} // tmem_get_package
 ...
 */
function tmem_get_package_by( $field, $value ) {
	...
	switch ( strtolower( $field ) ) {
		...
		case 'id':
			...
			if ( get_post_type( $package ) !== 'tmem-package' ) {
				return false;
			}
		...
		case 'event':
			$event_package = get_post_meta( $value, '_tmem_event_package', true );
 ...
 * @return arr
 */
function tmem_get_package_data( $package ) {

	$package_id = is_object( $package ) ? $package->ID : $package;
	$events     = tmem_get_package_event_types( $package_id );
	$users      = tmem_get_employees_with_package( $package_id );
	$items      = tmem_get_package_addons( $package_id );
	...
	if ( ! tmem_package_is_restricted_by_date( $package_id ) ) {
		$months[] = __( 'Always', 'mobile-events-manager' );
	} else {
		$availability = tmem_get_package_months_available( $package_id );
		...
		if ( ! $availability ) {
			...
			foreach ( $availability as $month ) {
				...
				$months[] = tmem_month_num_to_name( $availability[ $i ] );
				$i++;
			}
		}
	}
	...
	if ( in_array( 'all', $users ) ) {
		...
		foreach ( $users as $employee_id ) {
			...
			$employees[] = array( $employee_id => tmem_get_employee_display_name( $employee_id ) );
		}
	}

	if ( in_array( 'all', $events ) ) {
		$event_types = sprintf( esc_html__( 'All %s Types', 'mobile-events-manager' ), esc_html( tmem_get_label_singular() ) );
	} else {
	...
	if ( tmem_package_has_variable_prices( $package_id ) ) {
		$range = tmem_get_package_price_range( $package_id );

		$price = tmem_get_currency() . ' ' . tmem_format_amount( $range['low'] ) . ' &mdash; ' . tmem_format_amount( $range['high'] );

	} else {
		$price = tmem_get_currency() . ' ' . tmem_format_amount( tmem_get_package_price( $package_id ) );
	}

	if ( $items ) {
		foreach ( $items as $addon_id ) {
			$addons[] = array( $addon_id => tmem_get_addon_name( $addon_id ) );
		}
	}
	...
	$package_data = array(
		'name'         => tmem_get_package_name( $package_id ),
		...
		'items'        => $addons,
		'usage'        => array(
			'events' => tmem_count_events_with_package( $package_id ),
		),
	);

	return apply_filters( 'tmem_get_package_data', $package_data );

} // tmem_get_package_data
 ...
 */
function tmem_get_packages_in_category( $term_id ) {
	...
	return tmem_get_packages( $args );

} // tmem_get_packages_in_category
 ...
 */
function tmem_get_package_name( $package_id ) {
	$title = get_the_title( $package_id );

	return apply_filters( 'tmem_package_name', $title, $package_id );
} // tmem_get_package_name
 ...
 */
function tmem_get_employees_with_package( $package_id ) {
	$employees = get_post_meta( $package_id, '_package_employees', true );

	return apply_filters( 'tmem_employees_with_addon', $employees, $package_id );
} // tmem_get_employees_with_package
 ...
 */
function tmem_get_package_event_types( $package_id ) {
	$event_types = get_post_meta( $package_id, '_package_event_types', true );

	return apply_filters( 'tmem_package_event_types', $event_types, $package_id );
} // tmem_get_package_event_types
 ...
 */
function tmem_package_is_restricted_by_date( $package_id ) {
	...
	if ( $restricted ) {
		$months = tmem_get_package_months_available( $package_id );
	...
	return apply_filters( 'tmem_package_is_restricted_by_date', $restricted, $package_id );

} // tmem_package_is_restricted_by_date
 ...
 */
function tmem_get_package_months_available( $package_id ) {
	$months = get_post_meta( $package_id, '_package_months', true );

	return apply_filters( 'tmem_package_months_available', $months, $package_id );
} // tmem_get_package_months_available
 ...
 */
function tmem_get_package_price( $package_id, $date = null ) {
	if ( isset( $date ) ) {
		$price = tmem_get_package_price_for_month( $package_id, $date );
	} else {
		$price = get_post_meta( $package_id, '_package_price', true );
	}

	return apply_filters( 'tmem_package_price', $price, $package_id );
} // tmem_get_package_price
 ...
 * @return str The cost of the package.
 */
function tmem_get_package_price_for_month( $package_id, $date = null ) {

	if ( ! tmem_package_has_variable_prices( $package_id ) ) {
		return tmem_get_package_price( $package_id );
	}

	$price = tmem_get_package_price( $package_id );
	...
	$monthly_prices = tmem_get_package_variable_prices( $package_id );
...
} // tmem_get_package_price_for_month
 ...
 */
function tmem_package_has_variable_prices( $package_id ) {
	$variable_pricing = get_post_meta( $package_id, '_package_variable_pricing', true );

	return apply_filters( 'tmem_package_has_variable_pricing', $variable_pricing, $package_id );
} // tmem_package_has_variable_prices
 ...
 * @return bool True if variable pricing is enabled.
 */
function tmem_get_package_variable_prices( $package_id ) {
	$variable_pricing = get_post_meta( $package_id, '_package_variable_prices', true );

	return apply_filters( 'tmem_package_monthly_pricing', $variable_pricing, $package_id );
} // tmem_get_package_variable_prices
 ...
 */
function tmem_get_package_price_range( $package_id ) {
	...
	$prices = tmem_get_package_variable_prices( $package_id );
	...
	return apply_filters( 'tmem_package_price_range', $range, $package_id );

} // tmem_get_package_price_range
 ...
 */
function tmem_get_package_addons( $package_id ) {
	$items = get_post_meta( $package_id, '_package_items', true );

	return apply_filters( 'tmem_package_items', $items, $package_id );
} // tmem_get_package_addons
 ...
 */
function tmem_package_has_addon( $package_id, $addon_id ) {
	$addons = tmem_get_package_addons( $addon_id );

	return in_array( $addon_id, $addons );
} // tmem_package_has_addon
 ...
 */
function tmem_get_packages_with_addons( $addon_ids ) {

	return tmem_get_packages(
		array(
			'meta_query' => array(
				array(
					'key'     => '_package_items',
 ...
 * @return int
 */
function tmem_count_packages_with_addon( $addon_id ) {
	...
	if ( $packages ) {
		foreach ( $packages as $package ) {
			$addons = tmem_get_package_addons( $package->ID );
...
} // tmem_count_packages_with_addon
 ...
 */
function tmem_get_packages_by_employee( $employee_id = 0, $enabled = true ) {
	...
	$packages = tmem_get_packages( $args );

	return apply_filters( 'tmem_get_packages_by_employee', $packages );

} // tmem_get_packages_by_employee
 ...
 */
function tmem_employee_has_package( $package_id, $employee_id = 0 ) {

	$employees = tmem_get_employees_with_package( $package_id );
 ...
 */
function tmem_package_is_available_for_event_type( $package_id, $event_type_term = '' ) {

	if ( empty( $event_type_term ) ) {
		return true;
	}

	$event_types = tmem_get_package_event_types( $package_id );
...
} // tmem_package_is_available_for_event_type
 ...
 */
function tmem_package_is_available_for_event_date( $package_id, $event_date = '' ) {
	...
	$event_months = tmem_get_package_months_available( $package_id );
...
} // tmem_package_is_available_for_event_date
 ...
 */
function tmem_get_event_package( $event_id ) {
	$package = get_post_meta( $event_id, '_tmem_event_package', true );

	return apply_filters( 'tmem_event_package', $package );
} // tmem_get_event_package
 ...
 */
function tmem_get_events_with_package( $package_id ) {
	return tmem_get_events(
		array(
			'meta_query' => array(
				'key'     => '_tmem_event_package',
				'value'   => $package_id,
				'compare' => '=',
			),
		)
	);
} // tmem_get_events_with_package
 ...
 * @return int
 */
function tmem_count_events_with_package( $package_id ) {
	global $wpdb;
...
} // tmem_count_events_with_package
 ...
 */
function tmem_get_available_packages( $args = array() ) {
	...
	$packages = tmem_get_packages( $package_args );

	return apply_filters( 'tmem_available_packages', $packages );

} // tmem_get_available_packages
 ...
 */
function tmem_list_available_packages( $employee_id = 0, $price = false ) {
	...
	if ( ! empty( $employee_id ) ) {
		$packages = tmem_get_packages_by_employee( $employee_id, false );
	} else {
		$packages = tmem_get_packages();
	}
	...
	foreach ( $packages as $package ) {
		...
		if ( $price ) {
			$package_price = ' ' . tmem_currency_filter( tmem_format_amount( tmem_get_package_price( $package->ID ) ) );
		}

		$return[] = get_the_title( $package->ID ) . '' . $package_price;
	}

	$return = apply_filters( 'tmem_list_packages', $return, $price );

	return implode( '<br />', $return );

} // tmem_list_available_packages
 ...
 * @return str
 */
function tmem_get_package_excerpt( $package_id, $length = null ) {

	if ( ! isset( $length ) ) {
		$length = tmem_get_option( 'package_excerpt_length', 55 );
	}
	...
	return apply_filters( 'tmem_package_excerpt', $description );

} // tmem_get_package_excerpt
 ...
 */
function tmem_package_dropdown( $args = array(), $structure = true ) {

	global $current_user;

	$defaults = array(
		'name'            => '_tmem_event_package',
		'id'              => '',
		'class'           => '',
		'selected'        => '',
		'first_entry'     => '',
	...
	if ( $structure ) {
		$output = sprintf(
			...
			esc_attr( $args['name'] ),
			esc_attr( tmem_sanitize_key( $args['id'] ) ),
	...
	$output .= TMEM()->html->packages_dropdown( $args );
 ...
 * @return void
 */
function tmem_remove_package_from_events( $package_id ) {

	$events = tmem_get_events_with_package( $package_id );

	if ( $events ) {
		foreach ( $events as $event ) {
			delete_post_meta( $event->ID, '_tmem_event_package' );
		}
	}

} // tmem_remove_addons_from_events
add_action( 'tmem_delete_package', 'tmem_remove_package_from_events' );
 ...
 */
function tmem_get_addons( $args = array() ) {
	...
	return apply_filters( 'tmem_get_addons', get_posts( $args ) );
 ...
 * @return WP_Post object.
 */
function tmem_get_addon( $addon_id ) {
	return tmem_get_addon_by( 'id', $addon_id );
} // tmem_get_addon
 ...
 * @return obj The WP Post object.
 */
function tmem_get_addon_by( $field, $value ) {
	...
	switch ( strtolower( $field ) ) {
		...
		case 'id':
			...
			if ( get_post_type( $addon ) !== 'tmem-addon' ) {
				return false;
			}
 ...
 * @return arr
 */
function tmem_get_addon_data( $addon ) {

	$addon_id    = is_object( $addon ) ? $addon->ID : $addon;
	$events      = tmem_get_addon_event_types( $addon_id );
	$users       = tmem_get_employees_with_addon( $addon_id );
	$packages    = tmem_get_packages_with_addons( $addon_id );
	...
	if ( ! tmem_addon_is_restricted_by_date( $addon_id ) ) {
		$months[] = __( 'Always', 'mobile-events-manager' );
	} else {
		$availability = tmem_get_addon_months_available( $addon_id );
	...
	if ( tmem_addon_has_variable_prices( $addon_id ) ) {
		$range = tmem_get_addon_price_range( $addon_id );

		$price = tmem_get_currency() . ' ' . tmem_format_amount( $range['low'] ) . ' &mdash; ' . tmem_format_amount( $range['high'] );

	} else {
		$price = tmem_get_currency() . ' ' . tmem_format_amount( tmem_get_addon_price( $addon_id ) );
	}

	if ( $packages ) {
		foreach ( $packages as $package ) {
			$in_packages[] = array( $package->ID => tmem_get_package_name( $package->ID ) );
		}
	}
	...
	$addon_data = array(
		'name'         => tmem_get_addon_name( $addon_id ),
		...
		'usage'        => array(
			'packages' => tmem_count_packages_with_addon( $addon_id ),
			'events'   => tmem_count_events_with_addon( $addon_id ),
		),
	);

	return apply_filters( 'tmem_get_addon_data', $addon_data );
 ...
 * @return arr|bool Addons.
 */
function tmem_get_addons_in_category( $term_id ) {
	...
	return tmem_get_addons( $args );

} // tmem_get_addons_in_category
 ...
 */
function tmem_get_addon_name( $addon_id ) {
	$title = get_the_title( $addon_id );

	return apply_filters( 'tmem_addon_name', $title, $addon_id );
} // tmem_get_addon_name
 ...
 */
function tmem_get_employees_with_addon( $addon_id ) {
	$employees = get_post_meta( $addon_id, '_addon_employees', true );

	return apply_filters( 'tmem_employees_with_addon', $employees, $addon_id );
} // tmem_get_employees_with_addon
 ...
 */
function tmem_get_addon_event_types( $addon_id ) {
	...
	return apply_filters( 'tmem_addon_event_types', $event_types, $addon_id );
} // tmem_get_addon_event_types
 ...
 * @return bool True if restricted, otherwise false.
 */
function tmem_addon_is_restricted_by_date( $addon_id ) {
	...
	if ( $restricted ) {
		$months = tmem_get_addon_months_available( $addon_id );
	...
	return apply_filters( 'tmem_addon_is_restricted_by_date', $restricted, $addon_id );

} // tmem_addon_is_restricted_by_date
 ...
 */
function tmem_get_addon_months_available( $addon_id ) {
	$months = get_post_meta( $addon_id, '_addon_months', true );

	return apply_filters( 'tmem_addon_months_available', $months, $addon_id );
} // tmem_get_addon_months_available
 ...
 */
function tmem_get_addon_price( $addon_id, $date = null ) {
	if ( isset( $date ) ) {
		$price = tmem_get_addon_price_for_month( $addon_id, $date );
	} else {
		$price = get_post_meta( $addon_id, '_addon_price', true );
	}

	return apply_filters( 'tmem_addon_price', $price, $addon_id );
} // tmem_get_addon_price
 ...
 * @return str The cost of the addon.
 */
function tmem_get_addon_price_for_month( $addon_id, $date = null ) {

	if ( ! tmem_addon_has_variable_prices( $addon_id ) ) {
		return tmem_get_addon_price( $addon_id );
	}

	$price = tmem_get_addon_price( $addon_id );
	...
	$monthly_prices = tmem_get_addon_variable_prices( $addon_id );
...
} // tmem_get_addon_price_for_month
 ...
 * @return bool True if variable pricing is enabled.
 */
function tmem_addon_has_variable_prices( $addon_id ) {
	$variable_pricing = get_post_meta( $addon_id, '_addon_variable_pricing', true );

	return apply_filters( 'tmem_addon_has_variable_pricing', $variable_pricing, $addon_id );
} // tmem_addon_has_variable_prices
 ...
 * @return bool True if variable pricing is enabled.
 */
function tmem_get_addon_variable_prices( $addon_id ) {
	$variable_pricing = get_post_meta( $addon_id, '_addon_variable_prices', true );

	return apply_filters( 'tmem_addon_monthly_pricing', $variable_pricing, $addon_id );
} // tmem_get_addon_variable_prices
 ...
 * @return arr Array of low and high prices.
 */
function tmem_get_addon_price_range( $addon_id ) {
	...
	$prices = tmem_get_addon_variable_prices( $addon_id );
	...
	return apply_filters( 'tmem_addon_price_range', $range, $addon_id );

} // tmem_get_addon_price_range
 ...
 */
function tmem_get_addons_by_employee( $employee_id = 0, $enabled = true ) {
	...
	$addons = tmem_get_addons( $args );

	return apply_filters( 'tmem_get_addons_by_employee', $addons );

} // tmem_get_addons_by_employee
 ...
 */
function tmem_employee_has_addon( $addon_id, $employee_id = 0 ) {

	$employees = tmem_get_employees_with_addon( $addon_id );
 ...
 */
function tmem_addon_is_available_for_event_type( $addon_id, $event_type_term = '' ) {

	if ( empty( $event_type_term ) ) {
		return true;
	}

	$event_types = tmem_get_addon_event_types( $addon_id );
...
} // tmem_addon_is_available_for_event_type
 ...
 */
function tmem_addon_is_available_for_event_date( $addon_id, $event_date = '' ) {
	...
	$event_months = tmem_get_addon_months_available( $addon_id );
	$event_month  = gmdate( 'n', strtotime( $event_date ) );
...
} // tmem_addon_is_available_for_event_date
 ...
 */
function tmem_get_event_addons( $event_id ) {
	$addons = get_post_meta( $event_id, '_tmem_event_addons', true );

	return apply_filters( 'tmem_event_addons', $addons );
} // tmem_get_event_addons
 ...
 * @return mixed Array of WP_Post objects or false.
 */
function tmem_get_events_with_addons( $addon_ids ) {
	...
	return tmem_get_events( $args );

} // tmem_get_events_with_addons
 ...
 * @return int
 */
function tmem_count_events_with_addon( $addon_id ) {

	global $wpdb;

	$count  = 0;
	$query  = "SELECT * FROM $wpdb->postmeta WHERE meta_value != '' AND meta_key = '_tmem_event_addons'";
	$events = $wpdb->get_results( $query );

	if ( $events ) {
		foreach ( $events as $event ) {
			$addons = tmem_get_event_addons( $event->post_id );
 ...
 * @return int|false The event addons or false if no addons.
 */
function tmem_list_event_addons( $event_id, $price = false ) {
	...
	$event_addons = $tmem_event->get_addons();
	$event_date   = $tmem_event->date;
	...
	if ( $event_addons ) {
		...
		foreach ( $event_addons as $addon_id ) {
			...
			if ( $price ) {
				$addon_price = ' ' . tmem_currency_filter( tmem_format_amount( tmem_get_addon_price( $addon_id, $event_date ) ) );
			}

			$addons[] = tmem_get_addon_name( $addon_id ) . '' . $addon_price;
		}
	...
	return apply_filters( 'tmem_list_event_addons', $output );

} // tmem_list_event_addons
 ...
 */
function tmem_get_available_addons( $args = array() ) {
	...
	if ( ! empty( $args['event_id'] ) ) {
		$event_addons = tmem_get_event_addons( $args['event_id'] );
	...
	if ( ! empty( $args['package'] ) ) {
		if ( 'tmem-package' === get_post_type( $args['package_id'] ) ) {
			$package_items = tmem_get_package_addons( $args['package_id'] );
	...
	$addons = tmem_get_addons( $addon_args );

	return apply_filters( 'tmem_available_addons', $addons );

} // tmem_get_available_addons
 ...
 * @return str HTML formatted string listing package information
 */
function tmem_list_available_addons( $employee_id = 0, $price = false ) {
	...
	if ( ! empty( $employee_id ) ) {
		$addons = tmem_get_addons_by_employee( $employee_id, false );
	} else {
	...
	foreach ( $addons as $addon ) {
		...
		if ( $price ) {
			$addon_price = ' ' . tmem_currency_filter( tmem_format_amount( tmem_get_package_price( $addon->ID ) ) );
		}

		$return[] = get_the_title( $addon->ID ) . '' . $addon_price;
	}

	$return = apply_filters( 'tmem_list_available_addons', $return, $price );

	return implode( '<br />', $return );

} // tmem_list_available_addons
 ...
 * @return str
 */
function tmem_get_addon_excerpt( $addon_id, $length = null ) {
	...
	return apply_filters( 'tmem_addon_excerpt', $description );

} // tmem_get_addon_excerpt
 ...
 * @return HTML output for select field
 */
function tmem_addons_dropdown( $args = array(), $structure = true ) {
	global $current_user;
	...
	$output .= TMEM()->html->addons_dropdown( $args );
 ...
 */
function tmem_addons_checkboxes( $args = array() ) {
	global $current_user;
		...
		foreach ( $addons as $addon ) {
			if ( ! empty( $args['package'] ) ) {
				if ( is_numeric( $args['package'] ) ) {
					$package = tmem_get_package( $args['package'] );
				} else {
					$package = tmem_get_package_by( 'slug', $args['package'] );
				}

				if ( $package ) {
					$package_items = tmem_get_package_addons( $package->ID );
				}
			...
			if ( ! empty( $args['employee'] ) ) {
				if ( ! tmem_employee_has_addon( $addon->ID, $args['employee'] ) ) {
					continue;
				}
			}

			if ( $args['event_type'] ) {
				if ( ! tmem_addon_is_available_for_event_type( $addon->ID, $args['event_type'] ) ) {
					continue;
				}
			}

			if ( $args['event_date'] ) {
				if ( ! tmem_addon_is_available_for_event_date( $addon->ID, $args['event_date'] ) ) {
					continue;
				}
			} else {
			...
			$price = '';
			if ( true === $args['cost'] ) {
				$price .= ' - ' . tmem_currency_filter( tmem_format_amount( tmem_get_addon_price( $addon->ID, $args['event_date'] ) ) );
			}
		...
		foreach ( $options as $term => $addons ) {
			...
			foreach ( $addons as $items ) {
				foreach ( $items as $item_id => $item ) {
					$output .= '<label for="' . esc_attr( $args['name'] ) . '-' . $item_id . '" title="' . tmem_get_addon_excerpt( $item_id ) . '">';
					$output .= sprintf(
 ...
 */
function tmem_set_addon_category( $addon_id, $term_id ) {
 ...
 * @return void
 */
function tmem_remove_addons_from_packages( $addon_ids ) {

	if ( ! is_array( $addon_ids ) ) {
		$addon_ids = array( $addon_ids );
	}

	$packages = tmem_get_packages_with_addons( $addon_ids );

	if ( $packages ) {
		foreach ( $packages as $package ) {
			foreach ( $addon_ids as $addon_id ) {
				tmem_remove_addon_from_package( $package->ID, $addon_id );
			}
		}
	}

} // tmem_remove_addons_from_packages
add_action( 'tmem_delete_addon', 'tmem_remove_addons_from_packages', 10 );
 ...
 */
function tmem_remove_addon_from_package( $package_id, $addon_id ) {

	$addons = tmem_get_package_addons( $package_id );
	$items  = array();
...
} // tmem_remove_addon_from_package
 ...
 * @return void
 */
function tmem_remove_addons_from_events( $addon_ids ) {

	if ( ! is_array( $addon_ids ) ) {
		$addon_ids = array( $addon_ids );
	}

	$events = tmem_get_events_with_addons( $addon_ids );

	if ( $events ) {
		foreach ( $events as $event ) {
			foreach ( $addon_ids as $addon_id ) {
				tmem_remove_addon_from_event( $event->ID, $addon_id );
			}
		}
	}

} // tmem_remove_addons_from_events
add_action( 'tmem_delete_addon', 'tmem_remove_addons_from_events', 15 );
 ...
 * @param int $addon_id The ID of the addon to remove.
 */
function tmem_remove_addon_from_event( $event_id, $addon_id ) {

	$addons = tmem_get_event_addons( $event_id );
	$items  = array();
	...
	if ( $addons ) {
		...
		update_post_meta( $event_id, '_tmem_event_addons', $items );
	}

} // tmem_remove_addon_from_event
