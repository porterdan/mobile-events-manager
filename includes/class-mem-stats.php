 ...
 * MEM_Stats Class
 *
 * Base class for other MEM Stat classes.
...
class MEM_Stats {
	...
	public function get_predefined_dates() {
		...
		return apply_filters( 'mem_stats_predefined_dates', $predefined );
	...
	public function convert_date( $date, $end_date = false ) {
		...
		return apply_filters( 'mem_stats_date', $date, $end_date, $this );
	...
	public function get_events_by_date( $day = null, $month_num, $year = null, $hour = null ) {
		...
		$args   = apply_filters( 'mem_get_events_by_date_args', $args );
		$key    = 'mem_stats_' . substr( md5( serialize( $args ) ), 0, 15 );
	...
	public function get_total_income_by_date( $period = 'this_week' ) {
		return $this->get_txns_total_by_date( $period, 'mem-income' );
	...
	public function get_total_outgoings_by_date( $period = 'this_week' ) {
		return $this->get_txns_total_by_date( $period, 'mem-expenditure' );
	...
	public function get_txns_total_by_date( $period = 'this_week', $status = 'any' ) {
		...
		if ( $txns ) {
			...
			foreach ( $txns as $txn ) {
				...
				if ( 'any' === $args['post_status'] ) {
					...
					if ( 'mem-income' === $txn->post_status ) {
						$total += mem_sanitize_amount( get_post_meta( $txn->ID, '_mem_txn_total', true ) );
					} else {
						$total -= mem_sanitize_amount( get_post_meta( $txn->ID, '_mem_txn_total', true ) );
					}
				} else {
		...
		return mem_currency_filter( mem_format_amount( $total ) );
	...
	public function get_income_by_date( $day = null, $month_num, $year = null, $hour = null ) {
		...
		$args = apply_filters( 'mem_get_income_by_date_args', $args );
		...
		if ( $txns ) {
			...
			$income = $wpdb->get_var( "SELECT SUM(meta_value) FROM $wpdb->postmeta WHERE meta_key = '_mem_txn_total' AND post_id IN ({$txns})" );
	...
	public function get_expenses_by_date( $day = null, $month_num, $year = null, $hour = null ) {
		...
		$args = apply_filters( 'mem_get_expenses_by_date_args', $args );
		...
		if ( $txns ) {
			...
			$expense = $wpdb->get_var( "SELECT SUM(meta_value) FROM $wpdb->postmeta WHERE meta_key = '_mem_txn_total' AND post_id IN ({$txns})" );
	...
	public function get_earnings_by_date( $day = null, $month_num, $year = null, $hour = null ) {
		...
		$args     = apply_filters( 'mem_get_earnings_by_date_args', $args );
		$key      = 'mem_stats_' . substr( md5( serialize( $args ) ), 0, 15 );
	...
	public function get_conversions( $event_ids = false, $tax = 'event-types', $term, $start_date = false, $end_date = false ) {
		$converted_statuses = apply_filters( 'mem_converted_event_statuses', array( 'mem-approved', 'mem-contract', 'mem-completed', 'mem-cancelled' ) );
