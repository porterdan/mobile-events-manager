...
/**
 * MEM_DB_Playlists Class
 *
 * @since 1.5
 */
class MEM_DB_Playlists extends MEM_DB {
	...
	public function __construct() {
		...
		$this->table_name  = $wpdb->prefix . 'mem_playlists';
		$this->primary_key = 'id';
		$this->version     = '1.0';
	...
	public function get_required_fields() {
		return apply_filters(
			'mem_db_playlist_required_fields',
			array(
				'event_id',
				'artist',
				'song',
	...
	public function add( $data = array() ) {
		...
		if ( $return ) {
			foreach ( $meta as $key => $value ) {
				MEM()->playlist_meta->update_meta( $return, $key, $value );
			}
		}
	...
	public function get_entries( $args = array() ) {
		...
		$cache_key = md5( 'mem_playlist_' . serialize( $args ) );
	...
	public function count( $args = array() ) {
		...
		$cache_key = md5( 'mem_playlist_count' . serialize( $args ) );
