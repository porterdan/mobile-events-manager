 ...
 */
function tmem_playlist_global_limit() {
	return (int) tmem_get_option( 'playlist_limit' );
} // tmem_playlist_global_limit
 ...
 * @return arr
 */
function tmem_get_playlist_entry( $entry_id = 0 ) {
	...
	if ( get_post_type( $entry_id ) !== 'tmem-playlist' ) {
		return false;
	}

	return $entry;
} // tmem_get_playlist_entry
 ...
 * @return string
 */
function tmem_playlist_twitter_share( $event_id, $args = array() ) {

	$defaults = array(
		'text'        => sprintf( esc_html__( 'Looking forward to my %s? Add your song requests at', 'mobile-events-manager' ), esc_html( tmem_get_label_singular( true ) ) ),
 ...
 * @return string
 */
function tmem_playlist_facebook_share( $event_id, $args = array() ) {

	$defaults = array(
		'href'          => tmem_guest_playlist_url( $event_id ),
		'layout'        => 'button',
		'size'          => 'small',
		'mobile_iframe' => 'false',
	);
 ...
 * @return obj
 */
function tmem_get_playlist_entries( $event_id, $args = array() ) {

	$defaults = array(
		'post_type'      => 'tmem-playlist',
		'posts_per_page' => -1,
		'post_status'    => 'publish',
	...
	return apply_filters( 'tmem_get_playlist_entries', $entries, $event_id );

} // tmem_get_playlist_entries
 ...
 */
function tmem_get_playlist_entry_data( $entry_id ) {
	$entry_data = array(
		'song'     => get_post_meta( $entry_id, '_tmem_playlist_entry_song', true ),
		'artist'   => get_post_meta( $entry_id, '_tmem_playlist_entry_artist', true ),
		'added_by' => get_post_meta( $entry_id, '_tmem_playlist_entry_added_by', true ),
		'djnotes'  => get_post_meta( $entry_id, '_tmem_playlist_entry_djnotes', true ),
	);

	return apply_filters( 'tmem_get_playlist_entry_data', $entry_data );
} // tmem_get_playlist_entry_data
 ...
 */
function tmem_store_playlist_entry( $data ) {
	$meta = array(
	...
	$meta = apply_filters( 'tmem_insert_playlist_entry', $meta );

	do_action( 'tmem_insert_playlist_entry_before', $meta );

	$title = sprintf(
		__( '%1$s ID: %2$s %3$s %4$s', 'mobile-events-manager' ),
		esc_html( tmem_get_label_singular() ),
		tmem_get_option( 'event_prefix', '' ) . $event_id,
		$meta['song'],
		$meta['artist']
	);
	...
	if ( ! empty( $term ) ) {
		tmem_set_playlist_entry_category( $entry_id, $term );
	}

	foreach ( $meta as $key => $value ) {
		update_post_meta( $entry_id, '_tmem_playlist_entry_' . $key, $value );
	}

	do_action( 'tmem_insert_playlist_entry_after', $meta, $entry_id );

	return $entry_id;
} // tmem_store_playlist_entry
 ...
 */
function tmem_remove_stored_playlist_entry( $entry_id ) {
	do_action( 'tmem_delete_playlist_entry_before', $entry_id );

	$entry = wp_delete_post( $entry_id, true );

	do_action( 'tmem_delete_playlist_entry_after', $entry );

	return $entry;
} // tmem_remove_stored_playlist_entry
 ...
 * @return bool Whether or not the entry was created.
 */
function tmem_store_guest_playlist_entry( $data ) {
	$meta = array(
		'song'     => sanitize_text_field( $data['tmem_guest_song'] ),
		'artist'   => isset( $data['tmem_guest_artist'] ) ? sanitize_text_field( $data['tmem_guest_artist'] ) : '',
		'added_by' => ucwords( sanitize_text_field( $data['tmem_guest_name'] ) ),
		'to_tmem'  => '',
		'uploaded' => false,
	);
	...
	$event_id = absint( $data['tmem_playlist_event'] );
	$meta     = apply_filters( 'tmem_insert_guest_playlist_entry_meta', $meta );
	...
	do_action( 'tmem_insert_guest_playlist_entry', $entry_id, $event_id );

	return $entry_id;
} // tmem_store_guest_playlist_entry
 ...
 * @return bool
 */
function tmem_playlist_entry_exists( $entry_id ) {
	if ( tmem_get_playlist_entry( $entry_id ) ) {
		return true;
	}

	return false;
}//end tmem_playlist_entry_exists()
 ...
 */
function tmem_set_playlist_entry_category( $event_id, $term_id ) {
...
} // tmem_set_playlist_entry_category
 ...
 */
function tmem_get_playlist_by_category( $event_id, $args = array() ) {
	...
	$terms    = tmem_get_event_playlist_categories( $event_id, $args );
	$playlist = array();
 ...
 */
function tmem_get_event_playlist_categories( $event_id ) {
	$terms      = tmem_get_playlist_categories();
	$categories = array();
	...
	return $categories;
} // tmem_get_event_playlist_categories
 ...
 * @return int|false
 */
function tmem_get_event_playlist_limit( $event_id ) {
	$playlist_limit = get_post_meta( $event_id, '_tmem_event_playlist_limit', true );

	return (int) apply_filters( 'tmem_event_playlist_limit', $playlist_limit, $event_id );
} // tmem_get_event_playlist_limit
 ...
 */
function tmem_playlist_is_enabled( $event_id ) {
	if ( 'Y' !== get_post_meta( $event_id, '_tmem_event_playlist', true ) ) {
		return true;
	}

	return false;
} // tmem_playlist_is_enabled
 ...
 * @return bool
 */
function tmem_event_has_playlist( $event_id ) {
	$playlist = get_posts(
		array(
			'post_type'      => 'tmem-playlist',
			'post_status'    => 'publish',
 ...
 * @return bool True if the playlist is open, false if not.
 */
function tmem_playlist_is_open( $event_id ) {
	// Playlist disabled for this event.
	if ( ! tmem_playlist_is_enabled( $event_id ) ) {
		return false;
	}
 ...
 */
function tmem_count_playlist_entries( $event_id, $category = false ) {
	$entry_query = array(
		'post_type'      => 'tmem-playlist',
		'post_status'    => 'publish',
		'post_parent'    => $event_id,
 ...
 */
function tmem_playlist_duration( $event_id = '', $songs = '', $song_duration = 180 ) {
	if ( empty( $songs ) ) {
		$songs = tmem_count_playlist_entries( $event_id );
	}
	...
	return apply_filters( 'tmem_playlist_duration', $duration, $event_id, $songs, $song_duration );
} // tmem_playlist_duration
 ...
 */
function tmem_guest_playlist_url( $event_id ) {
	$access_code = get_post_meta( $event_id, '_tmem_event_playlist_access', true );

	if ( empty( $access_code ) ) {
		$url = '';
	} else {
		$url = tmem_get_formatted_url( tmem_get_option( 'playlist_page' ), true ) . 'guest_playlist=' . $access_code;
	}

	return $url;
} // tmem_guest_playlist_url
 ...
 */
function tmem_get_playlist_categories() {
	$terms = get_terms(
		array(
			'taxonomy'   => 'playlist-category',
			'hide_empty' => false,
 ...
 */
function tmem_playlist_category_dropdown( $args = '' ) {
	$defaults = array(
		'show_option_all'   => '',
		'show_option_none'  => '',
		'option_none_value' => '-1',
		...
		'exclude'           => '',
		'echo'              => 0,
		'selected'          => tmem_get_option( 'playlist_default_cat', 0 ),
		'hierarchical'      => 0,
		'name'              => 'tmem_category',
		'id'                => 'tmem_category',
		'class'             => 'tmem-input',
...
} // tmem_playlist_category_dropdown
 ...
 */
function tmem_generate_playlist_guest_code() {
	$code = substr( str_shuffle( 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789' ), 0, 9 );

	return apply_filters( 'tmem_generate_playlist_guest_code', $code );
} // tmem_generate_playlist_guest_code

/**
 * Retrieve playlist entries yet to be uploaded to TMEM.
 *
 * @since 1.3
 * @param
 * @return obj $entries WP_Query results of entries to upload.
 */
function tmem_get_playlist_entries_to_upload() {
	$args = array(
		'post_type'      => 'tmem-playlist',
		'post_status'    => 'publish',
		'posts_per_page' => -1,
		'meta_query'     => array(
			array(
				'key'     => '_tmem_playlist_entry_uploaded',
				'value'   => true,
				'compare' => '!=',
				'type'    => 'NUMERIC',
			),
	...
	$args = apply_filters( 'tmem_get_playlist_entries_to_upload', $args );
...
} // tmem_get_playlist_entries_to_upload
 ...
 */
function tmem_prepare_playlist_upload_data() {
	$entries = tmem_get_playlist_entries_to_upload();
	...
	foreach ( $entries as $entry ) {
		...
		if ( 'tmem-completed' != get_post_status( $entry->post_parent ) ) {
			continue;
		} else {

			$tmem_event = new TMEM_event( $entry->post_parent );
			...
			$uploads[ $entry->ID ] = array(
				...
				'event_date' => gmdate( 'Y-m-d', strtotime( $tmem_event->date ) ),
				'event_type' => esc_attr( urlencode( $tmem_event->get_type() ) ),
				'song'       => esc_attr( urlencode( stripslashes( get_post_meta( $entry->ID, '_tmem_playlist_entry_song', true ) ) ) ),
				'artist'     => esc_attr( urlencode( stripslashes( get_post_meta( $entry->ID, '_tmem_playlist_entry_artist', true ) ) ) ),
			);
	...
	return $uploads;
} // tmem_prepare_playlist_upload_data

/**
 * Process the playlist upload to TMEM.
 *
 * @since 1.3
 * @param
 * @return void
 */
function tmem_process_playlist_upload() {
	$entries = tmem_prepare_playlist_upload_data();

	if ( empty( $entries ) ) {
		TMEM()->debug->log_it( __( 'There are no playlist entries to upload', 'mobile-events-manager' ) );
	...
	$data = array(
		...
		'company' => urlencode( tmem_get_option( 'company_name', get_bloginfo( 'name' ) ) ),
	);
	...
	foreach ( $entries as $id => $entry ) {
		...
		$rpc = 'https://tmem.co.uk/?tmem-api=TMEM_PLAYLIST';
		...
		if ( $response ) {
			...
			update_post_meta( $id, '_tmem_playlist_entry_to_tmem', current_time( 'mysql' ) );
			update_post_meta( $id, '_tmem_playlist_entry_uploaded', true );
...
/**
 * Return the number of playlist entries pending upload to TMEM
 *
 * @since 1.3
 * @param
 * @return int The total number of entries pending upload.
 */
function tmem_get_pending_upload_playlist_entry_count() {
	$entries = tmem_get_playlist_entries_to_upload();
	...
	return $count;
} // tmem_get_pending_upload_playlist_entry_count

/**
 * Return the number of playlist entries uploaded to TMEM
 *
 * @since 1.3
 * @param
 * @return int The total number of entries uploaded.
 */
function tmem_get_uploaded_playlist_entry_count() {
	$args = array(
		'post_type'      => 'tmem-playlist',
		'post_status'    => 'publish',
		'posts_per_page' => -1,
	...
	return $count;
} // tmem_get_uploaded_playlist_entry_count
