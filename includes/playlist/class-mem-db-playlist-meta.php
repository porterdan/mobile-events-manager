...
class MEM_DB_Playlist_Meta extends MEM_DB {
	...
	public function __construct() {
		...
		$this->table_name  = $wpdb->prefix . 'mem_playlistmeta';
	...
	public function register_table() {
		global $wpdb;
		$wpdb->mem_playlistmeta = $this->table_name;
	} // register_table

	/**
	 * Retrieve playlist meta field for an entry.
	 *
	 * For internal use only. Use MEM_Playlist->get_meta() for public usage.
	 *
	...
	public function get_meta( $id = 0, $meta_key = '', $single = false ) {
		...
		return get_metadata( 'mem_playlist', $entry_id, $meta_key, $single );
	} // get_meta

	/**
	 * Add meta data field to a playlist entry.
	 *
	 * For internal use only. Use MEM_Playlist->add_meta() for public usage.
	 *
	...
	public function add_meta( $id = 0, $meta_key = '', $meta_value, $unique = false ) {
		...
		return add_metadata( 'mem_playlist', $id, $meta_key, $meta_value, $unique );
	} // add_meta

	/**
	 * Update playist entry meta field based on entry ID.
	 *
	 * For internal use only. Use MEM_Playlist->update_meta() for public usage.
	 *
	...
	public function update_meta( $id = 0, $meta_key = '', $meta_value, $prev_value = '' ) {
		...
		return update_metadata( 'mem_playlist', $id, $meta_key, $meta_value, $prev_value );
	} // update_meta

	/**
	 * Remove metadata matching criteria from a playlist entry.
	 *
	 * For internal use only. Use MEM_Playlist->delete_meta() for public usage.
	 *
	...
	public function delete_meta( $id = 0, $meta_key = '', $meta_value = '' ) {
		return delete_metadata( 'mem_playlist', $id, $meta_key, $meta_value );
	} // delete_meta
