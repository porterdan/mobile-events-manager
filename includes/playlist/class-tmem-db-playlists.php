...
/**
 * TMEM_DB_Playlists Class
 *
 * @since 1.5
 */
class TMEM_DB_Playlists extends TMEM_DB {
	...
	public function __construct() {
		...
		$this->table_name  = $wpdb->prefix . 'tmem_playlists';
		$this->primary_key = 'id';
		$this->version     = '1.0';
	...
	public function get_required_fields() {
		return apply_filters(
			'tmem_db_playlist_required_fields',
			array(
				'event_id',
				'artist',
				'song',
	...
	public function add( $data = array() ) {
		...
		if ( $return ) {
			foreach ( $meta as $key => $value ) {
				TMEM()->playlist_meta->update_meta( $return, $key, $value );
			}
		}
	...
	public function get_entries( $args = array() ) {
		...
		$cache_key = md5( 'tmem_playlist_' . serialize( $args ) );
	...
	public function count( $args = array() ) {
		...
		$cache_key = md5( 'tmem_playlist_count' . serialize( $args ) );
