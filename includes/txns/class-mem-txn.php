...
class MEM_Txn {
	...
	private function setup_txn( $txn ) {
		...
		if ( 'mem-transaction' !== $txn->post_type ) {
			return false;
		}
	...
	public function __get( $key ) {
		if ( method_exists( $this, 'get_' . $key ) ) {
			...
			return new WP_Error( 'mem-txn-invalid-property', sprintf( esc_html__( "Can't get property %s", 'mobile-events-manager' ), $key ) );
		}
	} // __get
	...
	public function create( $data = array(), $meta = array() ) {
		...
		do_action( 'mem_pre_txn_create', $data, $meta );

		$id = wp_insert_post( $data, true );

		if ( is_wp_error( $id ) ) {
			MEM()->debug->log_it( 'ERROR: ' . $id->get_error_message() );
		}
		...
		if ( $txn ) {
			...
			mem_update_txn_meta( $txn->ID, $meta );

			wp_update_post(
				array(
					'ID'         => $id,
					'post_title' => mem_get_option( 'event_prefix' ) . $id,
					'post_name'  => mem_get_option( 'event_prefix' ) . $id,
				)
			);
		}

		do_action( 'mem_post_txn_create', $id, $data, $meta );

		add_action( 'save_post_mem-transaction', 'mem_save_txn_post', 10, 3 );

		$cache_key = md5( sprintf( 'mem_event_income_txns_%s', $data['post_parent'] ) );
	...
	public function get_status() {
		if ( empty( $this->payment_status ) ) {
			$this->payment_status = get_post_meta( $this->ID, '_mem_txn_status', true );
		}
	...
	public function get_currency() {
		if ( empty( $this->currency ) ) {
			$this->currency = get_post_meta( $this->ID, '_mem_txn_currency', true );
		}
	...
	public function get_price() {
		if ( empty( $this->price ) ) {
			$this->price = get_post_meta( $this->ID, '_mem_txn_total', true );
		}
	...
	public function get_recipient_id() {
		...
		if ( empty( $this->recipient_id ) ) {
			...
			if ( 'mem-income' === get_post_status( $this->ID ) ) {
				$this->recipient_id = get_post_meta( $this->ID, '_mem_payment_from', true );
			} else {
				$this->recipient_id = get_post_meta( $this->ID, '_mem_payment_to', true );
			}
		}

		return apply_filters( 'mem_get_recipient_id', $this->recipient_id );
	} // get_recipient_id
	...
	public function get_type() {
		...
		return apply_filters( 'mem_transaction_type', $return, $this->ID );
	} // get_type
	...
	public function get_method() {
		$method = get_post_meta( $this->ID, '_mem_txn_source', true );

		return apply_filters( 'mem_transaction_method', $method, $this->ID );
	} // get_method
	...
	public function get_gateway() {
		$gateway = get_post_meta( $this->ID, '_mem_txn_gateway', true );

		return apply_filters( 'mem_transaction_gateway', $gateway, $this->ID );
	} // get_gateway
