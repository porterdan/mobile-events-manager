 ...
 * @since 1.4
 */
class TMEM_Txns_Query extends TMEM_Stats {

	/**
	 * The args to pass to the tmem_get_transactions() query
	 *
	 * @var array
	 * @access public
	 * @since 1.4
	...
	public function __construct( $args = array() ) {
		$defaults = array(
			...
			'post_type'  => array( 'tmem-transaction' ),
			'start_date' => false,
			'end_date'   => false,
			'number'     => 20,
			'page'       => null,
			...
			'user'       => null,
			'client'     => null,
			'status'     => array( 'tmem-income', 'tmem-expenditure' ),
			'meta_key'   => null,
			'year'       => null,
			'month'      => null,
			'day'        => null,
	...
	public function init() {
		...
		add_action( 'tmem_pre_get_txns', array( $this, 'date_filter_pre' ) );
		add_action( 'tmem_post_get_txns', array( $this, 'date_filter_post' ) );

		add_action( 'tmem_pre_get_txns', array( $this, 'orderby' ) );
		add_action( 'tmem_pre_get_txns', array( $this, 'status' ) );
		add_action( 'tmem_pre_get_txns', array( $this, 'month' ) );
		add_action( 'tmem_pre_get_txns', array( $this, 'per_page' ) );
		add_action( 'tmem_pre_get_txns', array( $this, 'page' ) );
		add_action( 'tmem_pre_get_txns', array( $this, 'user' ) );
		add_action( 'tmem_pre_get_txns', array( $this, 'client' ) );
		add_action( 'tmem_pre_get_txns', array( $this, 'search' ) );
		add_action( 'tmem_pre_get_txns', array( $this, 'mode' ) );
		add_action( 'tmem_pre_get_txns', array( $this, 'children' ) );
		add_action( 'tmem_pre_get_txns', array( $this, 'event' ) );
	} // init
	...
	public function get_txns() {
		...
		do_action( 'tmem_pre_get_txns', $this );
		...
		if ( $query->have_posts() ) {
			while ( $query->have_posts() ) {
				...
				$this->txns[] = apply_filters( 'tmem_txn', $txn, $txn_id, $this );
			}

			wp_reset_postdata();
		}

		do_action( 'tmem_post_get_txns', $this );
	...
	public function orderby() {
		switch ( $this->args['orderby'] ) {
			case 'amount':
				...
				$this->__set( 'meta_key', '_tmem_txn_total' );
				break;
			default:
	...
	public function search() {
		...
		if ( $is_email ) {
			...
			if ( is_object( $post ) && 'tmem-transaction' === $post->post_type ) {
