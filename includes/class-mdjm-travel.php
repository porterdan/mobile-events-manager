...
class MEM_Travel {
	...
	private function init() {
		if ( ! isset( $this->start_address ) ) {
			$this->start_address = mem_get_option( 'travel_primary' );
		}

		$this->units           = mem_get_option( 'travel_units' );
		$this->add_travel_cost = mem_get_option( 'travel_add_cost' );
	} // init
	...
	public function __get( $key ) {
		if ( method_exists( $this, 'get_' . $key ) ) {
			...
			return new WP_Error( 'mem-travel-invalid-property', sprintf( esc_html__( "Can't get property %s", 'mobile-events-manager' ), $key ) );
		}
	} // __get
	...
	private function prepare_query() {
		 ...
		 * @since	1.4
		 */
		$query_args = apply_filters( 'mem_travel_query_args', $query_args, $this );
	...
	public function process_query() {
		...
		$travel = apply_filters( 'mem_travel_data', $travel, $travel_data );
	...
	public function get_travel_data() {
		...
		return apply_filters( 'mem_get_travel_data', $this->data, $this );
	} // get_travel_data
	...
	function get_cost() {
		...
		$min       = mem_get_option( 'travel_min_distance' );
		$unit_cost = mem_get_option( 'cost_per_unit' );
		$round     = mem_get_option( 'travel_cost_round' );

		if ( intval( $this->distance ) >= $min ) {
			$this->cost = $this->distance * $unit_cost;

			if ( $round ) {
				$nearest = mem_get_option( 'travel_round_to' );
		...
		return apply_filters( 'mem_travel_cost', $this->cost, $this );
	} // get_cost
	...
	public function get_venue_address( $id ) {
		...
		if ( 'mem-event' !== $post_type && 'mem-venue' !== $post_type ) {
			return false;
		}

		$venue_address = mem_get_event_venue_meta( $id, 'address' );

		$address = ! empty( $venue_address ) ? $venue_address : false;
		$address = apply_filters( 'mem_get_venue_address', $address, $id );
	...
	public function get_employee_address( $employee_id ) {
		...
		$employee_address = mem_get_employee_address( $employee_id );

		if ( is_array( $employee_address ) ) {
			$employee_address = implode( ',', array_filter( $employee_address ) );
		}

		// To filter this use apply_filters( 'mem_get_employee_address', $address, $user_id );.
	...
	public function get_client_address( $client_id ) {
		$client_address = mem_get_client_address( $client_id );

		if ( is_array( $client_address ) ) {
			$client_address = implode( ',', array_filter( $client_address ) );
		}

		// To filter this use apply_filters( 'mem_get_client_address', $address, $client_id );.
	...
	public function set_destination( $dest ) {
		if ( is_array( $dest ) ) { // Address array is passed.
			...
			$this->destination_address = $this->get_venue_address( $mem_event->get_venue_id() );
	...
	function get_directions_url() {
		...
		return apply_filters( 'mem_directions_url', $url, $this );
