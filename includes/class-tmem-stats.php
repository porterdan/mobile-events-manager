 ...
 * TMEM_Stats Class
 *
 * Base class for other TMEM Stat classes.
...
class TMEM_Stats {
	...
	public function get_predefined_dates() {
		...
		return apply_filters( 'tmem_stats_predefined_dates', $predefined );
	 ...
	 *
	 * This calls the convert_date() tmember function to ensure the dates are formatted correctly
	 *
	 * @access public
	 * @since 1.4
	 * @return void
	...
	public function convert_date( $date, $end_date = false ) {
		...
		if ( array_key_exists( $date, $this->get_predefined_dates() ) ) {
			...
			return new WP_Error( 'invalid_date', __( 'Improper date provided.', 'mobile-events-manager' ) );
		...
		return apply_filters( 'tmem_stats_date', $date, $end_date, $this );
	...
	public function get_events_by_date( $day = null, $month_num, $year = null, $hour = null ) {
		...
		$args   = apply_filters( 'tmem_get_events_by_date_args', $args );
		$key    = 'tmem_stats_' . substr( md5( serialize( $args ) ), 0, 15 );
	...
	public function get_total_income_by_date( $period = 'this_week' ) {
		return $this->get_txns_total_by_date( $period, 'tmem-income' );
	...
	public function get_total_outgoings_by_date( $period = 'this_week' ) {
		return $this->get_txns_total_by_date( $period, 'tmem-expenditure' );
	...
	public function get_txns_total_by_date( $period = 'this_week', $status = 'any' ) {
		...
		if ( $txns ) {
			...
			foreach ( $txns as $txn ) {
				...
				if ( 'any' === $args['post_status'] ) {
					...
					if ( 'tmem-income' === $txn->post_status ) {
						$total += tmem_sanitize_amount( get_post_meta( $txn->ID, '_tmem_txn_total', true ) );
					} else {
						$total -= tmem_sanitize_amount( get_post_meta( $txn->ID, '_tmem_txn_total', true ) );
					}
				} else {
		...
		return tmem_currency_filter( tmem_format_amount( $total ) );
	...
	public function get_income_by_date( $day = null, $month_num, $year = null, $hour = null ) {
		...
		$args = apply_filters( 'tmem_get_income_by_date_args', $args );
		...
		if ( $txns ) {
			...
			$income = $wpdb->get_var( "SELECT SUM(meta_value) FROM $wpdb->postmeta WHERE meta_key = '_tmem_txn_total' AND post_id IN ({$txns})" );
	...
	public function get_expenses_by_date( $day = null, $month_num, $year = null, $hour = null ) {
		...
		$args = apply_filters( 'tmem_get_expenses_by_date_args', $args );
		...
		if ( $txns ) {
			...
			$expense = $wpdb->get_var( "SELECT SUM(meta_value) FROM $wpdb->postmeta WHERE meta_key = '_tmem_txn_total' AND post_id IN ({$txns})" );
	...
	public function get_earnings_by_date( $day = null, $month_num, $year = null, $hour = null ) {
		...
		$args     = apply_filters( 'tmem_get_earnings_by_date_args', $args );
		$key      = 'tmem_stats_' . substr( md5( serialize( $args ) ), 0, 15 );
	...
	public function get_conversions( $event_ids = false, $tax = 'event-types', $term, $start_date = false, $end_date = false ) {
		$converted_statuses = apply_filters( 'tmem_converted_event_statuses', array( 'tmem-approved', 'tmem-contract', 'tmem-completed', 'tmem-cancelled' ) );
