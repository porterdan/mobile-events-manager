...
/**
 * MEM_Task_Runner Class
 *
 * @since 1.4.7
 */
class MEM_Task_Runner {
	...
	private function setup_task( $task ) {
		$this->all_tasks = get_option( 'mem_schedules' );
	...
	public function complete_task() {
		...
		return update_option( 'mem_schedules', $this->all_tasks );
	} // complete_task
	...
	public function upload_playlists() {
		MEM()->debug->log_it( "*** Starting the $this->name task ***", true );

		mem_process_playlist_upload();

		MEM()->debug->log_it( "*** $this->name task Completed ***", true );
	...
	public function complete_events() {
		...
		$i         = 1;
		$completed = 0;
		$events    = mem_get_events( $this->build_query() );

		if ( $events ) {
			remove_action( 'save_post_mem-event', 'mem_save_event_post', 10, 3 );

			$count = count( $events );
			MEM()->debug->log_it( $count . ' ' . _n( 'event', 'events', $events, 'mobile-events-manager' ) . ' to be marked as completed' );

			foreach ( $events as $_event ) {
				$event = new MEM_Event( $_event->ID );
				...
				$end_date    = get_post_meta( $event->ID, '_mem_event_end_date', true );
				...
				$update = mem_update_event_status( $event->ID, 'mem-completed', $event->post_status );
				if ( $update ) {

					if ( mem_get_option( 'employee_auto_pay_complete' ) ) {
						mem_pay_event_employees( $event->ID );
					}
					...
					MEM()->debug->log_it( 'Event ' . $event->ID . ' marked as completed' );
				} else {
					MEM()->debug->log_it( 'Event ' . $event->ID . ' could not be marked as completed' );
				}
			}
			add_action( 'save_post_mem-event', 'mem_save_event_post', 10, 3 );
		}

		MEM()->debug->log_it( "$completed event(s) marked as completed" );
		MEM()->debug->log_it( "*** $this->name task Completed ***", true );
	...
	public function fail_enquiry() {
		...
		if ( $events ) {
			...
			$count = count( $events );
			MEM()->debug->log_it( $count . ' ' . _n( 'enquiry', 'enquiries', $events, 'mobile-events-manager' ) . ' to be marked as failed' );
			...
			foreach ( $events as $_event ) {
				...
				$update = mem_update_event_status( $event->ID, 'mem-failed', $event->post_status );
				if ( $update ) {
					mem_add_journal(
						array(
							'user_id'         => 1,
					...
					MEM()->debug->log_it( 'Event ' . $event->ID . ' marked as failed' );
				} else {
					MEM()->debug->log_it( 'Event ' . $event->ID . ' could not be marked as failed' );
				}
			}
			add_action( 'save_post_mem-event', 'mem_save_event_post', 10, 3 );
		}
	...
	public function balance_reminder() {
		...
		if ( $events ) {
			$count = count( $events );
			MEM()->debug->log_it( $count . ' ' . _n( 'event', 'events', $events, 'mobile-events-manager' ) . ' due balance' );

			foreach ( $events as $_event ) {
				$event = new MEM_Event( $_event->ID );
				MEM()->debug->log_it( 'Event: ' . $event->ID );
				if ( ! $event ) {
					continue;
				}
				...
				$email_args = array(
					...
					'message'   => mem_get_email_template_content( $this->options['email_template'] ),
					'track'     => true,
					/* translators: %s: Company name */
					'source'    => sprintf( esc_html__( 'Request %s Scheduled Task', 'mobile-events-manager' ), mem_get_balance_label() ),
				);
				...
				if ( mem_send_email_content( $email_args ) ) {
					MEM()->debug->log_it( 'Balance reminder sent to ' . $client->display_name );
					...
					update_post_meta( $event->ID, '_mem_event_last_updated_by', 0 );
					$event->complete_task( $this->slug );
					...
					MEM()->debug->log_it( 'ERROR: Balance reminder was not sent. Event ID ' . $event->ID );
				}
			}
		} else {
			MEM()->debug->log_it( 'No events requiring balance reminders' );
		}
	...
	public function request_deposit() {
		...
		if ( $events ) {
			$count = count( $events );
			MEM()->debug->log_it( $count . ' ' . _n( 'event', 'events', $events, 'mobile-events-manager' ) . ' due deposit' );
			...
			foreach ( $events as $_event ) {
				...
				$email_args = array(
					...
					'track'     => true,
					/* translators: %s: Company name */
					'source'    => sprintf( esc_html__( 'Request %s Scheduled Task', 'mobile-events-manager' ), mem_get_deposit_label() ),
				);
				...
				if ( mem_send_email_content( $email_args ) ) {
					MEM()->debug->log_it( 'Deposit request sent to ' . $client->display_name );
					...
					MEM()->debug->log_it( 'ERROR: Deposit request was not sent. Event ID ' . $event->ID );
				}
			}
		} else {
			MEM()->debug->log_it( 'No events requiring deposit requests' );
		}
	...
	public function playlist_notification() {
		...
		if ( $events ) {
			$count = count( $events );
			MEM()->debug->log_it( $count . ' ' . _n( 'event', 'events', $events, 'mobile-events-manager' ) . ' have new playlist entries within the past 24 hours' );
			...
			foreach ( $events as $_event ) {
				...
				$entries = mem_get_playlist_entries(
					$event->ID,
					array(
						'posts_per_page' => 1,
						'date_query'     => array(
				...
				if ( mem_send_email_content( $email_args ) ) {
					MEM()->debug->log_it( $this->name . ' sent to ' . $client->display_name );

					delete_post_meta( $event->ID, '_mem_playlist_client_notify' );
					...
					MEM()->debug->log_it( 'ERROR: Playlist notification was not sent. Event ID ' . $event->ID );
				}
			}
		} else {
			MEM()->debug->log_it( 'No events have new playlist entries within the past 24 hours' );
		}
	...
	public function playlist_employee_notify() {
		...
		if ( $events ) {
			$count = count( $events );
			MEM()->debug->log_it( $count . ' ' . _n( 'event', 'events', $events, 'mobile-events-manager' ) . " are scheduled within the next {$this->options['age']}" );
			...
			foreach ( $events as $_event ) {
				...
				if ( mem_send_email_content( $email_args ) ) {
					MEM()->debug->log_it( $this->name . ' sent to ' . $employee->display_name );
			...
			MEM()->debug->log_it( "No events are scheduled within the next {$this->options['age']}" );
		}
	...
	public function build_query() {
		if ( 'after_approval' !== $this->options['run_when'] ) {
			...
			$date_query = array(
				'key'     => '_mem_event_task_after_approval_' . $this->slug,
				'compare' => '<',
				'value'   => $run_date,
				'type'    => 'datetime',
			);
		...
		switch ( $this->slug ) {
			...
			case 'fail-enquiry':
				...
				$query = array(
					'post_status' => array( 'mem-unattended', 'mem-enquiry' ),
					'date_query'  => array(
						'before' => $expired,
					),
				);
			...
			case 'balance-reminder':
				$query = array(
					'post_status' => array( 'mem-approved', 'mem-awaitingdeposit' ),
					'meta_query'  => array(
						'relation' => 'AND',
						array(
							'key'     => '_mem_event_date',
			...
			case 'request-deposit':
				$query = array(
					...
					'meta_query'  => array(
						...
						array(
							'key'   => '_mem_event_deposit_status',
							'value' => 'Due',
						),
						array(
							'key'     => '_mem_event_deposit',
							'value'   => '0.00',
							'compare' => '>',
						),
					),
			...
			case 'playlist-notification':
				$query = array(
					'meta_key'   => '_mem_playlist_client_notify',
					'meta_value' => '1',
				);
				break;
		...
		return apply_filters( 'mem_task_query', $query, $this->slug, $this );
	} // build_query
