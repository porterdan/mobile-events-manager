...
/**
 * TMEM_HTML_Elements Class
 *
 * @since 1.3.7
 */
class TMEM_HTML_Elements {
	...
	public function event_status_dropdown( $name = 'post_status', $selected = 0 ) {
		$event_statuses = tmem_get_post_statuses( 'labels' );
		$options        = array();
		...
		$output = $this->select(
			array(
				...
				'data'             => array(
					...
					'search-placeholder' => sprintf( esc_html__( 'Type to search all %s statuses', 'mobile-events-manager' ), esc_html( tmem_get_label_singular( true ) ) ),
				),
			)
		);
	...
	public function event_type_dropdown( $args = array() ) {
		...
		$defaults = array(
			...
			'class'            => '',
			'id'               => '',
			'selected'         => tmem_get_option( 'event_type_default' ),
			'chosen'           => false,
			'placeholder'      => null,
			'multiple'         => false,
			'show_option_all'  => false,
			...
			'data'             => array(
				'search-type'        => 'type',
				/* translators: %s: different word for event */
				'search-placeholder' => sprintf( esc_html__( 'Type to search all %s types', 'mobile-events-manager' ), esc_html( tmem_get_label_singular( true ) ) ),
			),
		);
		...
		$categories      = get_terms( 'event-types', apply_filters( 'tmem_event_types_dropdown', $cat_args ) );
		$args['options'] = array();
		...
		$category_labels = tmem_get_taxonomy_labels( 'event-types' );
	 ...
	 * @return str $output Category dropdown
	 */
	public function enquiry_source_dropdown( $name = 'tmem_enquiry_source', $selected = 0 ) {

		$args = array(
			'hide_empty' => false,
		);

		$categories = get_terms( 'enquiry-source', apply_filters( 'tmem_enquiry_source_dropdown', $args ) );
		$options    = array();

		if ( empty( $selected ) ) {
			$selected = tmem_get_option( 'enquiry_source_default' );
		}
		...
		$category_labels = tmem_get_taxonomy_labels( 'enquiry-source' );
		$output          = $this->select(
			array(
				'name'             => $name,
				'selected'         => $selected,
	 ...
	 * @return str $output Category dropdown
	 */
	public function txn_type_dropdown( $name = 'tmem_txn_for', $selected = 0 ) {

		$args = array(
			'hide_empty' => false,
		);

		$categories = get_terms( 'transaction-types', apply_filters( 'tmem_txn_types_dropdown', $args ) );
		$options    = array();
		...
		$category_labels = tmem_get_taxonomy_labels( 'transaction-types' );
		$output          = $this->select(
			array(
				'name'             => $name,
				'selected'         => $selected,
				'options'          => $options,
				'show_option_all'  => false,
				'show_option_none' => __( ' - Select Txn Type - ', 'mobile-events-manager' ),
				'data'             => array(
					'search-type'        => 'txn_type',
					'search-placeholder' => __( 'Type to search all transaction statuses', 'mobile-events-manager' ),
				),
			)
		);
	...
	public function venue_dropdown( $args = array() ) {
		...
		$defaults = array(
			...
			'data'            => array(
				'search-type'        => 'venue',
				'search-placeholder' => __( 'Type to search all venues', 'mobile-events-manager' ),
			),
		);
		...
		if ( $args['allow_add'] ) {
			$args['options'] = array(
				'manual' => __( 'Enter Manually', 'mobile-events-manager' ),
				'client' => __( 'Use Client Address', 'mobile-events-manager' ),
			);
		}
	...
	public function month_dropdown( $args ) {
		...
		$defaults = array(
			...
			'multiple'    => false,
			'chosen'      => false,
			'placeholder' => __( 'Select a Month', 'mobile-events-manager' ),
		);
		...
		while ( $month <= 12 ) {
			$options[ absint( $month ) ] = tmem_month_num_to_name( $month, $args['fullname'] );
			$month++;
		}
	...
	public function employee_dropdown( $args = array() ) {
		...
		$defaults = array(
			...
			'data'             => array(
				...
				'search-placeholder' => __( 'Type to search all employees', 'mobile-events-manager' ),
			),
		);
		...
		$output = $this->select(
			array(
				...
				'placeholder'      => $args['placeholder'],
				'multiple'         => $args['multiple'],
				'show_option_none' => $employees ? $args['show_option_none'] : __( 'No Employees', 'mobile-events-manager' ),
	...
	public function roles_dropdown( $args = array() ) {
		...
		$defaults = array(
			'name'        => 'tmem_employee_roles',
			'value'       => '',
			'options'     => tmem_get_roles(),
			'blank_first' => true,
			'placeholder' => __( 'Select Role', 'mobile-events-manager' ),
			'chosen'      => false,
			'data'        => array(
				'search-type'        => 'roles',
				'search-placeholder' => __( 'Type to search all roles', 'mobile-events-manager' ),
			),
		);

		$args            = wp_parse_args( $args, $defaults );
		$args['options'] = tmem_get_roles();
	...
	public function client_dropdown( $args = array() ) {
		...
		$defaults = array(
			...
			'null_value'       => false,
			'add_new'          => false,
			'show_option_all'  => _x( 'All Clients', 'all dropdown items', 'mobile-events-manager' ),
			'show_option_none' => _x( 'Select a Client', 'no dropdown items', 'mobile-events-manager' ),
			'data'             => array(
				'search-type'        => 'client',
				'search-placeholder' => __( 'Type to search all clients', 'mobile-events-manager' ),
			),
		);
		...
		$clients = tmem_get_clients( $args['roles'] );
		...
		if ( ! empty( $args['add_new'] ) ) {
			$options['tmem_add_client'] = __( 'Add New Client', 'mobile-events-manager' );
		}
	...
	public function users_dropdown( $args = array() ) {
		...
		$defaults = array(
			...
			'placeholder'      => null,
			'multiple'         => false,
			'show_option_all'  => _x( 'All Users', 'all dropdown items', 'mobile-events-manager' ),
			'show_option_none' => _x( 'Select a User', 'no dropdown items', 'mobile-events-manager' ),
			'data'             => array(
				'search-type'        => 'user',
				'search-placeholder' => __( 'Type to search all users', 'mobile-events-manager' ),
			),
		);
	...
	public function packages_dropdown( $args = array() ) {
		$defaults = array(
			...
			'class'            => '',
			'selected'         => '',
			'show_option_none' => __( 'No Package', 'mobile-events-manager' ),
			'show_option_all'  => false,
			'chosen'           => false,
			'employee'         => false,
			'event_type'       => false,
			'event_date'       => false,
			'exclude_cats'     => null,
			'placeholder'      => __( 'Select a Package', 'mobile-events-manager' ),
			'multiple'         => false,
			'cost'             => true,
			'titles'           => false,
			'atts'             => array(),
			...
			'data'             => array(
				'search-type'        => 'package',
				'search-placeholder' => __( 'Type to search all packages', 'mobile-events-manager' ),
			),
		);
		...
		if ( $packages ) {
			...
			foreach ( $packages as $package ) {
				...
				if ( $args['employee'] ) {
					if ( ! tmem_employee_has_package( $package->ID, $args['employee'] ) ) {
						continue;
					}
				}

				if ( $args['event_type'] ) {
					if ( ! tmem_package_is_available_for_event_type( $package->ID, $args['event_type'] ) ) {
						continue;
					}
				}

				if ( $args['event_date'] ) {
					if ( ! tmem_package_is_available_for_event_date( $package->ID, $args['event_date'] ) ) {
						continue;
					}
				} else {
				...
				$price         = '';
				$package_price = tmem_get_package_price( $package->ID, $args['event_date'] );
				if ( true === $args['cost'] ) {
					$price .= ' - ' . tmem_currency_filter( tmem_format_amount( $package_price ) );
				}

				$args['options'][ $package->ID ] = $package->post_title . '' . $price;

				$args['atts'][ $package->ID ] = array( 'price' => tmem_sanitize_amount( $package_price ) );

				if ( $args['titles'] ) {
					$titles[ $package->ID ] = tmem_get_package_excerpt( $package->ID );
				}
			}
		}

		if ( empty( $args['options'] ) ) {
			$args['placeholder'] = __( 'No Packages Available', 'mobile-events-manager' );
		}
	...
	public function addons_dropdown( $args = array() ) {
		$defaults = array(
			...
			'class'            => '',
			'selected'         => '',
			'show_option_none' => __( 'No Addons', 'mobile-events-manager' ),
			'show_option_all'  => false,
			'chosen'           => false,
			'employee'         => false,
			'event_type'       => false,
			...
			'data'             => array(
				'search-type'        => 'addons',
				'search-placeholder' => __( 'Type to search all addons', 'mobile-events-manager' ),
			),
		);
		...
		$addons = tmem_get_addons( $addons_args );
		...
		if ( $addons ) {
			foreach ( $addons as $addon ) {
				...
				$price       = '';
				$addon_price = tmem_get_addon_price( $addon->ID, $args['event_date'] );
				if ( true === $args['cost'] ) {
					$price .= ' - ' . tmem_currency_filter( tmem_format_amount( $addon_price ) );
				}
				$desc = '';

				if ( $args['desc'] ) {
					$desc .= ' - ' . tmem_get_addon_excerpt( $addon->ID, $args['desc'] );
				}
				...
				$args['atts'][ $addon->ID ] = array( 'price' => tmem_sanitize_amount( $addon_price ) );

				if ( $args['titles'] ) {
					$titles[ $addon->ID ] = tmem_get_addon_excerpt( $addon->ID );
				}
			}
		}
	...
	public function time_hour_select( $args = array() ) {
		...
		if ( 'H:i' === tmem_get_option( 'time_format', 'H:i' ) ) {
			$i      = '00';
			$x      = '23';
			$format = 'H';
		} else {
	...
	public function time_minute_select( $args = array() ) {
		$options  = array();
		$minutes  = apply_filters( 'tmem_time_minutes', array( '00', '15', '30', '45' ) );
		$defaults = array(
	...
	public function time_period_select( $args = array() ) {
		...
		$options = array(
			'AM' => __( 'AM', 'mobile-events-manager' ),
			'PM' => __( 'PM', 'mobile-events-manager' ),
		);
	...
	public function select( $args = array() ) {
		...
		if ( ! $args['options_only'] ) {
			$output .= '<select name="' . esc_attr( $args['name'] ) . '' . $multi . '" id="' . esc_attr( tmem_sanitize_key( str_replace( '-', '_', $args['id'] ) ) ) . '" class="tmem-select ' . $class . '"' . $multiple . ' data-placeholder="' . $placeholder . '"' . $data_elements . '>' . "\r\n";
		}
	...
	public function text( $args = array() ) {
		...
		if ( ! empty( $args['data'] ) ) {
			foreach ( $args['data'] as $key => $value ) {
				$data .= 'data-' . tmem_sanitize_key( $key ) . '="' . esc_attr( $value ) . '" ';
			}
		}

		$output = '<span id="tmem-' . tmem_sanitize_key( $args['name'] ) . '-wrap">';

		$output .= '<label for="' . tmem_sanitize_key( $args['id'] ) . '">' . esc_html( $args['label'] ) . '</label>';
	...
	public function date_field( $args = array() ) {
		...
		if ( empty( $args['class'] ) ) {
			$args['class'] = 'tmem_datepicker';
		} elseif ( ! strpos( $args['class'], 'tmem_datepicker' ) ) {
			$args['class'] .= ' tmem_datepicker';
		}
	...
	public function textarea( $args = array() ) {
		...
		$output = '<span id="tmem-' . tmem_sanitize_key( $args['name'] ) . '-wrap">';
			...
			$output .= '<label for="' . tmem_sanitize_key( $args['name'] ) . '">' . esc_html( $args['label'] ) . '</label>';

			$output .= '<textarea name="' . esc_attr( $args['name'] ) . '" id="' . tmem_sanitize_key( $args['name'] ) . '" class="' . $class . '"' . $disabled . $placeholder . '>' . esc_attr( $args['value'] ) . '</textarea>';

		if ( ! empty( $args['desc'] ) ) {
			$output .= '<span class="tmem-description">' . esc_html( $args['desc'] ) . '</span>';
		}
	...
	public function ajax_user_search( $args = array() ) {
		...
		$defaults = array(
			'name'         => 'user_id',
			'value'        => null,
			'placeholder'  => __( 'Enter username', 'mobile-events-manager' ),
			'label'        => null,
			'desc'         => null,
			'class'        => '',
			'disabled'     => false,
		...
		$args['class'] = 'tmem-ajax-user-search ' . $args['class'];

		$output      = '<span class="tmem_user_search_wrap">';
			$output .= $this->text( $args );
			$output .= '<span class="tmem_user_search_results hidden"><a class="tmem-ajax-user-cancel" title="' . __( 'Cancel', 'mobile-events-manager' ) . '" aria-label="' . __( 'Cancel', 'mobile-events-manager' ) . '" href="#">x</a><span></span></span>';
		$output     .= '</span>';
