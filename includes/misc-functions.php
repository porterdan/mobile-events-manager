 ...
 * @return
 */
function tmem_format_datepicker_date() {
	$date_format = tmem_get_option( 'short_date_format', 'd/m/Y' );
	...
	return apply_filters( 'tmem_format_datepicker_date', $date_format );
} // tmem_format_datepicker_date

/**
 * Add the TMEM MCE Shortcode button.
 *
 * @since 1.3
 * @param
 * @return
 */
function tmem_display_shortcode_button() {
	...
	$screens = array(
		'tmem-event_page_tmem-comms',
		'tmem-event_page_tmem-settings',
	);

	// Add the TMEM TinyMCE buttons.
	$current_screen = get_current_screen();

	if ( in_array( get_post_type(), $post_types ) || in_array( $current_screen->id, $screens ) ) {

		if ( 'true' === get_user_option( 'rich_editing' ) ) {
			add_filter( 'mce_external_plugins', 'tmem_register_mce_plugin' );
			add_filter( 'mce_buttons', 'tmem_register_mce_buttons' );
		}
	}

} // tmem_display_shortcode_button
add_action( 'admin_head', 'tmem_display_shortcode_button' );
 ...
 */
function tmem_get_file_extension( $str ) {
	$parts = explode( '.', $str );
	return end( $parts );
} // tmem_get_file_extension
 ...
 */
function tmem_object_to_array( $object = array() ) {
	...
	if ( is_array( $object ) ) {
		...
		foreach ( $object as $item ) {
			if ( is_a( $object, 'TMEM_Event' ) ) {
				$return[] = $object->array_convert();
			} else {
				$return[] = tmem_object_to_array( $item );
			}
		}
	} else {
		if ( is_a( $object, 'TMEM_Event' ) ) {
			...
			foreach ( $return as $key => $value ) {
				$value          = ( is_array( $value ) || is_object( $value ) ) ? tmem_object_to_array( $value ) : $value;
				$return[ $key ] = $value;
			}
		}
	}
...
} // tmem_object_to_array

/**
 * Register the script that inserts ths TMEM Shortcodes into the content
 * when the TMEM Shortcode button is used
 *
 * @since 1.3
 * @param arr $plugin_array Array of registered MCE plugins.
 * @return arr $plugin_array Filtered array of registered MCE plugins
 */
function tmem_register_mce_plugin( $plugin_array ) {

	$plugin_array['tmem_shortcodes_btn'] = TMEM_PLUGIN_URL . '/assets/js/tmem-tinymce-shortcodes.js';

	return $plugin_array;

} // tmem_register_mce_plugin

/**
 * Register the TMEM Shortcode button within the TinyMCE interface
 *
 * @since   1.3
 * @params  arr     $buttons    Array of registered MCE buttons
 * @return  arr     $buttons    Filtered array of registered MCE buttons
 */
function tmem_register_mce_buttons( $buttons ) {

	array_push( $buttons, 'tmem_shortcodes_btn' );

	return $buttons;
} // tmem_register_mce_buttons
 ...
 * @return void
 */
function tmem_insert_datepicker( $args = array() ) {
	$defaults = array(
		'class'       => 'tmem_date',
	...
	jQuery(document).ready( function($)	{
		$("<?php echo $field; ?>").datepicker({
			dateFormat : "<?php echo tmem_format_datepicker_date(); ?>",
 ...
 */
function tmem_month_num_to_name( $n, $full = false ) {
	$timestamp = mktime( 0, 0, 0, $n, 1, 2005 );

	$output = $full ? 'F' : 'M';

	return date_i18n( $output, $timestamp );
} // tmem_month_num_to_name
 ...
 */
function tmem_generate_random_string( $prefix = '', $suffix = '', $length = 6, $lower = true ) {
 ...
 */
function tmem_get_php_arg_separator_output() {
	return ini_get( 'arg_separator.output' );
} // tmem_get_php_arg_separator_output
 ...
 */
function tmem_is_func_disabled( $function ) {
	$disabled = explode( ',', ini_get( 'disable_functions' ) );

	return in_array( $function, $disabled );
} // tmem_is_func_disabled
 ...
 */
function tmem_get_current_page_url() {
	...
	$uri = apply_filters( 'tmem_get_current_page_url', $uri );

	return $uri;
} // tmem_get_current_page_url
...
function tmem_get_user_ip() {
	...
	return apply_filters( 'tmem_get_user_ip', $ip_address );
} // tmem_get_user_ip
 ...
 */
function tmem_display_notice( $m ) {
	$message = tmem_messages( $m );

	$notice  = '<div class="tmem-alert tmem-alert-' . $message['class'] . '">';
	$notice .= ! empty( $message['title'] ) ? '<span><strong>' . $message['title'] . '</strong></span><br />' : '';
	$notice .= $message['message'] . '</div>';

	return apply_filters( 'tmem_display_notice', $notice, $m );
} // tmem_display_notice

/**
 * Display notice on front end.
 *
 * Check for super global $_GET['tmem-message'] key and return message if set.
 *
 * @since 1.3
 * @param
 * @return str Out the relevant message to the browser.
 */
function tmem_print_notices() {
	if ( ! isset( $_GET['tmem_message'] ) ) {
		return;
	}
	...
	if ( isset( $_GET['event_id'] ) ) {
		...
		echo tmem_do_content_tags( tmem_display_notice( sanitize_text_field( wp_unslash( $_GET['tmem_message'] ) ) ), $tmem_event->ID, $tmem_event->client );

	} else {
		echo tmem_display_notice( sanitize_text_field( wp_unslash( $_GET['tmem_message'] ) ) );
	}
} // tmem_print_notices
add_action( 'tmem_print_notices', 'tmem_print_notices' );
	...
	$messages = apply_filters(
		...
		array(
			...
			'missing_event'               => array(
				'class'   => 'error',
				'title'   => __( 'Error', 'mobile-events-manager' ),
				'message' => sprintf( esc_html__( 'We could not locate the details of your %s.', 'mobile-events-manager' ), esc_html( tmem_get_label_singular( true ) ) ),
			),
			...
			'contract_signed'             => array(
				...
				'message' => sprintf( esc_html__( 'You have successfully signed your %s contract. Confirmation will be sent to you via email in the next few minutes.', 'mobile-events-manager' ), esc_html( tmem_get_label_singular( true ) ) ),
			),
			'contract_not_signed'         => array(
				'class'   => 'error',
				'title'   => __( 'Error', 'mobile-events-manager' ),
				'message' => sprintf( esc_html__( 'Unable to sign %s contract.', 'mobile-events-manager' ), esc_html( tmem_get_label_singular( true ) ) ),
			),
			...
			'available'                   => array(
				'class'   => 'tmem_available',
				'message' => tmem_get_option( 'availability_check_pass_text', __( 'The date you selected is available.', 'mobile-events-manager' ) ),
			),
			'not_available'               => array(
				'class'   => 'tmem_notavailable',
				'message' => tmem_get_option( 'availability_check_fail_text', __( "We're not available on the selected date.", 'mobile-events-manager' ) ),
			),
			'missing_date'                => array(
				'class'   => 'error',
			...
			'missing_event'               => array(
				'class'   => 'error',
				'title'   => __( 'Sorry', 'mobile-events-manager' ),
				'message' => sprintf( esc_html__( 'We seem to be missing the %s details.', 'mobile-events-manager' ), esc_html( tmem_get_label_singular( true ) ) ),
			),
 ...
 */
function tmem_has_upgrade_completed( $upgrade_action = '' ) {
 ...
 */
function tmem_get_completed_upgrades() {

	$completed_upgrades = get_option( 'tmem_completed_upgrades', array() );

	return $completed_upgrades;

} // tmem_get_completed_upgrades
