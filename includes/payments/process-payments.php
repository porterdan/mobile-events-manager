 ...
 * @return void
 */
function tmem_process_payment_form() {

	do_action( 'tmem_pre_process_payment' );

	// Validate the form $_POST data.
	$valid_data = tmem_payment_form_validate_fields();

	// Allow themes and plugins to hook to errors.
	do_action( 'tmem_payment_error_checks', $valid_data, $_POST );
	...
	do_action( 'tmem_payment_before_gateway', $_POST, $payment_data );

	// Allow the payment data to be modified before a transaction is created.
	$payment_data = apply_filters(
		'tmem_payment_data_before_txn',
		$payment_data,
		$valid_data
	);

	$txn_id = tmem_create_payment_txn( $payment_data );
	...
	$payment_data = apply_filters(
		'tmem_payment_data_before_gateway',
		$payment_data,
		$valid_data
	);

	// Send info to the gateway for payment processing.
	tmem_send_to_gateway( $payment_data['gateway'], $payment_data );
	die();
} // tmem_process_payment_form
add_action( 'tmem_event_payment', 'tmem_process_payment_form' );
add_action( 'wp_ajax_tmem_event_payment', 'tmem_process_payment_form' );
add_action( 'wp_ajax_nopriv_tmem_event_payment', 'tmem_process_payment_form' );
 ...
 * @return bool|arr
 */
function tmem_payment_form_validate_fields() {
	...
	$client_data = tmem_get_payment_form_client( tmem_get_event_client_id( $event_id ) );
	...
	$valid_data = array(
		...
		'client_ip'   => esc_html( tmem_get_user_ip() ),
		'type'        => esc_html( tmem_get_payment_type() ),
		'total'       => esc_html( tmem_get_payment_total() ),
		'gateway'     => esc_html( tmem_payment_form_validate_gateway() ),
		'cc_info'     => esc_html( tmem_get_purchase_cc_info() ),
	);

	// Return collected data.
	return $valid_data;
} // tmem_payment_form_validate_fields

function tmem_get_payment_form_client( $client_id = 0 ) {
	if ( empty( $client_id ) ) {
		return false;
	}
	...
	$client_data = apply_filters( 'tmem_get_payment_form_client', $client_data, $client_id );

	return $client_data;

} // tmem_get_payment_form_client
 ...
 * @return string
 */
function tmem_payment_form_validate_gateway() {
	$gateway = tmem_get_default_gateway();

	// Check if a gateway value is present.
	if ( ! empty( $_REQUEST['tmem_gateway'] ) ) {
		$gateway = sanitize_text_field( wp_unslash( $_REQUEST['tmem_gateway'] ) );
	}

	return $gateway;
} // tmem_payment_form_validate_gateway
 ...
 * @return str
 */
function tmem_get_payment_type() {
	$type = sanitize_text_field( wp_unslash( $_POST['tmem_payment_amount'] ) );

	if ( 'deposit' === $type ) {
		$type = tmem_get_deposit_label();
	} elseif ( 'balance' === $type ) {
		$type = tmem_get_balance_label();
	} else {
		$type = tmem_get_other_amount_label();
	}

	return $type;

} // tmem_get_payment_type
 ...
 * @return str
 */
function tmem_get_payment_total() {
	$event_id = sanitize_text_field( wp_unslash( $_POST['event_id'] ) );
	$type     = sanitize_text_field( wp_unslash( $_POST['tmem_payment_amount'] ) );
	$total    = false;
	...
	if ( 'deposit' === $type ) {
		$type  = tmem_get_deposit_label();
		$total = $tmem_event->get_remaining_deposit();
	} elseif ( 'balance' === $type ) {
		$type  = tmem_get_balance_label();
		$total = $tmem_event->get_balance();
	} else {
		$type  = tmem_get_other_amount_label();
 ...
 * @return arr
 */
function tmem_get_purchase_cc_info() {
	$cc_info                   = array();
