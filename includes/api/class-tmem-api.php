...
class TMEM_API {
	...
	public function __construct() {
		$this->namespace = 'tmem/v' . self::VERSION;

		add_action( 'rest_api_init', array( $this, 'register_endpoints' ) );
		add_action( 'tmem-process_api_key', array( $this, 'process_api_key' ) );
		add_action( '/tmem/v1/availability', array( $this, 'availability_check' ) );
		add_action( '/tmem/v1/client', array( $this, 'get_client' ) );
		add_action( '/tmem/v1/employee', array( $this, 'get_employee' ) );
		add_action( '/tmem/v1/event', array( $this, 'get_event' ) );
		add_action( '/tmem/v1/events', array( $this, 'list_events' ) );
		add_action( '/tmem/v1/package', array( $this, 'get_package' ) );
		add_action( '/tmem/v1/packages', array( $this, 'list_packages' ) );
		add_action( '/tmem/v1/packages/options', array( $this, 'package_options' ) );
		add_action( '/tmem/v1/addon', array( $this, 'get_addon' ) );
		add_action( '/tmem/v1/addons', array( $this, 'list_addons' ) );
		add_action( '/tmem/v1/addons/options', array( $this, 'addon_options' ) );
	} // __construct
	...
	public function define_endpoints() {
		...
		return apply_filters( 'tmem_api_endpoints', $endpoints );
	...
	public function get_user() {
		...
		$user = get_transient( md5( 'tmem_api_user_' . $key ) );

		if ( false === $user ) {
			$user = $wpdb->get_var( $wpdb->prepare( "SELECT user_id FROM $wpdb->usermeta WHERE meta_key = %s LIMIT 1", $key ) );

			set_transient( md5( 'tmem_api_user_' . $key ), $user, DAY_IN_SECONDS );
		}
	...
	public function get_user_public_key( $user_id = 0 ) {
		...
		$cache_key       = md5( 'tmem_api_user_public_key' . $user_id );
		$user_public_key = get_transient( $cache_key );

		if ( empty( $user_public_key ) ) {

			$user_public_key = $wpdb->get_var( $wpdb->prepare( "SELECT meta_key FROM $wpdb->usermeta WHERE meta_value = 'tmem_user_public_key' AND user_id = %d", $user_id ) );
	...
	public function get_user_secret_key( $user_id = 0 ) {
		...
		$cache_key       = md5( 'tmem_api_user_secret_key' . $user_id );
		$user_secret_key = get_transient( $cache_key );

		if ( empty( $user_secret_key ) ) {

			$user_secret_key = $wpdb->get_var( $wpdb->prepare( "SELECT meta_key FROM $wpdb->usermeta WHERE meta_value = 'tmem_user_secret_key' AND user_id = %d", $user_id ) );
	...
	public function process_api_key( $args ) {
		...
		if ( ! isset( $_REQUEST['api_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['api_nonce'], 'tmem-api-nonce' ) ) ) ) {
		...
		$process = isset( $args['tmem_api_process'] ) ? strtolower( $args['tmem_api_process'] ) : false;
		...
		switch ( $process ) {
			...
			case 'generate':
				if ( $this->generate_api_key( $user_id ) ) {
					...
					delete_transient( 'tmem-total-api-keys' );

					wp_safe_redirect(
						add_query_arg(
							array( 'tmem-message' => 'api-key-generated' ),
							'edit.php?post_type=tmem-event&page=tmem-tools&tab=api_keys'
						)
					);
					exit();
					...
					wp_safe_redirect(
						add_query_arg(
							array( 'tmem-message' => 'api-key-failed' ),
							'edit.php?post_type=tmem-event&page=tmem-tools&tab=api_keys'
						)
					);
					exit();
			...
			case 'regenerate':
				...
				wp_safe_redirect(
					add_query_arg(
						array( 'tmem-message' => 'api-key-regenerated' ),
						'edit.php?post_type=tmem-event&page=tmem-tools&tab=api_keys'
					)
				);
				exit();
			...
			case 'revoke':
				...
				wp_safe_redirect(
					add_query_arg(
						array( 'tmem-message' => 'api-key-revoked' ),
						'edit.php?post_type=tmem-event&page=tmem-tools&tab=api_keys'
					)
				);
				exit();
	...
	public function revoke_api_key( $user_id = 0 ) {
		...
		if ( ! empty( $public_key ) ) {
			...
			delete_transient( md5( 'tmem_api_user_' . $public_key ) );
			delete_transient( md5( 'tmem_api_user_public_key' . $user_id ) );
			delete_transient( md5( 'tmem_api_user_secret_key' . $user_id ) );
	...
	public function update_key( $user_id ) {
		TMEM()->users->update_user_api_key( $user_id );
	} // update_key
	...
	public function availability_check() {
		...
		if ( ! isset( $this->request['date'] ) ) {
			...
			do_action( 'tmem_before_api_availability_check', $this );
			...
			$available_text   = ! empty( $this->request['avail_text'] ) ? $this->request['avail_text'] : tmem_get_option( 'availability_check_pass_text' );
			$unavailable_text = ! empty( $this->request['unavail_text'] ) ? $this->request['unavail_text'] : tmem_get_option( 'availability_check_fail_text' );
			$search           = array( '{EVENT_DATE}', '{EVENT_DATE_SHORT}' );
			$replace          = array( gmdate( 'l, jS F Y', strtotime( $date ) ), tmem_format_short_date( $date ) );
		...
		if ( ! empty( $result ) && ! empty( $result['available'] ) ) {
			...
			$response['availability'] = array(
				...
				'response'  => 'available',
				'employees' => $result,
				'message'   => tmem_do_content_tags( $message ),
			);
		} else {
		...
		do_action( 'tmem_after_api_availability_check', $this );
	...
	public function get_client() {
		...
		do_action( 'tmem_before_api_get_client', $this );
		...
		$events        = array();
		$client_events = tmem_get_client_events( $client->ID );
		$next_event    = tmem_get_clients_next_event( $client->ID );

		if ( $client_events ) {
			foreach ( $client_events as $event ) {
				$events[ $event->ID ] = get_post_meta( $event->ID, '_tmem_event_date', true );
			}
		}
		...
		$response['client'] = array(
			...
			'next_event' => array(
				...
				'date' => ! empty( $next_event ) ? get_post_meta( $next_event[0]->ID, '_tmem_event_date', true ) : '',
			),
		);

		$this->data = array_merge( $this->data, $response );

		do_action( 'tmem_after_api_get_client', $this );
	...
	public function get_employee() {
		...
		do_action( 'tmem_before_api_get_employee', $this );
		...
		if ( ! tmem_is_employee( $employee->ID ) ) {
		...
		$events          = array();
		$roles           = array();
		$tmem_roles      = TMEM()->roles->get_roles();
		$employee_events = tmem_get_employee_events( $employee->ID );
		$next_event      = tmem_get_employees_next_event( $employee->ID );
		$i               = 0;
		...
		if ( ! empty( $employee->roles ) ) {
			...
			foreach ( $employee->roles as $role ) {
				if ( array_key_exists( $role, $tmem_roles ) ) {
					$roles[ $role ] = $tmem_roles[ $role ];
				}
			}
		}
		...
		$response['employee'] = array(
			...
			'next_event'   => array(
				...
				'date' => ! empty( $next_event ) ? get_post_meta( $next_event->ID, '_tmem_event_date', true ) : '',
			),
			'total_events' => $i,
		);
		...
		do_action( 'tmem_after_api_get_employee', $this );
	...
	public function get_event() {
		...
		if ( ! tmem_employee_can( 'read_events', $this->user_id ) ) {
			$this->no_permsission();
		}

		do_action( 'tmem_before_api_get_event', $this );

		$tmem_event = tmem_get_event( $this->request['event_id'] );

		if ( ! $tmem_event ) {
			$error = array();
			/* translators: %s: Event Type */
			$error['error'] = sprintf( esc_html__( '%s does not exist.', 'mobile-events-manager' ), esc_html( tmem_get_label_singular() ) );

			$this->data = $error;
			$this->output();
		}

		$response['event'] = tmem_get_event_data( $tmem_event );

		$response['event'] = array_merge( array( 'id' => $tmem_event->ID ), $response['event'] );

		$this->data = array_merge( $this->data, $response );

		do_action( 'tmem_after_api_get_event', $this );
	...
	public function list_events() {
		...
		if ( ! isset( $this->request['employee_id'] ) && ! tmem_employee_can( 'read_events_all', $this->user_id ) ) {
			$this->no_permsission();
		}

		do_action( 'tmem_before_api_event_list', $this );

		if ( isset( $this->request['employee_id'] ) ) {
			$events = tmem_get_employee_events( $this->request['employee_id'] );
		} elseif ( isset( $this->request['client_id'] ) ) {
			$events = tmem_get_client_events( $this->request['client_id'] );
		} elseif ( isset( $this->request['date'] ) ) {
			$events = tmem_get_events_by_date( $this->request['date'] );
		} elseif ( isset( $this->request['status'] ) ) {
			$events = tmem_get_events_by_status( $this->request['status'] );
		} else {
		...
		if ( ! $events ) {
			$error = array();
			/* translators: %s: Event Type */
			$error['error'] = sprintf( esc_html__( 'No %s found.', 'mobile-events-manager' ), tmem_get_label_plural( true ) );
		...
		foreach ( $events as $event ) {
			$response['events'][ $event->ID ] = tmem_get_event_data( $event->ID );
			$i++;
		}
		...
		do_action( 'tmem_after_api_event_list', $this );
	...
	public function get_package() {
		...
		do_action( 'tmem_before_api_get_package', $this );

		if ( ! is_numeric( $this->request['package'] ) ) { // Using name or slug.
			$package = tmem_get_package_by( 'name', $this->request['package'] );
		} else {
			$package = tmem_get_package( $this->request['package'] );
		}
		...
		$response['package'] = tmem_get_package_data( $package );

		$response['package'] = array_merge( array( 'id' => $package->ID ), $response['package'] );

		$this->data = array_merge( $this->data, $response );

		do_action( 'tmem_after_api_get_package', $this );
	...
	public function list_packages() {
		...
		do_action( 'tmem_before_api_list_packages', $this );

		$all_packages = tmem_get_packages( array( 'suppress_filters' => false ) );

		if ( $all_packages ) {
			foreach ( $all_packages as $package ) {
				if ( isset( $this->request['employee_id'] ) && ! tmem_employee_has_package( $package->ID, $this->request['employee_id'] ) ) {
					continue;
				}
				if ( isset( $this->request['event_month'] ) && ! tmem_package_is_available_for_event_date( $package->ID, $this->request['event_month'] ) ) {
					continue;
				}
				if ( isset( $this->request['event_type'] ) && ! tmem_package_is_available_for_event_type( $package->ID, $this->request['event_type'] ) ) {
					continue;
				}
		...
		foreach ( $packages as $package ) {
			$response['packages'][ $package ] = tmem_get_package_data( $package );
			$i++;
		}
		...
		do_action( 'tmem_after_api_list_packages', $this );
	...
	public function package_options() {
		...
		$packages = tmem_package_dropdown( $args, false );
	...
	public function get_addon() {
		...
		do_action( 'tmem_before_api_get_addon', $this );

		if ( ! is_numeric( $this->request['addon'] ) ) { // Using name or slug.
			$addon = tmem_get_addon_by( 'name', $this->request['addon'] );
		} else {
			$addon = tmem_get_addon( $this->request['addon'] );
		}
		...
		$response['addon'] = tmem_get_addon_data( $addon );

		$response['addon'] = array_merge( array( 'id' => $addon->ID ), $response['addon'] );

		$this->data = array_merge( $this->data, $response );

		do_action( 'tmem_after_api_get_addon', $this );
	...
	public function list_addons() {
		...
		do_action( 'tmem_before_api_list_addons', $this );
		...
		if ( isset( $this->request['package'] ) ) {
			...
			if ( ! empty( $package_id ) ) {
				$package_addons = tmem_get_package_addons( $package_id );

				if ( $package_addons ) {
					foreach ( $package_addons as $package_addon ) {
						$all_addons[] = tmem_get_addon( $package_addon );
					}
				}
			}
		} else {
			$all_addons = tmem_get_addons( array( 'suppress_filters' => false ) );
		}

		if ( ! empty( $all_addons ) ) {

			foreach ( $all_addons as $addon ) {
				if ( isset( $this->request['employee_id'] ) && ! tmem_employee_has_addon( $addon->ID, $this->request['employee_id'] ) ) {
					continue;
				}
				if ( isset( $this->request['event_month'] ) && ! tmem_addon_is_available_for_event_date( $addon->ID, $this->request['event_month'] ) ) {
					continue;
				}
				if ( isset( $this->request['event_type'] ) && ! tmem_addon_is_available_for_event_type( $addon->ID, $this->request['event_type'] ) ) {
					continue;
				}
		...
		foreach ( $addons as $addon ) {
			$response['addons'][ $addon ] = tmem_get_addon_data( $addon );
			$i++;
		}
		...
		do_action( 'tmem_after_api_list_addons', $this );
