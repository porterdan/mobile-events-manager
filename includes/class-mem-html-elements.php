...
/**
 * MEM_HTML_Elements Class
 *
 * @since 1.3.7
 */
class MEM_HTML_Elements {
	...
	public function event_status_dropdown( $name = 'post_status', $selected = 0 ) {
		$event_statuses = mem_get_post_statuses( 'labels' );
		$options        = array();
		...
		$output = $this->select(
			array(
				...
				'data'             => array(
					...
					'search-placeholder' => sprintf( esc_html__( 'Type to search all %s statuses', 'mobile-events-manager' ), esc_html( mem_get_label_singular( true ) ) ),
				),
			)
		);
	...
	public function event_type_dropdown( $args = array() ) {
		...
		$defaults = array(
			...
			'class'            => '',
			'id'               => '',
			'selected'         => mem_get_option( 'event_type_default' ),
			'chosen'           => false,
			'placeholder'      => null,
			'multiple'         => false,
			'show_option_all'  => false,
			...
			'data'             => array(
				'search-type'        => 'type',
				/* translators: %s: different word for event */
				'search-placeholder' => sprintf( esc_html__( 'Type to search all %s types', 'mobile-events-manager' ), esc_html( mem_get_label_singular( true ) ) ),
			),
		);
		...
		$categories      = get_terms( 'event-types', apply_filters( 'mem_event_types_dropdown', $cat_args ) );
		$args['options'] = array();
		...
		$category_labels = mem_get_taxonomy_labels( 'event-types' );
	 ...
	 * @return str $output Category dropdown
	 */
	public function enquiry_source_dropdown( $name = 'mem_enquiry_source', $selected = 0 ) {

		$args = array(
			'hide_empty' => false,
		);

		$categories = get_terms( 'enquiry-source', apply_filters( 'mem_enquiry_source_dropdown', $args ) );
		$options    = array();

		if ( empty( $selected ) ) {
			$selected = mem_get_option( 'enquiry_source_default' );
		}
		...
		$category_labels = mem_get_taxonomy_labels( 'enquiry-source' );
		$output          = $this->select(
			array(
				'name'             => $name,
				'selected'         => $selected,
	 ...
	 * @return str $output Category dropdown
	 */
	public function txn_type_dropdown( $name = 'mem_txn_for', $selected = 0 ) {

		$args = array(
			'hide_empty' => false,
		);

		$categories = get_terms( 'transaction-types', apply_filters( 'mem_txn_types_dropdown', $args ) );
		$options    = array();
		...
		$category_labels = mem_get_taxonomy_labels( 'transaction-types' );
		$output          = $this->select(
			array(
				'name'             => $name,
				'selected'         => $selected,
	...
	public function month_dropdown( $args ) {
		...
		while ( $month <= 12 ) {
			$options[ absint( $month ) ] = mem_month_num_to_name( $month, $args['fullname'] );
			$month++;
		}
	...
	public function client_dropdown( $args = array() ) {
		...
		$clients = mem_get_clients( $args['roles'] );
		...
		if ( ! empty( $args['add_new'] ) ) {
			$options['mem_add_client'] = __( 'Add New Client', 'mobile-events-manager' );
		}
	...
	public function packages_dropdown( $args = array() ) {
		...
		if ( $packages ) {
			...
			foreach ( $packages as $package ) {
				...
				if ( $args['employee'] ) {
					if ( ! mem_employee_has_package( $package->ID, $args['employee'] ) ) {
						continue;
					}
				}

				if ( $args['event_type'] ) {
					if ( ! mem_package_is_available_for_event_type( $package->ID, $args['event_type'] ) ) {
						continue;
					}
				}

				if ( $args['event_date'] ) {
					if ( ! mem_package_is_available_for_event_date( $package->ID, $args['event_date'] ) ) {
						continue;
					}
				} else {
				...
				$price         = '';
				$package_price = mem_get_package_price( $package->ID, $args['event_date'] );
				if ( true === $args['cost'] ) {
					$price .= ' - ' . mem_currency_filter( mem_format_amount( $package_price ) );
				}

				$args['options'][ $package->ID ] = $package->post_title . '' . $price;

				$args['atts'][ $package->ID ] = array( 'price' => mem_sanitize_amount( $package_price ) );

				if ( $args['titles'] ) {
					$titles[ $package->ID ] = mem_get_package_excerpt( $package->ID );
				}
			}
		}
	...
	public function addons_dropdown( $args = array() ) {
		...
		$addons = mem_get_addons( $addons_args );
		...
		if ( $addons ) {
			foreach ( $addons as $addon ) {
				...
				$price       = '';
				$addon_price = mem_get_addon_price( $addon->ID, $args['event_date'] );
				if ( true === $args['cost'] ) {
					$price .= ' - ' . mem_currency_filter( mem_format_amount( $addon_price ) );
				}
				$desc = '';

				if ( $args['desc'] ) {
					$desc .= ' - ' . mem_get_addon_excerpt( $addon->ID, $args['desc'] );
				}
				...
				$args['atts'][ $addon->ID ] = array( 'price' => mem_sanitize_amount( $addon_price ) );

				if ( $args['titles'] ) {
					$titles[ $addon->ID ] = mem_get_addon_excerpt( $addon->ID );
				}
			}
		}
	...
	public function time_hour_select( $args = array() ) {
		...
		if ( 'H:i' === mem_get_option( 'time_format', 'H:i' ) ) {
			$i      = '00';
			$x      = '23';
			$format = 'H';
		} else {
	...
	public function time_minute_select( $args = array() ) {
		$options  = array();
		$minutes  = apply_filters( 'mem_time_minutes', array( '00', '15', '30', '45' ) );
		$defaults = array(
	...
	public function select( $args = array() ) {
		...
		if ( ! $args['options_only'] ) {
			$output .= '<select name="' . esc_attr( $args['name'] ) . '' . $multi . '" id="' . esc_attr( mem_sanitize_key( str_replace( '-', '_', $args['id'] ) ) ) . '" class="mem-select ' . $class . '"' . $multiple . ' data-placeholder="' . $placeholder . '"' . $data_elements . '>' . "\r\n";
		}
	...
	public function text( $args = array() ) {
		...
		if ( ! empty( $args['data'] ) ) {
			foreach ( $args['data'] as $key => $value ) {
				$data .= 'data-' . mem_sanitize_key( $key ) . '="' . esc_attr( $value ) . '" ';
			}
		}

		$output = '<span id="mem-' . mem_sanitize_key( $args['name'] ) . '-wrap">';

		$output .= '<label for="' . mem_sanitize_key( $args['id'] ) . '">' . esc_html( $args['label'] ) . '</label>';
	...
	public function date_field( $args = array() ) {
		...
		if ( empty( $args['class'] ) ) {
			$args['class'] = 'mem_datepicker';
		} elseif ( ! strpos( $args['class'], 'mem_datepicker' ) ) {
			$args['class'] .= ' mem_datepicker';
		}
	...
	public function textarea( $args = array() ) {
		...
		$output = '<span id="mem-' . mem_sanitize_key( $args['name'] ) . '-wrap">';
			...
			$output .= '<label for="' . mem_sanitize_key( $args['name'] ) . '">' . esc_html( $args['label'] ) . '</label>';

			$output .= '<textarea name="' . esc_attr( $args['name'] ) . '" id="' . mem_sanitize_key( $args['name'] ) . '" class="' . $class . '"' . $disabled . $placeholder . '>' . esc_attr( $args['value'] ) . '</textarea>';

		if ( ! empty( $args['desc'] ) ) {
			$output .= '<span class="mem-description">' . esc_html( $args['desc'] ) . '</span>';
		}
	...
	public function ajax_user_search( $args = array() ) {
		...
		$args['class'] = 'mem-ajax-user-search ' . $args['class'];

		$output      = '<span class="mem_user_search_wrap">';
			$output .= $this->text( $args );
			$output .= '<span class="mem_user_search_results hidden"><a class="mem-ajax-user-cancel" title="' . __( 'Cancel', 'mobile-events-manager' ) . '" aria-label="' . __( 'Cancel', 'mobile-events-manager' ) . '" href="#">x</a><span></span></span>';
		$output     .= '</span>';
