 ...
 * @since 1.4
 */
class MEM_Events_Query extends MEM_Stats {

	/**
	 * The args to pass to the mem_get_events() query
	 *
	 * @var array
	 * @access public
	 * @since 1.4
	...
	public function init() {
		add_action( 'mem_pre_get_events', array( $this, 'date_filter_pre' ) );
		add_action( 'mem_post_get_events', array( $this, 'date_filter_post' ) );

		add_action( 'mem_pre_get_events', array( $this, 'orderby' ) );
		add_action( 'mem_pre_get_events', array( $this, 'status' ) );
		add_action( 'mem_pre_get_events', array( $this, 'month' ) );
		add_action( 'mem_pre_get_events', array( $this, 'per_page' ) );
		add_action( 'mem_pre_get_events', array( $this, 'page' ) );
		add_action( 'mem_pre_get_events', array( $this, 'event_date' ) );
		add_action( 'mem_pre_get_events', array( $this, 'employee' ) );
		add_action( 'mem_pre_get_events', array( $this, 'client' ) );
		add_action( 'mem_pre_get_events', array( $this, 'search' ) );
		add_action( 'mem_pre_get_events', array( $this, 'source' ) );
		add_action( 'mem_pre_get_events', array( $this, 'type' ) );
	} // init
	...
	public function get_events() {
		do_action( 'mem_pre_get_events', $this );
		...
		if ( $query->have_posts() ) {
			while ( $query->have_posts() ) {
				...
				$this->events[] = apply_filters( 'mem_event', $event, $event_id, $this );
			}

			wp_reset_postdata();
		}

		do_action( 'mem_post_get_events', $this );
	...
	public function orderby() {
		switch ( $this->args['orderby'] ) {
			case 'event_date':
				...
				$this->__set( 'meta_key', '_mem_event_date' );
				break;
			case 'value':
				$this->__set( 'orderby', 'meta_value_num' );
				$this->__set( 'meta_key', '_mem_event_cost' );
				break;
			default:
	...
	public function search() {
		...
		if ( $is_email ) {
			...
			if ( is_object( $post ) && 'mem-event' === $post->post_type ) {
