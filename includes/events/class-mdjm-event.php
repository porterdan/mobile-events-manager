...
class MEM_Event {
	...
	public $post_title            = '';
	public $post_excerpt          = '';
	public $post_status           = 'mem-enquiry';
	...
	public function __get( $key ) {
		if ( method_exists( $this, 'get_' . $key ) ) {
			...
			return new WP_Error( 'mem-event-invalid-property', sprintf( esc_html__( "Can't get property %s", 'mobile-events-manager' ), $key ) );
		}
	} // __get
	...
	public function create( $data = array(), $meta = array() ) {
		...
		$default_meta = array(
			'_mem_event_date'            => gmdate( 'Y-m-d' ),
			'_mem_event_dj'              => ! mem_get_option( 'employer' ) ? 1 : 0,
			'_mem_event_playlist_access' => mem_generate_playlist_guest_code(),
			'_mem_event_playlist'        => mem_get_option( 'enable_playlists' ) ? 'Y' : 'N',
			'_mem_event_playlist_limit'  => mem_playlist_global_limit(),
			'_mem_event_contract'        => mem_get_default_event_contract(),
			'_mem_event_package_cost'    => 0,
			'_mem_event_addons_cost'     => 0,
			'_mem_event_travel_cost'     => 0,
			'_mem_event_additional_cost' => 0,
			'_mem_event_discount'        => 0,
			'_mem_event_cost'            => 0,
			'_mem_event_deposit'         => 0,
			'_mem_event_deposit_status'  => __( 'Due', 'mobile-events-manager' ),
			'_mem_event_balance_status'  => __( 'Due', 'mobile-events-manager' ),
			'mem_event_type'             => mem_get_option( 'event_type_default' ),
			'mem_enquiry_source'         => mem_get_option( 'enquiry_source_default' ),
			'_mem_event_venue_id'        => 'Manual',
			'_mem_event_tasks'           => array(),
		);

		$data = wp_parse_args( $data, $defaults );
		$meta = wp_parse_args( $meta, $default_meta );

		do_action( 'mem_event_pre_create', $data, $meta );
		...
		if ( $event ) {
			...
			if ( ! empty( $meta['mem_event_type'] ) ) {
				mem_set_event_type( $event->ID, $meta['mem_event_type'] );
				$meta['_mem_event_name'] = get_term( $meta['mem_event_type'], 'event-types' )->name;
				$meta['_mem_event_name'] = apply_filters( 'mem_event_name', $meta['_mem_event_name'], $id );
			}

			if ( ! empty( $meta['mem_enquiry_source'] ) ) {
				mem_set_enquiry_source( $event->ID, $meta['mem_enquiry_source'] );
			}

			if ( ! empty( $meta['_mem_event_start'] ) && ! empty( $meta['_mem_event_finish'] ) ) {

				if ( gmdate( 'H', strtotime( $meta['_mem_event_finish'] ) ) > gmdate( 'H', strtotime( $meta['_mem_event_start'] ) ) ) {
					$meta['_mem_event_end_date'] = $meta['_mem_event_date'];
				} else {
					$meta['_mem_event_end_date'] = gmdate( 'Y-m-d', strtotime( '+1 day', strtotime( $meta['_mem_event_date'] ) ) );
				}
			}

			if ( ! empty( $meta['_mem_event_package'] ) ) {
				$package_price                    = mem_get_package_price( $meta['_mem_event_package'], $meta['_mem_event_date'] );
				$meta['_mem_event_package_cost'] = $package_price;
				$meta['_mem_event_cost']        += $package_price;
			}

			if ( ! empty( $meta['_mem_event_addons'] ) ) {
				foreach ( $meta['_mem_event_addons'] as $addon ) {
					$addons_price                    = mem_get_addon_price( $addon );
					$meta['_mem_event_addons_cost'] = $addons_price;
					$meta['_mem_event_cost']       += $addons_price;
				}
			}

			if ( ! empty( $meta['_mem_event_travel_cost'] ) ) {
				$meta['_mem_event_cost'] += (float) $meta['_mem_event_travel_cost'];
			}

			if ( ! empty( $meta['_mem_event_additional_cost'] ) ) {
				$meta['_mem_event_cost'] += (float) $meta['_mem_event_additional_cost'];
			}

			if ( ! empty( $meta['_mem_event_discount'] ) ) {
				$meta['_mem_event_cost'] = $meta['_mem_event_cost'] - (float) $meta['_mem_event_discount'];
			}

			if ( empty( $meta['_mem_event_deposit'] ) ) {
				$meta['_mem_event_deposit'] = mem_calculate_deposit( $meta['_mem_event_cost'] );
			}

			mem_update_event_meta( $event->ID, $meta );

			wp_update_post(
				array(
					'ID'         => $id,
					'post_title' => mem_get_event_contract_id( $id ),
					'post_name'  => mem_get_event_contract_id( $id ),
				)
			);

		}

		do_action( 'mem_event_post_create', $id, $data );
	...
	public function get_event_data() {
		...
		$package_price   = $this->get_meta( '_mem_event_package_cost' );
		$addons          = $this->get_addons();
		$addons_price    = $this->get_meta( '_mem_event_addons_cost' );
		$travel_cost     = $this->get_meta( '_mem_event_travel_cost' );
		$additional_cost = $this->get_meta( '_mem_event_additional_cost' );
		$discount        = $this->get_meta( '_mem_event_discount' );
		...
		$this->data = array(
			...
			'contract_id'                     => mem_get_event_contract_id( $this->ID ),
			...
			'notes'                           => $this->get_meta( '_mem_event_notes' ),
			'package'                         => $package,
			...
			'primary_employee_payment_status' => $this->employee_id ? mem_event_employees_paid( $this->ID, $this->employee_id ) : false,
		...
		$this->data = apply_filters( 'mem_event_data', $this->data, $this );
	...
	public function get_client() {
		if ( ! isset( $this->client ) ) {
			$this->client = get_post_meta( $this->ID, '_mem_event_client', true );
		}
	...
	public function get_employee() {
		if ( ! isset( $this->employee_id ) ) {
			$this->employee_id = get_post_meta( $this->ID, '_mem_event_dj', true );
		}
	...
	public function get_all_employees() {
		...
		if ( ! isset( $this->employees ) ) {
			...
			if ( ! empty( $this->employee_id ) ) {
				...
				$payment_status = get_post_meta( $this->ID, '_mem_event_dj_payment_status', true );

				$employees[ $this->employee_id ] = array(
					'role_slug'      => 'dj',
					'role'           => translate_user_role( $wp_roles->roles['dj']['name'] ),
					'wage'           => mem_format_amount( get_post_meta( $this->ID, '_mem_event_dj_wage', true ) ),
			...
			$employees_data = get_post_meta( $this->ID, '_mem_event_employees_data', true );
	...
	public function get_contract() {
		...
		if ( ! $status || empty( $status ) ) {
			$contract = get_post_meta( $this->ID, '_mem_event_contract', true );
		} else {
			$contract = get_post_meta( $this->ID, '_mem_event_signed_contract', true );
		}

		return apply_filters( 'mem_event_contract', $contract, $this->ID );
	} // get_contract
	...
	public function get_contract_status() {
		if ( isset( $this->ID ) ) {
			...
			$signed_contract_id = get_post_meta( $this->ID, '_mem_event_signed_contract', true );

			if ( ! empty( $signed_contract_id ) && mem_contract_exists( $signed_contract_id ) && ( 'mem-approved' === $this->post_status || 'mem-approved' === $this->post_status || 'mem-awaitingdeposit' === $this->post_status ) ) {

				return apply_filters( 'mem_get_contract_status', $signed_contract_id, $this->ID );
	...
	public function get_date() {
		if ( ! isset( $this->date ) ) {
			$this->date = get_post_meta( $this->ID, '_mem_event_date', true );
		}
		 ...
		 */
		return apply_filters( 'mem_get_event_date', $this->date, $this->ID );
	} // get_date
	...
	public function get_long_date() {
		...
		return apply_filters( 'mem_event_long_date', $return, $this->date, $this->ID );
	} // get_long_date
	...
	public function get_short_date() {
		...
		if ( empty( $this->date ) ) {
			...
			$return = gmdate( mem_get_option( 'short_date_format', 'd/m/Y' ), strtotime( $this->date ) );
		}

		return apply_filters( 'mem_event_short_date', $return, $this->date, $this->ID );
	} // get_short_date
	...
	public function get_start_time() {
		$start = get_post_meta( $this->ID, '_mem_event_start', true );

		return apply_filters( 'mem_event_start', $start, $this->ID );
	} // get_start_time
	...
	public function get_finish_date() {
		$finish_date = get_post_meta( $this->ID, '_mem_event_end_date', true );
		 ...
		 * @param int $ID The event ID.
		 */
		return apply_filters( 'mem_event_finish_date', $finish_date, $this->ID );
	} // get_date
	...
	public function get_finish_time() {
		$finish = get_post_meta( $this->ID, '_mem_event_finish', true );

		return apply_filters( 'mem_event_finish', $finish, $this->ID );
	} // get_finish_time
	...
	public function get_setup_date() {
		$setup_date = get_post_meta( $this->ID, '_mem_event_djsetup', true );
		 ...
		 * @param int $ID The event ID.
		 */
		return apply_filters( 'mem_event_setup_date', $setup_date, $this->ID );
	} // get_setup_date
	...
	public function get_setup_time() {
		$setup_time = get_post_meta( $this->ID, '_mem_event_djsetup_time', true );

		return apply_filters( 'mem_event_setup_time', $setup_time, $this->ID );
	} // get_setup_time
	...
	public function get_name() {
		return mem_get_event_name( $this->ID );
	} // get_name
	...
	public function get_package() {
		return mem_get_event_package( $this->ID );
	} // get_package
	...
	public function get_package_price( $package = '', $date = '' ) {
		...
		$package_price = mem_get_package_price( $package, $date );
	...
	public function get_addons() {
		return mem_get_event_addons( $this->ID );
	} // get_addons
	...
	public function get_addons_price( $addons = array(), $date = '' ) {
		...
		foreach ( $addons as $addon ) {
			$addons_price = $addons_price + (float) mem_get_addon_price( $addon, $date );
		}
	...
	public function get_status() {
		...
		return apply_filters( 'mem_event_status', $return, $this->ID );
	} // get_status
	...
	public function get_type() {
		...
		return apply_filters( 'mem_event_type', $return, $this->ID );
	} // get_type
	...
	public function get_price() {
		...
		if ( ! isset( $this->price ) ) {
			...
			$this->price = get_post_meta( $this->ID, '_mem_event_cost', true );
		 ...
		 */
		return apply_filters( 'mem_get_event_price', $this->price, $this->ID );
	} // get_price
	...
	public function get_deposit() {
		...
		if ( ! isset( $this->deposit ) ) {
			...
			$this->deposit = get_post_meta( $this->ID, '_mem_event_deposit', true );
	...
	public function get_balance_status() {
		...
		if ( ! isset( $this->balance_status ) ) {
			...
			$this->balance_status = get_post_meta( $this->ID, '_mem_event_balance_status', true );

			if ( ! $this->balance_status || 'Paid' !== $this->balance_status || $this->get_price() > 0 ) {

				$this->balance_status = 'Due';

				if ( mem_sanitize_amount( $this->get_total_income() ) >= mem_sanitize_amount( $this->get_price() ) ) {
		 ...
		 * @param str|int $id The event ID.
		 */
		return apply_filters( 'mem_get_event_balance_status', $this->balance_status, $this->ID );
	} // get_balance_status
	...
	public function get_balance() {
		 ...
		 * @param str|int $id The event ID.
		 */
		return apply_filters( 'mem_get_event_balance', $this->balance, $this->ID );
	} // get_balance
	...
	public function get_total_income() {
		if ( ! isset( $this->income ) ) {
			...
			$this->income = '0.00';
			$cache_key    = md5( sprintf( 'mem_event_income_txns_%s', $this->ID ) );
			$txns         = get_transient( $cache_key );

			if ( false === $txns ) {
				$txns = mem_get_event_txns( $this->ID, array( 'post_status' => 'mem-income' ) );
	...
	public function get_total_outgoings() {
		if ( ! isset( $this->outgoings ) ) {
			...
			$this->outgoings = '0.00';
			$cache_key       = md5( sprintf( 'mem_event_outgoing_txns_%s', $this->ID ) );
			$txns            = get_transient( $cache_key );

			if ( false !== $txns ) {
				$txns = mem_get_event_txns( $this->ID, array( 'post_status' => 'mem-expenditure' ) );
				set_transient( $cache_key, $txns );
			}
	...
	public function get_wages_total() {
		 ...
		 * @param str|int $id The event ID.
		 */
		return apply_filters( 'get_wages_total', mem_format_amount( $wages ), $this->ID, $this->employees );
	} // get_wages_total
	...
	public function get_venue_id() {
		$venue_id = get_post_meta( $this->ID, '_mem_event_venue_id', true );
		 ...
		 */
		return apply_filters( 'mem_event_venue_id', $venue_id, $this->ID );
	} // get_venue_id
	...
	public function get_travel_data() {
		$travel_data = $this->get_meta( '_mem_event_travel_data' );
		 ...
		 */
		return apply_filters( 'mem_event_travel_data', $travel_data, $this->ID );
	} // get_travel_data
	...
	public function get_playlist_code() {
		if ( ! isset( $this->playlist_code ) ) {
			$this->playlist_code = get_post_meta( $this->ID, '_mem_event_playlist_access', true );
		}

		return apply_filters( 'mem_guest_playlist_code', $this->playlist_code, $this->ID );
	} // get_playlist_code
	...
	public function playlist_is_enabled() {
		...
		if ( 'Y' === get_post_meta( $this->ID, '_mem_event_playlist', true ) ) {
			$return = true;
		}

		return apply_filters( 'mem_playlist_status', $return, $this->ID );
	} // is_playlist_enabled
	...
	public function get_playlist_limit() {
		return mem_get_event_playlist_limit( $this->ID );
	} // get_playlist_limit
	...
	public function playlist_is_open() {
		...
		$date = get_post_meta( $this->ID, '_mem_event_date', true );
	...
	public function get_tasks() {
		$completed = $this->get_meta( '_mem_event_tasks' );
	...
	public function complete_task( $task = '' ) {
		...
		update_post_meta( $this->ID, '_mem_event_tasks', $this->tasks );
