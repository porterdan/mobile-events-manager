 ...
 * @since 1.4
 */
class TMEM_Event_Stats extends TMEM_Stats {
	...
	public function get_events( $start_date = false, $end_date = false, $event_date = false, $status = 'publish' ) {
		...
		add_filter( 'tmem_count_events_where', array( $this, 'count_events_where' ) );

		if ( is_array( $status ) ) {
			$count = 0;
			foreach ( $status as $event_status ) {
				$count += tmem_count_events()->$event_status;
			}
		} else {
			$count = tmem_count_events()->$status;
		}

		remove_filter( 'tmem_count_events_where', array( $this, 'count_events_where' ) );
