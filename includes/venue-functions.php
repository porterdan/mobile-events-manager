 ...
 */
function mem_add_venue( $venue_name = '', $venue_meta = array() ) {
	 ...
	 */
	$args = apply_filters( 'mem_add_venue', $args );

	/**
	 * Fire the `mem_pre_add_venue` action.
	 *
	 * @since 1.3
	 * @param str $name Name of venue
	 * @param arr $venue_meta Array of venue meta data
	 */
	do_action( 'mem_pre_add_venue', $name, $venue_meta );
	...
	/**
	 * Fire the `mem_post_add_venue` action.
	 *
	 * @since 1.3
	 * @param str $venue_id Post ID of new venue
	 */
	do_action( 'mem_post_add_venue', $venue_id );
 ...
 */
function mem_get_venues( $args = array() ) {
	...
	return apply_filters( 'mem_get_venues', $venues );
 ...
 */
function mem_get_event_venue_meta( $item_id, $field = '' ) {

	$prefix = '';

	if ( 'mem-venue' === get_post_type( $item_id ) ) {
		$venue_id = $item_id;
	} else {
		$venue_id = get_post_meta( $item_id, '_mem_event_venue_id', true );
		$prefix   = '_mem_event';
	...
	switch ( $field ) {
		...
		case 'details':
			$return = mem_get_venue_details( $venue_id );
			break;
 ...
 */
function mem_get_venue_details( $venue_id ) {
 ...
 */
function mem_venue_dropdown( $args = array() ) {

	$defaults = array(
		'name'              => '_mem_event_venue',
		'id'                => '',
		'selected'          => '',
		'first_entry'       => '',
		'first_entry_value' => '',
	...
	if ( ! empty( $venues ) ) {
		foreach ( $venues as $venue ) {
			$address  = mem_get_event_venue_meta( $venue->ID, 'address' );
			$town     = mem_get_event_venue_meta( $venue->ID, 'town' );
		...
		$output .= '<option value="" disabled="disabled">' . apply_filters( 'mem_no_venues', __( 'No venues exist', 'mobile-events-manager' ) ) . '</option>';
	}
 ...
 * @return str
 */
function mem_do_venue_details_table( $venue_id = '', $event_id = '' ) {
	...
	$venue_address  = mem_get_event_venue_meta( $venue_id, 'address' );
	...
	$venue_notes    = mem_get_event_venue_meta( $venue_id, 'notes' );
	$venue_details  = mem_get_venue_details( $venue_id );
	$employee_id    = ! empty( $event_id ) ? mem_get_event_primary_employee_id( $event_id ) : '';
	$output         = array();
	...
	ob_start(); ?>
	<div id="mem-venue-details-fields" class="mem-event-venue-details-sections-wrap">
		<div class="mem-custom-event-sections">
			<div class="mem-custom-event-section">
				<span class="mem-custom-event-section-title">
					<?php if ( ! empty( $venue_name ) ) : ?>
						<?php printf( esc_html__( 'Venue Details for %s', 'mobile-events-manager' ), esc_attr( $venue_name ) ); ?>
					<?php else : ?>
						<?php printf( esc_html__( 'No venue has been selected for this %s', 'mobile-events-manager' ), esc_html( mem_get_label_singular( true ) ) ); ?>
					<?php endif; ?>
				</span>

				<?php foreach ( $output as $key => $value ) : ?>
					<span class="mem-view-venue-<?php echo $key; ?>">
						<?php echo $value; ?>
					</span>
				<?php endforeach; ?>
				<?php do_action( 'mem_after_venue_notes', $venue_address, $employee_id ); ?>
			</div>
		</div>
	</div>
