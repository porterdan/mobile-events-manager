 ...
 */
function tmem_add_journal( $args = array(), $meta = array() ) {
	// Return if journaling is disabled.
	if ( ! tmem_get_option( 'journaling', false ) ) {
		return false;
	}
	...
	$comment_author = ( ! empty( $data['user_id'] ) ) ? get_userdata( $data['user_id'] ) : 'tmem';
	...
	do_action( 'tmem_pre_add_journal', $data, $meta, $comment_data );
	...
	$comment_meta = array(
		'tmem_type'       => ! empty( $meta['type'] ) ? $meta['type'] : 'tmem-journal',
		'tmem_visibility' => ! empty( $meta['visibility'] ) ? $meta['visibility'] : '0',
		'tmem_notify'     => ! empty( $meta['notify'] ) ? $meta['notify'] : '',
		'tmem_to'         => ! empty( $meta['to'] ) ? $meta['to'] : '',
		'tmem_isread'     => ! empty( $meta['isread'] ) ? $meta['isread'] : '',
	);
	...
	do_action( 'tmem_post_add_journal', $data, $meta, $comment_data );
 ...
 */
function tmem_get_journal_entries( $event_id ) {
	remove_action( 'pre_get_comments', 'tmem_hide_journal_entries', 10 );

	$journals = get_comments( array( 'post_id' => $event_id ) );

	add_action( 'pre_get_comments', 'tmem_hide_journal_entries', 10 );

	return $journals;
} // tmem_get_journal_entries
 ...
 * @return str
 */
function tmem_event_get_journal_entries_html( $note, $event_id = 0 ) {
	...
	if ( ! empty( $journal->user_id ) ) {
		...
		$user = __( 'TMEM Bot', 'mobile-events-manager' );
	}
...
} // tmem_event_get_journal_entries_html

/**
 * Exclude notes (comments) on tmem-event post type from showing in Recent
 * Comments widgets.
 *
 * @since 1.4.3
 ...
 * @return void
 */
function tmem_hide_journal_entries( $query ) {
	global $wp_version;

	if ( version_compare( floatval( $wp_version ), '4.1', '>=' ) ) {

		if ( isset( $_REQUEST['p'] ) && 'tmem-event' == get_post_type( sanitize_text_field( wp_unslash( $_REQUEST['p'] ) ) ) ) {
			return;
		}
 ...
 */
function tmem_hide_journal_entries_pre_41( $clauses, $wp_comment_query ) {
	...
	return $clauses;
} // tmem_hide_journal_entries_pre_41
add_filter( 'comments_clauses', 'tmem_hide_journal_entries_pre_41', 10, 2 );


/**
 * Exclude notes (comments) on tmem-event post type from showing in comment feeds.
 *
 * @since 1.4.3
 * @param arr $where
 * @param obj $wp_comment_query WordPress Comment Query Object.
...
/**
 * Remove TMEM Journal Comments from the wp_count_comments function.
 *
 * @since 1.4.3
 ...
 */
function tmem_remove_journal_entries_in_comment_counts( $stats, $post_id ) {
	global $wpdb, $pagenow;
	...
	if ( apply_filters( 'tmem_count_journal_entries_in_comments', false ) ) {
		return $stats;
	}
	...
	return $stats;
} // tmem_remove_journal_entries_in_comment_counts
add_filter( 'wp_count_comments', 'tmem_remove_journal_entries_in_comment_counts', 10, 2 );
