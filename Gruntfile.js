...
module.exports = function( grunt ) {
	...
	var pluginConfig = {
		...
		makepot: {
			target: {
				options: {
					...
					exclude: ['build/.*'],
					mainFile: 'mobile-events-manager.php', // Main project file.
					potFilename: 'mobile-events-manager.pot', // Name of the POT file.
					potHeaders: {
					...
					processPot: function( pot ) {
						pot.headers['report-msgid-bugs-to'] = 'https://tmem.co.uk/';
		...
		checktextdomain: {
			options:{
				text_domain: 'mobile-events-manager',
		...
		// svn settings.
		svn_settings: {
			path: 'https://plugins.svn.wordpress.org/mobile-events-manager/<%= pkg.name %>',
		...
		cssmin:	{
			build:	{
				files: {
					'<%= dirs.main.css %>/tmem-admin.min.css': ['<%= dirs.main.css %>/tmem-admin.css'],
					'<%= dirs.templates.css %>/tmem.min.css': ['<%= dirs.templates.css %>/tmem.css']
				}
			}
		},
		...
		uglify: {
			dist: {
				files: {
					...
					'<%= dirs.main.js %>/tmem-ajax.min.js': ['<%= dirs.main.js %>/tmem-ajax.js']
				}
			}
		},
