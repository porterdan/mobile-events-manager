=== TMEM Event Management ===
Contributors: jackjpdiscos
...
TMEM Event Management is the solution for Event based businesses who wish to automate their processes and enable clients to manage events.
...
TMEM Event Management allows you to manage your events from enquiry stage through to completion. Add your employees, your clients and then create an event. 

It's not just restricted to DJs though - We can support any type of event planner.
...
* Developer friendly
* A number of [plugin extensions](https://www.mobileeventsmanager.co.uk/add-ons/ "TMEM Add-ons") are available to further enhance the automation including Contact Forms, Payment Gateways and Calendar Sync
...
1. Enter "TMEM Event Management" into the Search Plugins text box and hit Enter
1. Click "Install Now" within the TMEM Event Management plugin box
...
1. Click "Choose File" and select the mobile-events-manager.zip file you downloaded
1. Click "Install Now"
1. Once installation has finished, select "Activate Plugin"
1. Once activation has completed, click the "Settings" link under the TMEM Event Management plugin
1. Installation has now completed. Next you need to [configure TMEM](https://www.mobileeventsmanager.co.uk/docs/installation/ "TMEM Initial Configuration").

== Frequently Asked Questions ==

= Is any support provided? =

Support can be obtained via our online [Support Pages](https://www.mobileeventsmanager.co.uk/support/ "TMEM Support").
Support is provided on a best-endeavours basis. For premium support you must be an active license holder for one of the [TMEM Event Management add-ons](https://www.mobileeventsmanager.co.uk/add-ons/ "TMEM Event Management add-ons")

= Is there a Pro version with additional features? =

Premium addons are available to enhance the plugin via our [Plugin Store](https://www.mobileeventsmanager.co.uk/add-ons/ "TMEM Add-ons")

== Screenshots ==

1. An overview of the TMEM Event Management Dashboard screen as seen by an Administrator when they are logged into the WordPress Admin UI
...
**Bug**: Incorrect return variable within `tmem_get_all_absences()` resulted in meta not being removed with absences
...
**New**: Added **Compliance** tab within TMEM Events -> Settings
**New**: Added ability to present and require acceptance of Privacy Policy for all TMEM related forms
**New**: Added ability to present and require acceptance of Terms & Conditions for all TMEM related forms (payment forms, contact forms, etc.)
**New**: User information stored as part of TMEM will be exported as part of the WordPress Export Personal Data process
**New**: Capture time of acceptance for privacy policy and terms and conditions
**New**: Added TMEM GDPR Privacy Policy template suggestion
...
**NOTE**: The `playlist-client.php`, `playlist-guest.php`, `profile-client.php` `tmem.css` and `tmem.min.css` files have been updated. If you have customized templates and copied to your theme, you will need to compare the changes
...
**Fix**; Correctly honour the min/max values for a number field within the `TMEM_HTML_Elements` class

= 1.4.7.8 =

**Released 10th January, 2018**

* IMPORTANT: An admin updating their own user profile may result in a loss of TMEM permissions
...
* Added option to export and import TMEM settings via the TMEM menu, Tools -> Import/Export so settings can be exported from one site and imported into another
...
* Added TMEM_Cache_Helper class to automatically prevent caching of TMEM pages - so long as caching plugins adhere to standards!
...
* Issue [#177](https://github.com/tmem/mobile-events-manager/issues/177/ "Issue #177") - Corrected end date format for exporting of transactions which was causing CSV export failure
* Issue [#176](https://github.com/tmem/mobile-events-manager/issues/176/ "Issue #176") - Incorrect URL format for redirects on availability check when there is no availability
* Issue [#174](https://github.com/tmem/mobile-events-manager/issues/174/ "Issue #174") - Some automated tasks were not running if the associated meta key was missing from the event
* Issue [#180](https://github.com/tmem/mobile-events-manager/issues/180/ "Issue #180") - Client shortcodes displayed employee details when templates are emailed to employees via the Communication feature
...
* Added `tmem_do_honeypot_check()` function
...
* Issue [#170](https://github.com/tmem/mobile-events-manager/issues/170/ "Issue #170") - Unattended Enquiries were missing when `Hide Inactive Events?` was enabled
* Issue [#166](https://github.com/tmem/mobile-events-manager/issues/166/ "Issue #166") - Package select field may generate a PHP notice if not set to an employer
* Issue [#167](https://github.com/tmem/mobile-events-manager/issues/167/ "Issue #167") - Journal entries are not visible
* Issue [#168](https://github.com/tmem/mobile-events-manager/issues/168/ "Issue #168") - Duplicate event statuses returned from tmem_all_event_status() function
...
* Issue [#162](https://github.com/tmem/mobile-events-manager/issues/162/ "Issue #162") - Reduced width of contract signing fields to avoid mis-alignment
* Corrected path to gateway log file. Logs were not being written
* Issue [#163](https://github.com/tmem/mobile-events-manager/issues/163/ "Issue #163") - Custom Event Field descriptions were not displayed correctly
...
* Allow passing of input field name within the /tmem/v1/addons/options/ API route to support Dynamic Contact Forms
...
* Issue [#151](https://github.com/tmem/mobile-events-manager/issues/151/ "Issue #151") - `{dj_email}` content tag does not render content
* Issue [#152](https://github.com/tmem/mobile-events-manager/issues/152/ "Issue #152") - Make sure Custom Client Field slugs do not contain non-standard characters and stripslashes from the labels
* Issue [#153](https://github.com/tmem/mobile-events-manager/issues/153/ "Issue #153") - Corrected display for the `{ddmmyyyy}` content tag

**Tweaks**

* Add the `data-cfasync="false"` attribute to TMEM JS scripts that are enqueued to tell CloudFlare Rocket Loader to ignore them
* Do not include journal entries in comment counts on the dashboard, or associated with the `tmem-event` post type
* Update all TMEM Journal entries to type `tmem-journal`
* Ensure we're using the `tmem_add_journal()` function and no other deprecated function to insert journal entries
...
* Issue [#144](https://github.com/tmem/mobile-events-manager/issues/144/ "Issue #144") - Incorrectly validating event ID renders *{event_name}* empty
* Issue [#143](https://github.com/tmem/mobile-events-manager/issues/143/ "Issue #143") - Addon price not displaying when using *{event_addons_cost}* shortcode
* Issue [#137](https://github.com/tmem/mobile-events-manager/issues/137/ "Issue #137") - Order packages and addons in ascending order
* Issue [#139](https://github.com/tmem/mobile-events-manager/issues/139/ "Issue #139") - Auto setting employee for Unattended Enquiries received via Contact Forms may obscure available packages and add-ons
...
**[REST API](https://www.mobileeventsmanager.co.uk/docs/api/tmem-rest-api-introduction/ "TMEM REST API") introduced**

A number of [API endpoints](https://www.mobileeventsmanager.co.uk/docs/api/tmem-rest-api-endpoints/) have been introduced enabling developers to easily and securely integrate and query TMEM.

In order to use an API endpoint, a user must have an API key and token which can be generated under *TMEM->Tools->API Keys*. Exceptions for the Availability and all Package/Addon related endpoints, which are public and require no authentication to generate a response.

**Endpoints Include**

* [/event/](https://www.mobileeventsmanager.co.uk/docs/api/tmem-rest-api-event/ "Event REST API Endpoint") to query a single event by ID
* [/events/](https://www.mobileeventsmanager.co.uk/docs/api/tmem-rest-api-events/ "Events REST API Endpoint") to retrieve all events or query multiple events by date, employee, client, or status
* [/employee/](https://www.mobileeventsmanager.co.uk/docs/api/tmem-rest-api-employee/ "Employee REST API Endpoint") to retrieve an employees details by their user ID
* [/client/](https://www.mobileeventsmanager.co.uk/docs/api/tmem-rest-api-client/ "Client REST API Endpoint") to retrieve a clients details by their user ID
* [/availability/](https://www.mobileeventsmanager.co.uk/docs/api/tmem-rest-api-availability/ "Availability REST API Endpoint") to query availability by date. Further filtering by employees or roles is also possible
* [/package/](https://www.mobileeventsmanager.co.uk/docs/api/tmem-rest-api-package/ "Package REST API Endpoint") to retrieve information regarding a single equipment package
* [/packages/](https://www.mobileeventsmanager.co.uk/docs/api/tmem-rest-api-packages/ "Packages REST API Endpoint") to retrieve multiple equipment packages. Filtering possible
* [/addon/](https://www.mobileeventsmanager.co.uk/docs/api/tmem-rest-api-addon/ "Addon REST API Endpoint") to retrieve information regarding a single addon
* [/addons/](https://www.mobileeventsmanager.co.uk/docs/api/tmem-rest-api-addons/ "Addons REST API Endpoint") to retrieve multiple equipment addons. Filtering possible.
...
* Settings for Packages and Addons have been moved to *TMEM -> Settings -> Events -> Packages & Add-ons*
...
* [Content Tags](https://www.mobileeventsmanager.co.uk/docs/content-tags/ "Content Tags") added to output travel data including cost, distance, directions and time

**Tweaks**

* Add songs to playlist from within admin
* Added System Info data under *TMEM->tools*. May be requested during support requests
...
* Availability checker on the front end now uses TMEM REST API if Ajax enabled
* Improved licensing validation for addons
* Added event count to *At a Glance* dashboard widget

**Bug Fixes**

* TMEM Event Management user fields not updating within Admin user profile if value is removed
...
* Corrected message parameter when data is missing from contract signing form. Fixes issue [#124](https://github.com/tmem/mobile-events-manager/issues/124/ "Issue #124")
* Better validation of checkboxes for contract signing form. Fixes issue [#126](https://github.com/tmem/mobile-events-manager/issues/126/ "Issue #126")
* User ID is not converted to integer value in URL string. Fixes issue [#125](https://github.com/tmem/mobile-events-manager/issues/125/ "Issue #125")
...
**Bug Fixes**
* Corrected content tag for retrieving `{dj_email}`. Fixes issue [#121](https://github.com/tmem/mobile-events-manager/issues/121/ "Issue #121")

**New**
* Added `{dj_lastname}` content tag.

**Tweaks**
* Added `tmem_get_employee_phone()` and `tmem_get_employee_alt_phone()` functions
* Added `tmem_get_client_login()` and `tmem_get_client_alt_phone()` functions
...
**Bug Fixes**
* Corrected hook for setting custom artist label. `DJ` will now be correctly substituted. Fixes issue [#104](https://github.com/tmem/mobile-events-manager/issues/104/ "Issue #104")
* `tmem_get_addons()` and `tmem_get_packages()` may have generated a PHP notice if no add-ons/packages were defined. Fixes issue [#118](https://github.com/tmem/mobile-events-manager/issues/118/ "Issue #118")
...
**Tweaks**
* Added `tmem-tick` css class within admin
...
* Corrected shortcode `[tmem-quote]` in description for `quotes_page` setting option. Props @nikmartin
* Added hooks to the new client table on the events screen
* Added hooks `tmem_before_add_new_client` and `tmem_after_add_new_client`
* Corrected hook name within `tmem_process_payment_form()`
* Added hooks `tmem_before_events_overview` and `tmem_after_events_overview` to overview widget
...
* Issue [#108](https://github.com/tmem/mobile-events-manager/issues/108/ "Issue #108") - Missing css class selector for event action buttons

**Tweaks**

* Added `Failed` as transaction status
* Added function `tmem_get_required_payment_fields()` function to enable registration of required fields on the payments form
* Added function `tmem_required_payment_field()` function to determine if a payment form field is required
...
* TMEM now supports multiple payment gateways

**Bug Fixes**

* Issue [#108](https://github.com/tmem/mobile-events-manager/issues/108/ "Issue #108") When creating an event set the default venue ID as manual to capture fields from Contact Forms
...
* Added `tmem_update_txn_status()` function for easier status updates of transactions
* Added `tmem_has_gateway()` function to check if a gateway is enabled
...
* Added `tmem_send_gateway_receipt()` function which is called as part of the `complete event payment process` hooks
* Added function `tmem_get_user_ip()` to determine users IP address
...
**Tweaks**
* Added new tmem-alert css classes to be introduced across the system in due course
...
**Bug Fixes**
* Do not save event cost as a formatted value. Causes issues with some currencies. Issue [#105(https://github.com/tmem/mobile-events-manager/issues/105/ "Issue #105")

**Tweaks**
* Added function `tmem_get_event_txns()`
* Added function `tmem_list_event_txns()`
* Use `tmem_get_event_txns()` when calculating total event income
* Use `tmem_list_event_txns()` for `{payment_history}` content tag
...
* Issue [#100](https://github.com/tmem/mobile-events-manager/issues/100/ "Issue #100") PLaylist count was not displayed correctly when using view filters
...
* Spelling correction in `$tmem_allowed_taxonomies` variable

= 1.3.7.5 =

**Released, Saturday 25th June 2016**
**Bug Fixes**
* Issue [#101](https://github.com/tmem/mobile-events-manager/issues/101/ "Issue #101") Corrected {guest_playlist_url} output and redirect visits to incorrect URL

= 1.3.7.4 =

**Released, Friday 24th June 2016**
**Bug Fixes**
* Issue [#97](https://github.com/tmem/mobile-events-manager/issues/97/ "Issue #97") Process deposit/balance payments after meta updates in case their values have changed
* Issue [#96](https://github.com/tmem/mobile-events-manager/issues/96/ "Issue #96") Corrected display of un-saved event address fields and ensure that the correct value is initially selected for the venue list
...
**New**
* Select how you want events to be sorted on the admin screen. `Default Order By` and `Default Order` setting options added within TMEM->Settings->Events

**Bug Fixes**
* Issue [#94](https://github.com/tmem/mobile-events-manager/issues/94/ "Issue #94") Venue content tags not returning correct values
* Issue [#84](https://github.com/tmem/mobile-events-manager/issues/84/ "Issue #84") `Enable Playlist` not remaining checked
* Issue [#93](https://github.com/tmem/mobile-events-manager/issues/93/ "Issue #93") Event type filter should display event count for each event type
* Issue [#92](https://github.com/tmem/mobile-events-manager/issues/92/ "Issue #92") Maintain current event status view when using event filters
* Issue [#90](https://github.com/tmem/mobile-events-manager/issues/90/ "Issue #90") Save the `DJ` setup date value when after it is auto populated
* Issues [#88](https://github.com/tmem/mobile-events-manager/issues/88/ "Issue #88") and [#83(https://github.com/tmem/mobile-events-manager/issues/83/ "Issue #83") Deposit Paid should not be checked when creating a new event. Ensure that the Balance Paid checkbox holds is checked status

**Tweaks**
* Remove deprecated use of `TMEM_MULTI` from the `tmem_event_post_filter_list()` function
* Added `tmem_get_employee_address()` function
* Added `tmem_get_employee_post_code()` function
...
* Request [#76](https://github.com/tmem/mobile-events-manager/issues/76/ "Request #76") Added content tag `{client_alt_phone}` to display the clients additional phone # if provided
* Request [#79](https://github.com/tmem/mobile-events-manager/issues/79/ "Request #79") Form validation on communication page
...
* Pre-load $employee_id within the TMEM_Event class
* Added function `tmem_get_clients_next_event()` to replace method in deprecated TMEM_Events class
...
* New class `TMEM_HTML_Elements` for easier display of input fields
* Custom client fields are now displayed two fields per row
* Custom event fields are now displayed two fields per row
* Added function `tmem_get_event_remaining_deposit()`
* Added functions `tmem_get_taxonomy_labels()` and `tmem_get_post_statuses()`
* Added function `tmem_month_num_to_name()`
* Added function `tmem_sanitize_key()`
* Added function `tmem_get_event_deposit_type()`
* Added `get_start_time()`, `get_finish_time(`, `get_package()`, `get_addons()` to TMEM_Event class
* Added method `get_remaining_deposit()` to TMEM_Event class
* Make sure all ajax functions are prefixed `tmem_` and suffixed `_ajax`
* Better formatting of currencies
* Added functions `tmem_get_enquiry_sources()` and `tmem_enquiry_sources_dropdown()`
* Added client functions `tmem_get_client_full_address()`, `tmem_get_client_last_login()`, `tmem_do_client_details_table`
* Removed deprecated methods from soon to be deprecated TMEM_Txns class
...
* Issue [#70](https://github.com/tmem/mobile-events-manager/issues/70/ "Issue 70") Client phone number does not save when created on the events screen
* Issue [#72](https://github.com/tmem/mobile-events-manager/issues/72/ "Issue 72") Use remove_query_arg for redirect link on login page
* Issue [#73](https://github.com/tmem/mobile-events-manager/issues/73/ "Issue 73") Admin is unable to review signed contracts
* Issue [#75](https://github.com/tmem/mobile-events-manager/issues/75/ "Issue 75") `Unattended Enquiry` and `Enquiry` admin bar links had extra whitespace causing link to malfunction
* Issue [#77](https://github.com/tmem/mobile-events-manager/issues/77/ "Issue 77") `{application_home}` content tag not correctly returning URL

= 1.3.6 =

* **New** : TMEM now uses internal login functions for the Client Zone login process
* **New** : Added `[tmem-login]` shortcode to present a login form on any page
* **New** : Added `{event_employees}` and `{event_employees_roles}` content tags
* **Bug Fix**: Issue [#68](https://github.com/tmem/mobile-events-manager/issues/68/ "Issue 68") `{guest_playlist_url}` is missing part of the string

= 1.3.5.5 =

* **Bug Fix**: Issue [#66](https://github.com/tmem/mobile-events-manager/issues/66/ "Issue 66") Template content does not load when a recipient is auto loaded into the communication page
...
* **Bug Fix**: Issue [#63](https://github.com/tmem/mobile-events-manager/issues/63/ "Issue 63") Incorrect formatting caused transactions over 999.99 to be displayed as 1
...
* **Tweak** : Added `Added method get_remaining_deposit()` to TMEM_Event class
...
* **Tweak** : Moved `TMEM_Employee_Table` class to start of file 
...
* **Bug Fix**: Issue [#60](https://github.com/tmem/mobile-events-manager/issues/60/ "Issue 60") Random echo statement causes PHP notice on employee permission update
...
* **Bug Fix**: Issue [#53](https://github.com/tmem/mobile-events-manager/issues/53/ "Issue 53") Do not double format the event deposit cost
* **Bug Fix**: Issue [#54](https://github.com/tmem/mobile-events-manager/issues/50/ "Issue 54") Display action buttons even when no event ID is passed
* **Tweak**: Allow filtering of the number of action buttons being displayed via `add_filter( 'tmem_action_buttons_in_row', '*your_function*' );`. Default is 4
...
* **Bug Fix**: Issue [#50](https://github.com/tmem/mobile-events-manager/issues/50/ "Issue 50"). Extra div close tag on quotes page
* **Bug Fix**: Issue [#51](https://github.com/tmem/mobile-events-manager/issues/51/ "Issue 51"). Availability widget does not redirect with Ajax turned on
* **Bug Fix**: Issue [#49](https://github.com/tmem/mobile-events-manager/issues/49/ "Issue 49"). Correctly decode html entities within email subject
* **Bug Fix**: Issue [#47](https://github.com/tmem/mobile-events-manager/issues/47/ "Issue 47"). `{quotes_url}` content tag missing event ID
* **Bug Fix**: Issue [#46](https://github.com/tmem/mobile-events-manager/issues/46/ "Issue 46"). Fixed deposits not inserted if event has zero cost
* **Bug Fix**: Re-added package and add-on pricing to events edit screen
* **Tweak**: Added translation to bulk action entry `Add Role`
* **Tweak**: Removed deprecated constant TMEM_PAYMENTS
* **Tweak**: Added `tmem_compare_template_version()` and `tmem_get_template_files()` functions

= 1.3.4.1 =

* **Bug Fix**: Issue [#41](https://github.com/tmem/mobile-events-manager/issues/41/ "Issue 41"). Rejected enquiries need to have status updated even if no reject reason is provided.
* **Bug Fix**: Issue [#42](https://github.com/tmem/mobile-events-manager/issues/41/ "Issue 42"). `{contract_url}` generating incorrect output.

= 1.3.4 =

* **New**: Record employee event payments feature. See [Employee Event Payments](https://www.mobileeventsmanager.co.uk/docs/employee-event-payments/ "TMEM Employee Event Payments")
...
* **New**: Added `{available_addons}` and `{available_addons_cost}` content tags. See [Content Tags](https://www.mobileeventsmanager.co.uk/docs/content-tags/ "Content Tags Available with TMEM Event Management")
* **New**: Added `tmem_get_clients_next_event()` function
...
* **Bug Fix**: Issue #39. Check for `tmemeventid` on guest playlist access to maintain backwards compatibility for existing events
* **Tweak**: Improved license handler class
* **Tweak**: Renamed core.php to plugins.php
* **Tweak**: Removed use of deprecated `TMEM_EVENT_POSTS` constant
...
* **Bug Fix**: Issue #27. Custom Event fields with a `\` in the name do not get processed. Re-check your custom content tags at *TMEM -> Events -> Custom Event Fields*
...
* **Tweak**: Removed deprecated file tmem-functions-admin.php
...
* **Tweak**: Removed deprecated `send_to_email()` method within the `TMEM_PlayList_Table` class
...
* **New**: Use fully customisable templates for all TMEM Client Zone pages
...
* **New**: Added Event Staff checkbox on user profile screen for administrators. Check to tell TMEM that the admin is an TMEM employee otherwise they have no TMEM permissions
...
* **Tweak**: *TMEM Overview* dashboard widget has been updated and now displays event and earnings overviews for Month to Date, Year to Date and the previous year
* **Tweak**: Display names of employees who received a copy of a tracked email within the Communication history page
* **Tweak**: Main TMEM class is now a singleton class
* **Tweak**: Post actions and filters no longer in classes
* **Tweak**: Replaced get_link method with tmem_get_formatted_url function
* **Tweak**: New tmem.css file. Can be customised for front end
* **Tweak**: Added content filters for emailing and printing the event playlist
* **Tweak**: Cleaner files and directory structure
* **Tweak**: Don't restrict access to JetPack and don't hide WP menus for TMEM roles. Caused some conflicts with other plugins
...
* **Bug Fix**: No TMEM data should be returned from a front end search
...
**Released 25th November, 2015**
	...
	* **Tweak**: Accomodate changes in other TMEM plugins
...
**Released 22nd November, 2015**
	...
	* **Tweak**: TMEM Shortcodes button renamed to TMEM and new structure and options added
	...
	* **Tweak**: Replace Mobile Events Manager with TMEM in WP dashboard widgets
	* **Tweak**: Change title to TMEM Event Management in TMEM dashboard
	* **Bug Fix**: User roles should only register during install
	* **Bug Fix**: WP Dashboard TMEM Overview now has correct edit URL
...
**Released 31st October, 2015**
	...
	* **Tweak**: Preparation for TMEM to PDF extension
	* **Tweak**: Rebranded to TMEM Event Management on the plugin screen
	* **Tweak**: Rebranded to TMEM Events on the menu and admin bar
 ...
 * **New**: Event playlists can now be controlled per event. If not enabled, the Manage Playlist action button is not displayed within the Client Zone
	...
	* **New**: Added `tmem_event_metaboxes` developer action hook
...
**Released 22nd October, 2015**
	...
	* **New**: Support for TMEM Google Calendar Sync add-on
	...
	* **Bug Fix**: Fixed availability checker function on TMEM Dashboard
...
**Released Wednesday, 23rd September, 2015**
	...
	* **Tweak**: Added OOP code for TMEM extensions - coming soon
...
**Released Saturday 12th September, 2015**
	...
	* **New**: When your client makes an online payment, if the Payment Gateway charges for the transaction, that charge is recorded within TMEM
	...
	* **New**: Notify Admin?<a href="<?php echo tmem_get_admin_page( 'clientzone_settings' ); ?>">Setting</a> added and enabled by default. With selected, admin will receive email notification when a client accepts a quotation, or signs a contract via the <?php echo tmem_get_application_name(); ?>
...
**Released Thursday 20th August, 2015**
	...
	* **Bug Fix**: issue with validation of the date field when used with #TMEM Contact Forms
...
**Released Wednesday 19th August, 2015**
	...
	* **Bug Fix**: Playlist upload to #TMEM error
	* **Bug Fix**: Changing Packages &amp; Addons for existing events now correctly re-calculates the event cost
	* **Bug Fix**: Playlist entries are now successfully submitted to the TMEM servers when songs and artists contain apostraphe's
...
**Released Sunday 28th June, 2015**
	...
	* **Bug Fix**: Playlist submission to TMEM date error
...
Join our Facebook group for all the latest discussions, news and more - https://www.facebook.com/groups/mobiledjmanager
	...
	* **Tweak**: All outbound emails are sent from the defined system address. If your settings dictate that emails come from DJ's,
		...
		DJ's who have email addresses that do not end in the same domain name as the website where TMEM is installed, cannot send emails
	...
	* **Tweak**: Begun updating <?php echo tmem_get_application_name(); ?> pages for HTML5 &amp; CSS3 compliance. Not yet completed
	* **New**: Create backups of the TMEM database tables and download within the debugging screen
...
**Released Thursday 14th May, 2015**
	...
	* **Tweak**: You can now add custom content above TMEM shortcode content within Client Zone pages
...
Watch out for the next major release of TMEM...coming soon to include a re-designed Event interface, greater email tracking, faster response times and much more!
...
TMEM Event Management for WordPress is now fully integrated with PayPal enabling you to take online payments securely via your website.
	...
	* Full PayPal integration means the TMEM application receives information from the PayPal IPN API system and updates the booking and journal automatically after verifying payment is completed
...
**Transaction Page**
A new Transactions page has been added to the TMEM system and is available via the TMEM Event Management menu's within the WordPress admin interface.
...
**Bug Fixes &amp; Minor Enhancements**
		...
		* **New**: Setting added **Deposit Label** enabling you to change the terminology used on both the front end and backend of your website. Some people prefer <code>Booking Fee</code> for example. Defaults to **Deposit**. Retmember to update any email or contract templates as necessary
		 * **New**: Setting added **Balance Label** enabling you to change the terminology used on both the front end and backend of your website. Defaults to <code>Balance</code>. Retmember to update any email or contract templates as necessary
		...
		* **Bug Fix**: WordPress "reserves" some form field names such as **name** so if you used this field name within the TMEM Contact Forms, the form did not submit correctly. This is rectified
...
Our fully customisable Contact Forms enable full management of events from the initial client enquiry all the way through to the completion of the event.
	...
	* jQuery Validation
	* Point successful Availability Checks to your TMEM Contact Form page for additional functionality


**Bug Fixes &amp; Minor Enhancements**

	* **New**: Create Clients directly from the Add New Event screen as part of the event creation process
	* **New**: TMEM Contact Form Widget enabling you to add your TMEM Contact Form to multiple web pages quickly and easily
...
= 1.4.7.8 =
Resolves an issue that may result in administrators losing TMEM permissions. Upgrade ASAP.
